...
class DocumentClassification(BaseModel):
    ...
    sectionNumbersCovered: List[str] = Field(
        description="List of section numbers covered in this block (e.g., ['1.1', '1.2', '2.0'])", default=[]
    )
...
class DomainExtractor:
    ...
    async def get_block_sections(self, text_blocks) -> List[str]:
            ...
            doc.update(
                {
                    ...
                    "sectionNumbersCovered": metadata.sectionNumbersCovered
                }
            )
    ...
    async def _create_placeholder(self, session, url, data, headers) -> dict:
    ...
    async def _get_signed_url(self, session, url, data, headers) -> dict:
    ...
    async def _upload_to_signed_url(self, session, signed_url, data) -> int:
    ...
    async def save_summary_to_storage(
        ...
        try:
            ...
            for data in block_data:
                enhanced_block_data.append({
                    "block_id": data["_key"],
                    "block_num": data["blockNum"],
                    "block_text": data["text"],
                    "block_summary": data["summary"],
                    "section_numbers_covered": data["sectionNumbersCovered"]
                })
            ...
            upload_data = {
                ...
                "summary": summary_doc,
                "block_data": enhanced_block_data if enhanced_block_data else []
            }
    ...
    async def delete_summary_from_storage(self, org_id: str, record_id: str, summary_document_id: str = None) -> bool:
        """Delete summary document from storage

        Args:
            org_id (str): Organization ID
            record_id (str): Record ID to delete
            summary_document_id (str): Summary document ID to delete
        Returns:
        ...
        try:
            ...
            # Make DELETE request
            async with aiohttp.ClientSession() as session:
                url = f"{nodejs_endpoint}{Routes.STORAGE_DELETE.value.format(documentId=summary_document_id)}"

                async with session.delete(url, headers=headers) as response:
                    if response.status == StatusCodeConstants.NOT_FOUND.value:
                        self.logger.warning("⚠️ Document not found: %s", summary_document_id)
                        return True

                    if response.status not in (StatusCodeConstants.SUCCESS.value, StatusCodeConstants.NO_CONTENT.value):
                        error_text = await response.text()
                        self.logger.error(
                    ...
                    self.logger.info("✅ Successfully deleted document: %s", summary_document_id)
                    return True
