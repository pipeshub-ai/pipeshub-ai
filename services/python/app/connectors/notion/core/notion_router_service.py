...
class NotionRouterService:
    ...
    async def fetch_file_data(self, block_id: str):
        ...
        try:
            ...
            file_data_response = None
            if file_data:
                if block_format == "txt" and isinstance(file_data, bytes):
                    # For text files, include the actual text content
                    try:
                        file_data_response = file_data.decode('utf-8')
                    except UnicodeDecodeError:
                        # If can't decode as text, use base64
                        file_data_response = base64.b64encode(file_data).decode('utf-8')
                else:
