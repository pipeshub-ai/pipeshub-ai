...
from app.config.utils.named_constants.arangodb_constants import (
    CollectionNames, EventTypes, ExtensionTypes, MimeTypes, ProgressStatus
)
...
class EventProcessor:
    ...
    async def _download_from_signed_url(self, signed_url: str, record_id: str, doc: dict) -> bytes:
        """
        ...
        for attempt in range(max_retries):
            ...
            try:
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.get(signed_url) as response:
                        if response.status != 200:
                            raise aiohttp.ClientError(f"Failed to download file: {response.status}")
                ...
                self.logger.warning(f"Download attempt {attempt + 1} failed: {str(e)}. Retrying in {delay} seconds...")
                
                if attempt == max_retries - 1:  # Last attempt failed
                    self.logger.error(f"❌ All download attempts failed for record {record_id}: {repr(e)}")
                    doc.update({
                        "indexingStatus": ProgressStatus.FAILED.value,
                        "extractionStatus": ProgressStatus.FAILED.value,
                        "reason": f"Download failed after {max_retries} attempts: {str(e)}. File id: {record_id}"
                    })
                    await self.arango_service.batch_upsert_nodes([doc], CollectionNames.RECORDS.value)
                    raise Exception(f"Download failed after {max_retries} attempts: {str(e)}. File id: {record_id}")
    ...
    async def on_event(self, event_data: dict):
        ...
        try:
            ...
            supported_mime_types = [MimeTypes.GMAIL.value, 
