...
class TestAPIIntegration:
        ...
        if len(available_services) > 0:
            ...
            for service_name, response in available_services.items():
                ...
                if data and "status" in data:
                    print(f"{service_name} has consistent health format")
                else:
                    print(f"{service_name} has different health format: {list(data.keys()) if data else 'None'}")
        else:
            print("No services available for consistency testing")
    ...
    async def test_api_timeout_handling(self, http_client, test_config):
        ...
        for service_name, url in services:
            try:
                response = await http_client.get(f"{url}/health", timeout=short_timeout)
                print(f"{service_name} responded within 100ms (unexpected)")
            except httpx.TimeoutException:
                print(f"{service_name} timeout handling working")
            except httpx.RequestError:
                print(f"{service_name} not available for timeout testing")
    
    @pytest.mark.asyncio
    async def test_api_cors_headers(self, http_client, test_config):
        ...
        for service_name, url in services:
            try:
                ...
                response = await http_client.options(f"{url}/health", timeout=5.0)
                ...
                if response.status_code in [200, 204]:
                    ...
                    if any(cors_headers.values()):
                        print(f"{service_name} has CORS headers: {cors_headers}")
                    else:
                        print(f"{service_name} doesn't have CORS headers")
                else:
                    print(f"{service_name} OPTIONS request returned {response.status_code}")
                    
            except httpx.RequestError:
                print(f"{service_name} not available for CORS testing")
    
    @pytest.mark.asyncio
    async def test_api_content_type_headers(self, http_client, test_config):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "application/json" in content_type:
                        print(f"{service_name} returns JSON content type")
                    else:
                        print(f"{service_name} content type: {content_type}")
                else:
                    print(f"{service_name} returned status {response.status_code}")
                    
            except httpx.RequestError:
                print(f"{service_name} not available for content type testing")
