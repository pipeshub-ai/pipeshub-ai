...
def _sanitize_sensitive_data(data: Union[Dict[str, Any], list, str, int, float, bool, None]) -> Union[Dict[str, Any], list, str, int, float, bool, None]:
    """Remove sensitive fields from data for logging."""
    if not isinstance(data, dict):
        return data

    sensitive_fields = {'password', 'token', 'secret', 'api_key', 'access_token', 'refresh_token', 'authorization'}
    sanitized = data.copy()
    ...
    for key in sanitized:
        if isinstance(key, str) and key.lower() in sensitive_fields:
            ...
            sanitized[key] = _sanitize_sensitive_data(sanitized[key])
...
class HTTPClient:
    ...
    def _log_request(self, method: str, url: str, **kwargs) -> None:
        ...
        if "json" in kwargs:
            sanitized_json = _sanitize_sensitive_data(kwargs['json'])
            logger.debug(f"  Body: {json.dumps(sanitized_json, indent=2)}")
        if "params" in kwargs:
    ...
    def _log_response(self, response: Response) -> None:
        ...
        try:
            sanitized_json = _sanitize_sensitive_data(response.json())
            logger.debug(f"  Body: {json.dumps(sanitized_json, indent=2)}")
        except Exception:
