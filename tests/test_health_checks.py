...
import pytest
import httpx
from conftest import ServiceHealthChecker
...
class TestHealthUtilities:
    ...
    def test_health_checker_initialization(self, http_client):
        """Test that health checker can be initialized."""
        checker = ServiceHealthChecker(http_client)
        assert checker is not None
        assert checker.client == http_client
    
    @pytest.mark.asyncio
    async def test_health_checker_with_invalid_url(self, http_client):
        """Test health checker behavior with invalid URL."""
        checker = ServiceHealthChecker(http_client)
        
        # Test with non-existent service
        result = await checker.check_python_service("http://localhost:9999", "nonexistent")
    ...
    @pytest.mark.asyncio
    async def test_health_checker_with_malformed_url(self, http_client):
        """Test health checker behavior with malformed URL."""
        checker = ServiceHealthChecker(http_client)
        
        # Test with malformed URL
        result = await checker.check_python_service("not-a-valid-url", "malformed")
