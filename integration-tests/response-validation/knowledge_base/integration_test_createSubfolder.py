"""
Knowledge Base API – Create Subfolder Response Validation Integration Tests
===========================================================================

Tests POST /api/v1/knowledgeBase/{kbId}/folder/{folderId}/subfolder
(``createSubfolder``) against OpenAPI schemas.

**Request:** required ``folderName`` (1–255 chars).

**Response:** ``Folder`` on 200; ``ErrorResponse`` on 400, 401, 403, 404.
...
CONTROLLER_GUARD_BODIES: list[tuple[str, dict[str, Any]]] = [
    ...
    ("format_specifier", {"folderName": "My %s Subfolder"}),
    ("javascript_protocol", {"folderName": "javascript:alert(1)"}),
]


@pytest.mark.integration
class TestCreateSubfolderOpenApiRequestSchema:
    """Static alignment: request JSON ↔ OpenAPI createSubfolder request."""

    @pytest.mark.parametrize("label,body", VALID_BODIES)
    def test_valid_bodies_match_openapi(self, label: str, body: dict[str, Any]) -> None:
        assert_request_body_matches_openapi_operation(body, _CREATE_SUBFOLDER_OP)
...
class TestCreateSubfolder:
    """POST /api/v1/knowledgeBase/{kbId}/folder/{folderId}/subfolder — createSubfolder."""
    ...
    def test_create_subfolder_response_matches_openapi(self) -> None:
        """Create KB → root folder → subfolder, validate 200 body matches OpenAPI Folder schema."""
        ...
        try:
            folder_resp = self.create_folder(self.base_url, self.access_token, kb_id, f"{_UNIQUE_MARKER}-parent", self.timeout)
            assert folder_resp.status_code == 200, f"Root folder setup failed: {folder_resp.text}"
            folder_id = folder_resp.json()["id"]

            resp = self.create_subfolder(self.base_url, self.access_token, kb_id, folder_id, f"{_UNIQUE_MARKER}-child", self.timeout)
            assert resp.status_code == 200, f"Expected 200, got {resp.status_code}: {resp.text}"
            body = resp.json()
            assert_response_matches_openapi_operation(body, _CREATE_SUBFOLDER_OP)
            assert body.get("name") == f"{_UNIQUE_MARKER}-child"
            assert isinstance(body.get("id"), str)
        finally:
            self.delete_kb(self.base_url, self.access_token, kb_id, self.timeout)

    def test_create_nested_subfolder(self) -> None:
        """Create three levels deep: KB → folder → subfolder → nested subfolder."""
        create_resp = self.create_kb(self.base_url, self.access_token, f"{_UNIQUE_MARKER}-nested-kb", self.timeout)
        assert create_resp.status_code == 200, f"KB setup failed: {create_resp.text}"
        kb_id = create_resp.json()["id"]

        try:
            folder_resp = self.create_folder(self.base_url, self.access_token, kb_id, f"{_UNIQUE_MARKER}-L1", self.timeout)
            assert folder_resp.status_code == 200, folder_resp.text
            l1_id = folder_resp.json()["id"]

            sub_resp = self.create_subfolder(self.base_url, self.access_token, kb_id, l1_id, f"{_UNIQUE_MARKER}-L2", self.timeout)
            assert sub_resp.status_code == 200, f"L2 subfolder failed: {sub_resp.text}"
            l2_id = sub_resp.json()["id"]

            resp = self.create_subfolder(self.base_url, self.access_token, kb_id, l2_id, f"{_UNIQUE_MARKER}-L3", self.timeout)
            assert resp.status_code == 200, f"Expected 200 (L3 nesting), got {resp.status_code}: {resp.text}"
            assert_response_matches_openapi_operation(resp.json(), _CREATE_SUBFOLDER_OP)
        finally:
            self.delete_kb(self.base_url, self.access_token, kb_id, self.timeout)

    def test_subfolder_name_boundary(self) -> None:
        ...
        try:
            ...
            resp = self.create_subfolder(self.base_url, self.access_token, kb_id, folder_id, folder_name, self.timeout)
    ...
    def test_controller_guards_reject_dangerous_foldername(self, label: str, body: dict[str, Any]) -> None:
        ...
        try:
            ...
            resp = requests.post(
                f"{self.base_url}/api/v1/knowledgeBase/{kb_id}/folder/{folder_id}/subfolder",
            ...
            assert_response_matches_openapi_operation(err_body, _CREATE_SUBFOLDER_OP, status_code="400")
        finally:
            self.delete_kb(self.base_url, self.access_token, kb_id, self.timeout)
    ...
    def test_no_auth_returns_401(self) -> None:
        ...
        resp = requests.post(
            f"{self.base_url}/api/v1/knowledgeBase/{_FAKE_UUID}/folder/{_FAKE_FOLDER_ID}/subfolder",
        ...
        err_body = resp.json()
        assert_response_matches_openapi_ref(err_body, _ERROR_REF)
        assert_response_matches_openapi_operation(err_body, _CREATE_SUBFOLDER_OP, status_code="401")

    def test_nonexistent_kb_returns_403(self) -> None:
        """Valid auth, non-existent UUID kbId → 403."""
        resp = self.create_subfolder(
            self.base_url, self.access_token, _FAKE_UUID, _FAKE_FOLDER_ID, "Test Subfolder", self.timeout,
        )
        assert resp.status_code == 403, f"Expected 403, got {resp.status_code}: {resp.text}"
        err_body = resp.json()
        assert_response_matches_openapi_ref(err_body, _ERROR_REF)
        assert_response_matches_openapi_operation(err_body, _CREATE_SUBFOLDER_OP, status_code="403")

    def test_nonexistent_parent_folder_returns_404(self) -> None:
        """Valid KB, non-existent folderId → 404."""
        create_resp = self.create_kb(self.base_url, self.access_token, f"{_UNIQUE_MARKER}-404-kb", self.timeout)
        assert create_resp.status_code == 200, f"KB setup failed: {create_resp.text}"
        kb_id = create_resp.json()["id"]

        try:
            resp = self.create_subfolder(
                self.base_url, self.access_token, kb_id, _FAKE_FOLDER_ID, "Test Subfolder", self.timeout,
            )
            assert resp.status_code == 404, f"Expected 404, got {resp.status_code}: {resp.text}"
            err_body = resp.json()
            assert_response_matches_openapi_ref(err_body, _ERROR_REF)
            assert_response_matches_openapi_operation(err_body, _CREATE_SUBFOLDER_OP, status_code="404")
        finally:
            self.delete_kb(self.base_url, self.access_token, kb_id, self.timeout)
