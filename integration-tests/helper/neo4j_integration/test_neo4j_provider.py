...
class TestNeo4jProvider(Neo4jProvider):
    ...
    async def count_records(self, connector_id: str, *, scoped: bool = False) -> int:
        ...
        Placeholder stubs that still carry ``BELONGS_TO`` are included — use
        :meth:`get_placeholder_records` to isolate those. Default (``False``) counts every
        record for the connector — required by suites whose records have no RecordGroup
        (e.g. KB uploads).
        """
        if not self.client:
