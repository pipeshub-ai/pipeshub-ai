...
# Summary prefix carried by every ticket the mutation tests create on the primary project.
# The CI matrix runs the arango and neo4j legs against the *same* Jira site, so a run sees
# the other leg's in-flight tickets; both the live-Jira baselines and the graph counts skip
# anything carrying this marker so each run only measures data it owns.
