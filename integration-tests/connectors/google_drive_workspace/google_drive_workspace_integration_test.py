...
"""Google Drive Workspace – folder_ids sync filter integration tests.
...
class TestDriveWorkspaceFolderFilter:
    ...
    async def test_tc_ff_001_folder_ids_filter_syncs_seed_subtree(
        self,
        drive_workspace_connector: dict[str, Any],
        connector_assertions: ConnectorAssertions,
    ) -> None:
        connector_id = drive_workspace_connector["connector_id"]
        seed_id = drive_workspace_connector["seed_folder_id"]
        nested_id = drive_workspace_connector["nested_folder_id"]
        child_id = drive_workspace_connector["child_file_id"]
        oos_folder_id = drive_workspace_connector["oos_folder_id"]
        oos_file_id = drive_workspace_connector["oos_file_id"]
        ...
        await connector_assertions.assert_record_exists(
            ...
            RecordAssertion(
                ...
                record_name=drive_workspace_connector["child_file_name"],
    ...
    async def test_tc_ff_005_file_reenters_scope_resyncs(
        ...
        connector_id = drive_workspace_connector["connector_id"]
        seed_id = drive_workspace_connector["seed_folder_id"]
        leave_file_id = drive_workspace_connector["leave_file_id"]
        oos_folder_id = drive_workspace_connector["oos_folder_id"]
    ...
    async def test_tc_ff_007_folder_enters_scope_pulls_descendants(
        ...
        seed_id = drive_workspace_connector["seed_folder_id"]
        oos_folder_id = drive_workspace_connector["oos_folder_id"]
        movable_id = drive_workspace_connector["movable_folder_id"]
        inside_id = drive_workspace_connector["movable_inside_file_id"]
    ...
    async def test_tc_ff_008_oos_create_stays_out(
        ...
        in_scope_id = (
            drive_workspace_connector.get("new_file_id")
            or drive_workspace_connector.get("leave_file_id")
        )
    ...
    async def test_tc_ff_010_placeholder_root_ancestor_present(
        ...
        connector_id = drive_workspace_connector["connector_id"]
        root_id = drive_workspace_connector["root_folder_id"]
        seed_id = drive_workspace_connector["seed_folder_id"]
