...
| `electron-prepare.mjs` | Compiles Electron TS, copies `out/` → `electron/out/`, generates PNG for the packager |
...
| Script | Produces |
|--------|-----------|
| `electron:build:mac` | macOS `.dmg` (+ `.blockmap`) under `dist-electron/` |
| `electron:build:win` | Windows `.exe` installer under `dist-electron/` |
