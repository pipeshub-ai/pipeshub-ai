...
import { ConnectorStatsData, ConnectorStatsResponse } from '../types/types';
  ...
  const fetchConnectorStats = useCallback(
    async (isManualRefresh = false): Promise<void> => {
      ...
      try {
        ...
        const apiUrl = `/api/v1/knowledgeBase/stats/${connectorId}`;
        ...
        if (data) {
          ...
          setConnectorStats({
            ...
            connectorId: data.connector._key,
          } as ConnectorStatsData);
        } else {
    ...
    if (connectorStats === undefined && !initialLoading) {
      return (
        ...
        >
          ...
          <Typography variant="body2">{`No data found for connector "${connectorName}".`}</Typography>
        </Alert>
      );
    }
      ...
      <Grid container spacing={1.5}>
        {connectorStats && (
          <Grid
            ...
            md={6}
            lg={4}
            key={`${connectorStats.connector}-${connectorStats.connectorId}`}
          >
            <ConnectorStatsCard connectorStatsData={connectorStats} />
          </Grid>
        )}
      </Grid>
    );
