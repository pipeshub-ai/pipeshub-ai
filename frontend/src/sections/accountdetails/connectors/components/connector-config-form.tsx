...
const ConnectorConfigForm = ({ connector, onClose, onSuccess }: ConnectorConfigFormProps) => {
  ...
  useEffect(() => {
    const fetchConnectorConfig = async () => {
      try {
        setLoading(true);
        const response = await ConnectorApiService.getConnectorConfig(connector.name.toUpperCase());
        setConnectorConfig(response);
        
        // Initialize form data with existing values
        setFormData({
          auth: response.config.auth.values || {},
          sync: {
            selectedStrategy: response.config.sync.selectedStrategy || response.config.sync.supportedStrategies[0] || '',
            ...(response.config.sync as any).values || {},
          },
          filters: response.config.filters.values || {},
        });
      } catch (error) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Auth config will be stored in etcd, sync/filters in database
      const configToSave = {
        auth: formData.auth, // Will be stored in etcd
        sync: {
          ...connectorConfig.config.sync,
