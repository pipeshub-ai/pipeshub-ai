...
class AgentApiService {
  ...
  static async getKnowledgeBases(params?: {
    ...
    if (response.data.knowledgeBases && response.data.pagination) {
      ...
      return {
        ...
        filters: response.data.filters || { applied: {}, available: { permissions: [], sortFields: [], sortOrders: [] } }
      };
    }
  ...
  private static transformAgentFormData(data: Partial<AgentFormData>): Partial<AgentFormData> {
    ...
    if (transformed.kb && Array.isArray(transformed.kb)) {
      ...
      transformed.kb = transformed.kb.filter(id => typeof id === 'string' && id.trim().length > 0);
    }

    // Clean up empty arrays
    Object.keys(transformed).forEach(key => {
      if (Array.isArray(transformed[key as keyof AgentFormData]) && transformed[key as keyof AgentFormData]?.length === 0) {
        transformed[key as keyof AgentFormData] = [];
      }
    });
