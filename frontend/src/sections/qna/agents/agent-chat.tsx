...
const AgentChat = () => {
  ...
  useEffect(() => {
    if (agentKey) {
      AgentApiService.getAgent(agentKey).then((agent) => {
        setAgent(agent);
        setAvailableModels(agent.models);
        setAvailableTools(agent.tools.map((tool: string) => ({ name: tool })));
      });
    }
  }, [agentKey]);
  console.log(agent);
  ...
  useEffect(() => {
    if (!currentConversationId && agent?.startMessage) {
      ...
      if (existingMessages.length === 0) {
        const startMessage: FormattedMessage = {
          type: 'bot',
        ...
        streamingManager.setConversationMessages(conversationKey, [startMessage]);
      }
    }
  }, [currentConversationId, agent?.startMessage, getConversationKey, streamingManager]);
