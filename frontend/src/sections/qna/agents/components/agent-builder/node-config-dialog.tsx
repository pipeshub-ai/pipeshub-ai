...
const NodeConfigDialog: React.FC<NodeConfigDialogProps> = memo(({
  ...
  const renderConfigField = useCallback((key: string, value: any) => {
    ...
    if (node?.data.type.startsWith('tool-') && !node?.data.type.startsWith('tool-group-')) {
      if (key === 'approvalConfig') {
        const selectedUsers = users ? users.filter(user => value?.approvers?.users?.includes(user._id)) : [];
        const selectedGroups = groups ? groups.filter(group => value?.approvers?.groups?.includes(group._id)) : [];
          ...
          <Grid item xs={12} key={key}>
            <Box sx={{ 
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                <Icon icon={shieldIcon} width={20} height={20} style={{ color: theme.palette.warning.main }} />
                <Typography
                  variant="body2"
                  color="text.primary"
                  sx={{ fontWeight: 600 }}
                ...
                <Box>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      {[
                        ...
                        { value: 'unanimous', label: 'Unanimous', description: 'All approvers' }
                      ].map((option) => (
                        <Box
                          key={option.value}
                          ...
                          sx={{
                            ...
                            borderRadius: 1,
                            cursor: 'pointer',
                            bgcolor: value?.approvalThreshold === option.value
                  ...
                  <Box sx={{ 
                    ...
                    <Typography variant="caption" color="text.secondary">
                      ...
                      <strong>{(selectedUsers.length || 0) + (selectedGroups.length || 0)} approver(s)</strong>{' '}
        ...
        return (
          <Grid item xs={12} key={key}>
            <Typography
              ...
              sx={{ mb: 1, fontWeight: 500 }}
            >
              Selected Tools ({Array.isArray(value) ? value.length : 0} of {node.data.config?.tools?.length || 0})
            </Typography>
            <Box sx={{ 
              maxHeight: 200, 
              overflow: 'auto',
              ...
              {node.data.config?.tools?.map((tool: any, index: number) => (
                <FormControlLabel
                  ...
                  sx={{ width: '100%', mb: 1, alignItems: 'flex-start' }}
                />
              ))}
            </Box>
          </Grid>
        ...
        return (
          <Grid item xs={12} key={key}>
            <Typography
              ...
              sx={{ mb: 1, fontWeight: 500 }}
            >
              Selected Knowledge Bases ({Array.isArray(value) ? value.length : 0} of {node.data.config?.knowledgeBases?.length || 0})
            </Typography>
            <Box sx={{ 
              maxHeight: 200, 
              overflow: 'auto',
    ...
    if (node?.data.type.startsWith('app-')) {
      if (key === 'searchScope' || key === 'fileTypes' || key === 'services') {
        const options = key === 'searchScope' ? ['all', 'channels', 'dms', 'inbox', 'sent', 'drafts'] :
                      key === 'fileTypes' ? ['all', 'documents', 'images', 'videos', 'presentations'] :
          ...
          <Grid item xs={12} key={key}>
            ...
            <FormControl fullWidth>
              <Select
                ...
                onChange={(e) => setConfig((prev) => ({ ...prev, [key]: Array.isArray(value) ? [e.target.value] : e.target.value }))}
                sx={{
                  borderRadius: 1,
                  '&:hover .MuiOutlinedInput-notchedOutline': {
                    borderColor: theme.palette.primary.main,
    ...
    if (['modelKey', 'modelName', 'provider', 'modelType', 'toolId', 'fullName', 'appName', 'appDisplayName', 'tools', 'knowledgeBases'].includes(key)) {
      return null;
    }
  ...
  }, [config, node?.data.type, theme.palette.primary.main, users, groups, getAvatarColor, userChipStyle, groupChipStyle, isDark]);
