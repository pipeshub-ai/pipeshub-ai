...
import { Model, ChatMode, Tool } from 'src/sections/qna/agents/agent-chat';
...
const AgentChatInput: React.FC<ChatInputProps> = ({
  ...
  const allAppOptions: AppOption[] = useMemo(() => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      await onSubmit(
        ...
        selectedChatMode?.id,
        selectedTools, // app_name.tool_name format - PERSISTENT
        selectedKBs, // KB IDs - PERSISTENT
        selectedApps // App names - PERSISTENT
      );
    } catch (error) {
          ...
          >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Tooltip title={`Model: ${selectedModel?.modelName || 'Select'}`}>
                ...
                >
                  {selectedModel?.modelName?.slice(0, 8) || 'Model'}
                </Button>
              </Tooltip>
            </Box>
          </Box>
