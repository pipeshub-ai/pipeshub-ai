...
export function McpCatalogLayout({
  ...
  const filteredCustomInstances = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    if (!q) return customInstances;
    return customInstances.filter(
      (i) => i.name.toLowerCase().includes(q) || (i.description ?? '').toLowerCase().includes(q)
    );
  }, [customInstances, searchQuery]);

  const hasAnyContent = templateRows.length > 0 || instances.length > 0;
  const hasResults = filteredTemplateRows.length > 0 || filteredCustomInstances.length > 0;
    ...
    >
      ...
      {isLoading ? (
        ...
        <Grid columns={{ initial: '2', md: '3', lg: '4' }} gap="4" style={{ width: '100%' }}>
          {filteredTemplateRows.map(({ template, instances: templateInstances }) => (
            <McpServerCard
              key={template.typeId}
              title={template.displayName}
              description={template.description}
              iconUrl={template.icon}
              variant={templateInstances.length > 0 ? 'active' : 'registry'}
