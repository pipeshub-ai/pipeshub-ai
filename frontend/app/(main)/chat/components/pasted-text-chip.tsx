...
import { formatPasteMeta } from '../utils/paste-attachment';
...
export function PastedTextChip({ file, onPreview, onShowInTextField, onRemove, onRetry }: PastedTextChipProps) {
  ...
  const metaLabel =
    file.pasteCharCount !== undefined && file.pasteLineCount !== undefined
      ? formatPasteMeta(file.pasteCharCount, file.pasteLineCount)
      : undefined;
    ...
    >
      <Flex direction="column" gap="2">
        ...
        <Flex
          direction="column"
          gap="1"
          onClick={file.status === 'uploaded' || file.status === 'uploading' ? onPreview : undefined}
          style={{ minWidth: 0, cursor: file.status === 'error' ? 'default' : 'pointer' }}
        >
          <Text
            size="1"
            weight="medium"
