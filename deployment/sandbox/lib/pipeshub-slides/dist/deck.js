...
import { ACCENT_BAND_HEIGHT, BODY_Y, LAYOUTS, MARGINS, TITLE_HEIGHT, TITLE_Y, } from "./layout.js";
...
export class Deck {
    ...
    twoColumn(options) {
        return this.contentSlide(options.title, (slide) => {
            ...
            this.renderColumn(slide, options.left, MARGINS.left, BODY_Y, leftW, bodyH);
            this.renderColumn(slide, options.right, MARGINS.left + leftW + gutter, BODY_Y, rightW, bodyH);
        });
    }
    statGrid(options) {
        return this.contentSlide(options.title, (slide) => {
    ...
    renderColumn(slide, content, x, y, w, h) {
        const palette = this.theme.palette;
        const type = this.theme.type;
        if ("bullets" in content) {
            slide.addText(content.bullets.map((t, i) => ({
                text: t,
                options: {
                    bullet: true,
        ...
        if ("imagePath" in content || "imageData" in content) {
            ...
            if (content.imagePath)
                opts.path = content.imagePath;
            if (content.imageData)
                opts.data = content.imageData;
            slide.addImage(opts);
            if (content.caption) {
        ...
        if ("chart" in content) {
            ...
            slide.addChart(map[chart.type], data, {
                x,
                y,
                w,
                h,
