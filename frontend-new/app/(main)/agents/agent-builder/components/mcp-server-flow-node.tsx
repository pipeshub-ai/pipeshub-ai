...
import { AGENT_MCP_FALLBACK_ICON, normalizeDisplayName } from '../display-utils';
...
function toolKey(t: McpServerFlowTool): string {
  return t.namespacedName || t.name;
}

function toolTitle(t: McpServerFlowTool): string {
  return t.name.replace(/_/g, ' ');
}

function mergeToolsUnique(a: McpServerFlowTool[], b: McpServerFlowTool[]): McpServerFlowTool[] {
  const map = new Map<string, McpServerFlowTool>();
  [...a, ...b].forEach((t) => {
    const k = toolKey(t);
    if (k) map.set(k, t);
  });
...
export function McpServerFlowNode({
  id,
  data,
  selected,
  readOnly,
  ...
  const displayName =
    (cfg.displayName as string) || (cfg.mcpServerName as string) || data.label;

  const tools = (cfg.tools as McpServerFlowTool[]) || [];
  const availableTools = useMemo(() => {
    const c = (data.config || {}) as Record<string, unknown>;
    const tList = (c.tools as McpServerFlowTool[]) || [];
    const avail = (c.availableTools as McpServerFlowTool[]) || [];
  ...
  const handleAddTool = useCallback(
    (tool: McpServerFlowTool) => {
      setNodes((nodes) =>
        nodes.map((node) => {
          if (node.id !== id) return node;
          const c = (node.data.config || {}) as Record<string, unknown>;
          const cur = (c.tools as McpServerFlowTool[]) || [];
          const sel = (c.selectedTools as string[]) || [];
          if (cur.some((existing) => toolKey(existing) === toolKey(tool))) return node;
          return {
            ...node,
            data: {
              ...node.data,
              config: {
                ...c,
                tools: [...cur, tool],
                selectedTools: sel.includes(tool.name) ? sel : [...sel, tool.name],
                availableTools: mergeToolsUnique((c.availableTools as McpServerFlowTool[]) || [], [tool]),
              },
            },
          };
        })
        ...
        nodes.map((node) => {
          ...
          return {
            ...
            data: {
              ...
              config: {
                ...c,
                tools: cur.filter((existing) => existing.name !== toolName),
  ...
  const icon =
    (cfg.iconPath as string) || (data.icon as string) || 'dns';
              ...
              <Flex align="center" justify="center" style={{ flexShrink: 0, lineHeight: 0 }} aria-hidden>
                {isIconUrl(icon) ? (
                  <ThemeableAssetIcon
                    ...
                    fallbackSrc={AGENT_MCP_FALLBACK_ICON}
                  />
                ) : (
        ...
        <Box px="3" py="3" style={{ background: FLOW_NODE_PANEL_BG }}>
          <Flex align="center" justify="between" gap="2" mb="2">
            <Flex align="center" gap="2">
              <MaterialIcon name="settings_ethernet" size={16} color="var(--agent-flow-text-muted)" />
                    ...
                    <Flex direction="column" gap="1">
                      {toolsToAdd.map((tool) => (
                        <button
                          key={toolKey(tool)}
                          type="button"
                          title={tool.description || tool.namespacedName || toolTitle(tool)}
                          onClick={(e) => {
                            e.stopPropagation();
                            handleAddTool(tool);
                          }}
          ...
          {tools.length === 0 ? (
            ...
            >
              <MaterialIcon name="settings_ethernet" size={28} color="var(--agent-flow-text-muted)" />
                  ...
                  >
                    ...
                    {!readOnly ? (
                      <IconButton
                        ...
                        variant="soft"
                        color="red"
                        className="mcp-flow-remove-tool"
