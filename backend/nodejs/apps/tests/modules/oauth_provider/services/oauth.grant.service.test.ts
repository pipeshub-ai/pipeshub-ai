...
import {
  OAuthRefreshToken,
  IOAuthRefreshToken,
} from '../../../../src/modules/oauth_provider/schema/oauth.refresh_token.schema';
import {
  OAuthAccessToken,
  IOAuthAccessToken,
} from '../../../../src/modules/oauth_provider/schema/oauth.access_token.schema';
import {
  OAuthApp,
  IOAuthApp,
...
import {
  Users,
  User,
} from '../../../../src/modules/user_management/schema/users.schema';
...
describe('OAuthGrantService', () => {
  ...
  describe('listUserGrants', () => {
    it('returns active refresh tokens with app metadata and lastUsedAt', async () => {
      ...
      const aggregateStub = sinon
        .stub(OAuthAccessToken, 'aggregate')
        .resolves([{ _id: rtId, lastUsedAt: lastUsed }]);
      ...
      expect(matchStage.parentRefreshTokenId.$in.map(String)).to.include(
        rtId.toString(),
      );
    ...
    it('aggregates lastUsedAt matching active access tokens grouped by parentRefreshTokenId', async () => {
      ...
      expect(result).to.have.length(2);
      expect(
        result.find((r) => r.id === rtId1.toString())?.lastUsedAt,
      ).to.deep.equal(new Date('2026-09-10T11:00:00Z'));
      expect(
        result.find((r) => r.id === rtId2.toString())?.lastUsedAt,
      ).to.deep.equal(new Date('2026-09-11T11:00:00Z'));
      ...
      expect(
        matchStage.parentRefreshTokenId.$in.map(String),
      ).to.include.members([rtId1.toString(), rtId2.toString()]);
  ...
  describe('revokeUserGrant', () => {
    ...
    it('revokes all access tokens in the refresh-token family for a rotated grant', async () => {
      const grantId = new Types.ObjectId().toString();
      ...
      expect(updateManyStub.calledOnce).to.be.true;
      const filterArg = updateManyStub.firstCall.args[0];
      expect((filterArg as any).parentRefreshTokenId).to.deep.equal({ $eq: familyId });
    });
    ...
    it('revokes standalone access token when refresh token not found', async () => {
      ...
      sinon.stub(OAuthAccessToken, 'findOne').resolves(mockAt);
    ...
    it('revokes refresh token and associated access tokens within a transaction when REPLICA_SET_AVAILABLE is true', async () => {
      ...
      try {
        ...
        expect(mockRt.isRevoked).to.be.true;
        expect(
          (mockRt.save as sinon.SinonStub).firstCall.args[0],
        ).to.deep.equal({ session: mockSession });
        expect(updateManyStub.calledOnce).to.be.true;
        expect(updateManyStub.firstCall.args[2]).to.deep.equal({
          session: mockSession,
        });
      } finally {
