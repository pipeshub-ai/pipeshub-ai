...
export const autoAuthenticateInstance =
  (appConfig: AppConfig) =>
  async (req: AuthenticatedUserRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      requireUser(req);
      const { instanceId } = req.params;
      const r = await executeConnectorCommand(
        buildUrl(appConfig, `/instances/${instanceId}/auto-authenticate`),
        HttpMethod.POST,
        await forwardHeaders(req),
      );
      handleConnectorResponse(r, res, 'Auto-authenticate MCP instance', 'Auto-authentication failed');
    } catch (error: any) {
      next(handleBackendError(error, 'auto-authenticate MCP instance'));
    }
  };
