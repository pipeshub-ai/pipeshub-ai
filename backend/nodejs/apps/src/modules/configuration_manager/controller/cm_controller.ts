...
export const getOutlookCredentials =
  (keyValueStoreService: KeyValueStoreService) =>
  async (
    req: AuthenticatedUserRequest | AuthenticatedServiceRequest,
    res: Response,
    ...
    try {
      ...
      const encryptedOutlookCredentials =
        await keyValueStoreService.get<string>(
          `${configPaths.connectors.outlook.config}/${orgId}`,
        );
      if (encryptedOutlookCredentials) {
        const outlookCredentials = JSON.parse(
          EncryptionService.getInstance(
            configManagerConfig.algorithm,
            configManagerConfig.secretKey,
          ).decrypt(encryptedOutlookCredentials),
        );
        res.status(200).json(outlookCredentials).end();
      } else {
        res.status(200).json({}).end();
      }
    } catch (error: any) {
      logger.error('Error getting Outlook credentials', { error });
      next(error);
    }
  };

export const setOutlookCredentials =
  (keyValueStoreService: KeyValueStoreService) =>
  async (
    req: AuthenticatedUserRequest | AuthenticatedServiceRequest,
    res: Response,
    ...
    try {
      ...
      // Todo: Do a health check for the credentials
      const encryptedOutlookCredentials = EncryptionService.getInstance(
        configManagerConfig.algorithm,
        configManagerConfig.secretKey,
      ).encrypt(JSON.stringify(credentials));
      await keyValueStoreService.set<string>(
        `${configPaths.connectors.outlook.config}/${orgId}`,
        encryptedOutlookCredentials,
      );
      res
        .status(200)
        .json({ message: 'Outlook credentials created successfully' });
    } catch (error: any) {
      logger.error('Error creating Outlook credentials', { error });
      next(error);
    }
  };
