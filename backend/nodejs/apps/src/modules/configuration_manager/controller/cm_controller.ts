...
export const createBrandingConfig =
  (keyValueStoreService: KeyValueStoreService) =>
    async (req: AuthenticatedUserRequest, res: Response, next: NextFunction) => {
      try {
        const brandingConfig = req.body;
        const configManagerConfig = loadConfigurationManagerConfig();
        const encryptedBrandingConfig = EncryptionService.getInstance(
          configManagerConfig.algorithm,
          configManagerConfig.secretKey,
        ).encrypt(JSON.stringify(brandingConfig));

        await keyValueStoreService.set<string>(
          configPaths.platform.branding,
          encryptedBrandingConfig,
        );

        res.status(HTTP_STATUS.OK).json({ message: 'Branding configuration saved successfully' });
      } catch (error: any) {
        logger.error('Error creating branding config', { error });
        next(error);
      }
    };
...
export const getBrandingConfig =
  (keyValueStoreService: KeyValueStoreService) =>
    async (_req: AuthenticatedUserRequest | AuthenticatedServiceRequest, res: Response, next: NextFunction) => {
      try {
        const configManagerConfig = loadConfigurationManagerConfig();
        const encryptedBrandingConfig = await keyValueStoreService.get<string>(
          configPaths.platform.branding,
        );

        if (encryptedBrandingConfig) {
          const brandingConfig = JSON.parse(
            EncryptionService.getInstance(
              configManagerConfig.algorithm,
              configManagerConfig.secretKey,
            ).decrypt(encryptedBrandingConfig),
          );
          res.status(HTTP_STATUS.OK).json(brandingConfig);
        } else {
