...
class DataSourceEntitiesProcessor:
    ...
    async def initialize(self) -> None:
        ...
        bootstrap_servers = producer_config.get("brokers") or producer_config.get(
            "bootstrap_servers"
        )
    ...
    async def _handle_parent_record(
        self, record: Record, tx_store: TransactionStore
    ) -> None:
        if record.parent_external_record_id:
            ...
            if (
                ...
                and record.parent_record_type is RecordType.FILE
                and record.record_type is RecordType.FILE
            ):
            ...
            if parent_record and isinstance(parent_record, Record):
                if (
                    ...
                    and record.parent_external_record_id
                    and (
                        record.parent_record_type == RecordType.MAIL
                        or record.parent_record_type == RecordType.WEBPAGE
                    )
                ):
                ...
                await tx_store.create_record_relation(
                    parent_record.id, record.id, relation_type
                )

    async def _handle_record_group(
        self, record: Record, tx_store: TransactionStore
    ) -> None:
    ...
    async def _handle_updated_record(
        self, record: Record, existing_record: Record, tx_store: TransactionStore
    ) -> None:
        # Set org_id for the record
        record.org_id = self.org_id
        self.logger.info(
            "Updating existing record: %s, version %d -> %d",
            record.record_name,
    ...
    async def _handle_record_permissions(
                ...
                if permission.entity_type == EntityType.USER.value:
                    ...
                    if permission.email:
                        ...
                        if not user and permission.email:
                            user = await self._create_external_user(
                                permission.email, record.connector_name, tx_store
                            )
                    ...
                    if user_group:
                        from_collection = (
                            f"{CollectionNames.GROUPS.value}/{user_group.id}"
                        )
                    else:
                        self.logger.warning(
                            f"User group with external ID {permission.external_id} not found in database"
                        )
                        continue
    ...
    async def _create_external_user(
        self, email: str, connector_name: str, tx_store
    ) -> AppUser:
    ...
    async def on_updated_record_permissions(
        self, record: Record, permissions: list[Permission]
    ) -> None:
        self.logger.info(
            f"Starting permission update for record: {record.record_name} ({record.id})"
        )
        ...
        try:
            async with self.data_store_provider.transaction() as tx_store:
                ...
                self.logger.info(
                    f"Deleted {deleted_count} old permission edge(s) for record: {record.id}"
                )

                # Step 2: Add the new permissions by reusing the existing helper method.
                if permissions:
                    self.logger.info(
                        f"Adding {len(permissions)} new permission edge(s) for record: {record.id}"
                    )
                ...
                if not record.inherit_permissions:
                    ...
                    if record_group:
                        to_key = (
                            f"{CollectionNames.RECORD_GROUPS.value}/{record_group.id}"
                        )
                    ...
                    self.logger.info(
                        f"No new permissions to add for record: {record.id}"
                    )

                self.logger.info(
                    f"Successfully updated permissions for record: {record.id}"
                )

        except Exception as e:
            self.logger.error(
                f"Failed to update permissions for record {record.id}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def _process_record(
        ...
        existing_record = await tx_store.get_record_by_external_id(
            connector_name=record.connector_name, external_id=record.external_record_id
        )
        ...
        async with self.data_store_provider.transaction() as tx_store:
            ...
            await self.messaging_producer.send_message(
                "record-events",
                {
                    ...
                    "timestamp": get_epoch_timestamp_in_ms(),
                    "payload": processed_record.to_kafka_record(),
                },
                key=record.id,
            )
    ...
    async def on_new_record_groups(
        self, record_groups: list[tuple[RecordGroup, list[Permission]]]
    ) -> None:
        try:
            async with self.data_store_provider.transaction() as tx_store:
                for record_group, permissions in record_groups:
                    ...
                    existing_record_group = (
                        await tx_store.get_record_group_by_external_id(
                    ...
                    if existing_record_group is None:
                        ...
                        self.logger.info(
                            f"Creating new record group with id: {record_group.id}"
                        )
                    else:
                        record_group.id = existing_record_group.id
                        self.logger.info(
                            f"Updating existing record group with id: {record_group.id}"
                        )
                    ...
                    self.logger.info(
                        f"Creating BELONGS_TO edge for RecordGroup {record_group.id} to Org {self.org_id}"
                    )
                    await tx_store.batch_create_edges(
                        [org_relation],
                    ...
                    if record_group.parent_external_group_id:
                        ...
                        if parent_record_group:
                            self.logger.info(
                                f"Creating BELONGS_TO edge for RecordGroup '{record_group.name}' to parent '{parent_record_group.name}'"
                            )
                    ...
                    for permission in permissions:
                        ...
                        if permission.entity_type == EntityType.USER:
                            user = None
                            if permission.email:
                                user = await tx_store.get_user_by_email(
                                    permission.email
                                )

                            if user:
                                from_collection = (
                                    f"{CollectionNames.USERS.value}/{user.id}"
                                )
                            else:
                                self.logger.warning(
                                    f"Could not find user with email {permission.email} for RecordGroup permission."
                                )
                            ...
                            if permission.external_id:
                                user_group = (
                                    await tx_store.get_user_group_by_external_id(
                                        connector_name=record_group.connector_name,
                                        external_id=permission.external_id,
                                    )
                                )
                            ...
                            if user_group:
                                ...
                                self.logger.warning(
                                    f"Could not find group with external_id {permission.external_id} for RecordGroup permission."
                                )
                        ...
                        if from_collection:
                            record_group_permissions.append(
                                permission.to_arango_permission(
                    ...
                    if record_group_permissions:
                        self.logger.info(
                            f"Creating/updating {len(record_group_permissions)} PERMISSION edges for RecordGroup {record_group.id}"
                        )
                        await tx_store.batch_create_edges(
                            record_group_permissions,
                            collection=CollectionNames.PERMISSION.value,
                    ...
                    if record_group.parent_record_group_id:
                        await tx_store.create_record_groups_relation(
                            record_group.id, record_group.parent_record_group_id
                        )
    ...
    async def update_record_group_name(
        self,
        folder_id: str,
        new_name: str,
        old_name: str = None,
        ...
        try:
            ...
            self.logger.error(
                f"Failed to update record group name for {folder_id}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def on_new_user_groups(
        self, user_groups: list[tuple[AppUserGroup, list[AppUser]]]
    ) -> None:
            ...
            async with self.data_store_provider.transaction() as tx_store:
                for user_group, members in user_groups:
                    ...
                    if existing_user_group is None:
                        ...
                        self.logger.info(
                            f"Creating new user group with id: {user_group.id}"
                        )
                    else:
                        # Overwrite the new UUID with the existing one
                        user_group.id = existing_user_group.id
                        self.logger.info(
                            f"Updating existing user group with id: {user_group.id}"
                        )
                    ...
                    for member in members:
                        ...
                        if not user:
                            self.logger.warning(
                                f"Could not find user with email {member.email} for UserGroup permission."
                            )
                            continue
                    ...
                    if user_group_permissions:
                        self.logger.info(
                            f"Creating/updating {len(user_group_permissions)} PERMISSION edges for UserGroup {user_group.id}"
                        )
                        await tx_store.batch_create_edges(
                            user_group_permissions,
                            collection=CollectionNames.PERMISSION.value,
    ...
    async def get_all_app_users(self, app_name: Connectors) -> list[AppUser]:
        async with self.data_store_provider.transaction() as tx_store:
            ...
            return [
                AppUser.from_arango_user(app_user)
        ...
        try:
            async with self.data_store_provider.transaction() as tx_store:
                ...
                existing_edge = await tx_store.get_edge(
                    from_key, to_key, CollectionNames.PERMISSION.value
                )
                if existing_edge:
                    self.logger.info(
    ...
    async def on_user_group_deleted(
        ...
        try:
            async with self.data_store_provider.transaction() as tx_store:
                ...
                self.logger.info(
                    f"Deleting user group: {group_name} (internal_id: {group_internal_id})"
                )

                # Delete the node and edges
                await tx_store.delete_nodes_and_edges(
                    [group_internal_id], CollectionNames.GROUPS.value
                )
    ...
    async def _delete_group_organization_edges(
        self, tx_store, group_internal_id: str
    ) -> None:
        ...
        try:
            ...
            if edge_deleted:
                self.logger.info(
                    f"Deleted BELONGS_TO edge from group {group_internal_id} to org {self.org_id}"
                )
            else:
                self.logger.debug(
                    f"No BELONGS_TO edge found from group {group_internal_id} to org"
                )

        except Exception as e:
            self.logger.error(
                f"Error deleting organization edges for group {group_internal_id}: {e}"
            )
