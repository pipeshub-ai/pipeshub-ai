    ...
    async def on_record_metadata_update(self, record: Record) -> None:
        ...
        async with self.data_store_provider.transaction() as tx_store:
            ...
            if processed_record and existing_record is not None:
                ...
                # Re-save: _process_record already persisted processed_record with the
                # reindex-triggering status _preserve_indexing_state just overwrote.
                # old_path=None makes this a no-op for storage -- the real move (if
                # any) was already captured above via inner_moves.
                await self._handle_updated_record(processed_record, existing_record, tx_store, old_path=None)
