            ...
            async with self.data_store_provider.transaction() as tx_store:
                ...
                if records_to_publish:
                    for record in records_to_publish:
                        try:
                            kafka_payload = record.to_kafka_record()
                            await self.messaging_producer.send_message(
                                    "record-events",
                                    {"eventType": "newRecord", "timestamp": get_epoch_timestamp_in_ms(), "payload": kafka_payload},
                                    key=record.id
                                )
                        except Exception as e:
                            self.logger.error(f"Error processing record {record.__class__.__name__} (ID: {record.id}): {str(e)}")
                            raise
        except Exception as e:
