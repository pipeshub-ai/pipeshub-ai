...
class DataSourceEntitiesProcessor:
    ...
    async def reindex_existing_records(self, records: list[Record]) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to publish reindex events: {e!s}")
            raise e
    ...
    async def on_new_app_roles(self, roles: list[tuple[AppRole, list[AppUser]]]) -> None:
        ...
        try:
            ...
            self.logger.error(f"Transaction on_new_app_roles failed: {e!s}")
            raise e
    ...
    async def on_app_role_deleted(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to delete app role {external_role_id}: {e!s}",
                exc_info=True,
            )
            return False
