...
class DataSourceEntitiesProcessor:
    ...
    async def on_new_external_orgs(
        self,
        orgs_with_edges: list[tuple[Org, dict, dict]],
    ) -> None:
        """
        For each (org, prospect_edge, customer_edge): if an external org with the same
        name exists, delete the salesProspect and salesCustomer edges to that org; then
        upsert the org and upsert the prospect and customer edges.
        """
        try:
            if not orgs_with_edges:
                self.logger.warning("on_new_external_orgs received empty list; skipping.")
                return
            ...
            async with self.data_store_provider.transaction() as tx_store:
                ...
                for org, prospect_edge_attrs, customer_edge_attrs in orgs_with_edges:
                    ...
                    prospect_edge = {**prospect_edge_attrs}

                    prospect_edge["to_id"] = org.id
                    prospect_edge["to_collection"] = CollectionNames.ORGS.value
                    prospect_edge["from_id"] = self.org_id
                    prospect_edge["from_collection"] = CollectionNames.ORGS.value
                    await tx_store.batch_upsert_orgs([org.to_arango_org()])
                    await tx_store.batch_create_edges(
                        [prospect_edge],
                        collection=CollectionNames.SALES_PROSPECT.value,
                    )
                    if customer_edge_attrs:
                        customer_edge = {**customer_edge_attrs}
                        customer_edge["to_id"] = org.id
                        customer_edge["to_collection"] = CollectionNames.ORGS.value
                        customer_edge["from_id"] = self.org_id
                        customer_edge["from_collection"] = CollectionNames.ORGS.value
                        await tx_store.batch_create_edges(
                            [customer_edge],
                            collection=CollectionNames.SALES_CUSTOMER.value,
                        )
            ...
            self.logger.error(f"on_new_external_orgs failed: {str(e)}", exc_info=True)
            raise

    async def on_new_leads(
        self,
        lead_with_edges: list[tuple[Person, dict]],
    ) -> None:
        """
        Upsert Person nodes for leads and create salesLead edges (registered org -> person).
        Each edge's to_id must match the corresponding Person.id (e.g. Salesforce Lead Id).

        Args:
            lead_with_edges: List of tuples (person, sales_lead_edge); each sales_lead_edge must have "to_id"
                (person _key) plus edge attributes: company, title, status, rating, industry, leadSource,
                annualRevenue, externalId, startTime, endTime.
        """
        try:
            if not lead_with_edges:
                self.logger.warning(
                    "on_new_leads received empty lead_with_edges; skipping."
                )
                return

            async with self.data_store_provider.transaction() as tx_store:
                    for person, sales_lead_edge in lead_with_edges:
                        existing_person = await tx_store.get_nodes_by_field_in(
                            collection=CollectionNames.PEOPLE.value,
                            field="email",
                            values=[person.email]
                        )
                        if existing_person:
                            self.logger.info(f"Found existing person node for lead {person.email}")
                            person.id = existing_person[0].get("id") or existing_person[0].get("_key")
                            await tx_store.delete_edges_to(
                                to_id=person.id,
                        ...
                        self.logger.info(f"Upserting {person.email} person node for lead.")
                        await tx_store.batch_upsert_people([person])
                        ...
                        self.logger.info(
                            f"Created salesLead edge (registered org -> person) for {person.email}."
                        )

        except Exception as e:
            self.logger.error(
                f"handle_sales_lead_with_people failed: {str(e)}", exc_info=True
            )
            raise

    async def on_new_contacts(
        self,
        contact_with_edges: list[tuple[Person, dict, dict]],
    ) -> None:
        """
        Upsert Person nodes for contacts and create salesContact edges (registered org -> person).
        Each edge's to_id must match the corresponding Person.id (e.g. Salesforce Contact Id).
        """
        try:
            if not contact_with_edges:
                self.logger.warning(
                    "on_new_contacts received empty contact_with_edges; skipping."
                )
                return

            async with self.data_store_provider.transaction() as tx_store:
                for person, sales_contact_edge, member_of_edge in contact_with_edges:
                    self.logger.info(f"Upserting {person.email} person node for contact.")
                    #find person node by email using get_nodes_by_field_in
                    existing_person = await tx_store.get_nodes_by_field_in(
                        collection=CollectionNames.PEOPLE.value,
                        field="email",
                        values=[person.email]
                    ...
                    if existing_person:
                        self.logger.info(f"Found existing person node for {person.email}")
                        person.id = existing_person[0].get("id") or existing_person[0].get("_key")
                        await tx_store.delete_edges_to(
                            to_id=person.id,
                            to_collection=CollectionNames.PEOPLE.value,
                    ...
                    if sales_contact_edge:
                        ...
                        if member_of_edge:
                            ...
                            if account_name:
                                existing_org = await tx_store.get_nodes_by_field_in(
                                    collection=CollectionNames.ORGS.value,
                                    field="name",
                                    values=[account_name]
                                )
                                if existing_org:
                                    org_id = existing_org[0].get("id") or existing_org[0].get("_key")
                                    member_of_edge["to_id"] = org_id
                                    member_of_edge["from_id"] = person.id
                                    member_of_edge["from_collection"] = CollectionNames.PEOPLE.value
                                    member_of_edge["to_collection"] = CollectionNames.ORGS.value
                                    await tx_store.batch_create_edges(
                                        [member_of_edge],
                                        collection=CollectionNames.MEMBER_OF.value,
                                    )
            ...
            self.logger.error(
                f"handle_sales_contact failed: {str(e)}", exc_info=True
            )
            raise

    async def handle_deals_edges(self, deals_with_edges: list[tuple[Record, dict, dict]]) -> None:
        """
        Create dealOf edges (record group -> account org).
        Used by Salesforce connector to link deal record groups to their account orgs.
        """
        try:
            if not deals_with_edges:
                self.logger.debug("handle_deals_of received empty edges; skipping.")
                return

            async with self.data_store_provider.transaction() as tx_store:
                for record, dealof_edge, salesdeal_edge in deals_with_edges:
                    ...
                    if existing_record is not None:
                        self.logger.info(f"Found existing record for {record.external_record_id}")
                        record.id = existing_record.id
                        await tx_store.delete_edges_to(
                            to_id=record.id,
                            to_collection=CollectionNames.RECORDS.value,
                        ...
                        if dealof_edge is not None:
                            existing_org = await tx_store.get_nodes_by_field_in(
                                collection=CollectionNames.ORGS.value,
                                field="name",
                                values=[dealof_edge.get("accountName")]
                            )
                            if existing_org is not None:
                                org_id = existing_org[0].get("id") or existing_org[0].get("_key")
                                dealof_edge.pop("accountName", None)
                                dealof_edge["from_id"] = existing_record.record_group_id
                                dealof_edge["from_collection"] = CollectionNames.RECORD_GROUPS.value
                                dealof_edge["to_id"] = org_id
                                dealof_edge["to_collection"] = CollectionNames.ORGS.value
                                await tx_store.batch_create_edges(
                                    [dealof_edge],
                        ...
                        if salesdeal_edge:
                            salesdeal_edge["from_id"] = self.org_id
                            salesdeal_edge["from_collection"] = CollectionNames.ORGS.value
                            salesdeal_edge["to_id"] = record.id
                            salesdeal_edge["to_collection"] = CollectionNames.RECORDS.value
                            await tx_store.batch_create_edges(
                                [salesdeal_edge],
                                collection=CollectionNames.SALES_DEAL.value,
                            )
            ...
            self.logger.error(
                f"handle_deals_edges failed: {str(e)}", exc_info=True
            )
            raise

    async def handle_sold_in_edges(
        self,
        sold_in_items: list[tuple[Record, list[dict[str, Any]]]],
    ) -> None:
        try:
            if not sold_in_items:
                self.logger.debug("handle_sold_in_edges received empty list; skipping.")
                return

            async with self.data_store_provider.transaction() as tx_store:
                # 1. Delete all soldIn edges that point to each deal
                for deal_record, edges_list in sold_in_items:
                    ...
                    # 2. Form the new edges (only for items that are not deleted)
                    await tx_store.batch_create_edges(
                        edges_list,
                        collection=CollectionNames.SOLD_IN.value,
                    )
            ...
            self.logger.error(
                f"handle_sold_in_edges failed: {str(e)}", exc_info=True
            )
            raise
