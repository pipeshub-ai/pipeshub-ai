...
class BaseRateLimiter(IRateLimiter):
    ...
    def __init__(
        self, logger: logging.Logger, max_rate: int = 100, time_window: int = 60
    ) -> None:
    ...
    async def acquire(self, operation: str = "default") -> bool:
        ...
        try:
            ...
            self._operation_usage[operation] = (
                self._operation_usage.get(operation, 0) + 1
            )
            ...
            self.logger.warning(
                f"Rate limit exceeded for operation '{operation}'. Error: {e!s}",
            )
            return False
