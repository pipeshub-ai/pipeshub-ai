...
class OAuthProvider:
    ...
    async def refresh_access_token(self, refresh_token: str) -> OAuthToken:
        ...
        async with session.post(self.config.token_url, data=data) as response:
            if response.status == HttpStatusCode.FORBIDDEN.value:
                ...
                raise Exception(
                    f"Token refresh failed with 403 Forbidden. This usually means the refresh token has expired or is invalid. Response: {error_text}"
                )
            response.raise_for_status()
            token_data = await response.json()
    ...
    async def start_authorization(
        self, *, return_to: str | None = None, use_pkce: bool = True, **extra
    ) -> str:
    ...
    async def handle_callback(self, code: str, state: str) -> OAuthToken:
        ...
        token = await self.exchange_code_for_token(
            code=code, state=state, code_verifier=oauth_data.get("code_verifier")
        )
        self.token = token
