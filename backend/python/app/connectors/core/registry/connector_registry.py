...
class ConnectorRegistry:
            ...
            for module_name in modules:
                try:
                    ...
                    for attr_name in dir(module):
                        ...
                        if (
                            isclass(attr)
                            and hasattr(attr, "_connector_metadata")
                            and hasattr(attr, "_is_connector")
                        ):
    ...
    async def _create_app_in_db(
        self, app_name: str, metadata: dict[str, Any]
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            app_doc = await arango_service.batch_upsert_nodes(
                [doc], self.collection_name
            )
            if not app_doc:
    ...
    async def _deactivate_app_in_db(self, app_name: str) -> bool:
        ...
        try:
            ...
            if existing_doc:
                ...
                if not list(cursor):
                    self.logger.warning(
                        f"Failed to deactivate app {app_name}: app not found."
                    )
                    return False
    ...
    async def sync_with_database(self) -> bool:
        ...
        try:
            ...
            if deactivated_apps:
                self.logger.info(
                    f"Deactivated {len(deactivated_apps)} apps not in registry: {deactivated_apps}"
                )
    ...
    async def get_all_connectors(self) -> list[dict[str, Any]]:
        ...
        for app_name, metadata in self._connectors.items():
            ...
            connector_info = {
                ...
                "iconPath": metadata.get("config", {}).get(
                    "iconPath", "/assets/icons/connectors/default.svg"
                ),
                "supportsRealtime": metadata.get("config", {}).get(
    ...
    async def get_active_connector(self) -> list[dict[str, Any]]:
        ...
        return [
            ...
            if connector.get("isActive", False)
        ]
    ...
    async def get_inactive_connector(self) -> list[dict[str, Any]]:
        ...
        return [
            connector
            for connector in all_connectors
            if not connector.get("isActive", False)
        ]
    ...
    async def get_connectors_by_group(self, app_group: str) -> list[dict[str, Any]]:
        ...
        return [
            connector
            for connector in await self.get_all_connectors()
    ...
    async def get_filter_options(self) -> dict[str, list[str]]:
        ...
        """
        app_groups = list(
            set(metadata["appGroup"] for metadata in self._connectors.values())
        )

        auth_types = list(
            set(metadata["authType"] for metadata in self._connectors.values())
        )
    ...
    async def create_app_when_configured(self, app_name: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if existing_app:
                self.logger.info(
                    f"App {app_name} already exists in database, skipping creation"
                )
                return existing_app
        except Exception as e:
    ...
    async def update_connector(
        self, app_name: str, updates: dict[str, Any]
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            if not existing_doc:
                self.logger.error(
                    f"App {app_name} not found in database. Please configure the connector first."
                )
                return None
            ...
            if not list(cursor):
                self.logger.warning(
                    f"Failed to update connector for app {app_name}: app not found."
                )
                return None
