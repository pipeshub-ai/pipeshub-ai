...
class JiraClient:
    ...
    async def make_authenticated_json_request(
        ...
        **kwargs,
        ...
        headers["Authorization"] = (
            f"{token.get('token_type')} {token.get('access_token')}"
        )
    ...
    async def fetch_issues_with_permissions(
        self, project_key: str, project_id: str, user: AppUser
        ...
        while True:
            issues_batch = await self.make_authenticated_json_request(
                "GET", url, params={"projectKey": project_key, "maxResults": 25}
            )
    ...
    async def fetch_projects_with_permissions(
        self,
        ...
        projects = []
        while True:
            projects_batch = await self.make_authenticated_json_request(
                "GET",
                url,
                params={
                    "maxResults": 25,
    ...
    async def fetch_issue_content(
        ...
        description_text = (
            f"Description: {adf_to_text(description)}" if description else ""
        )
...
@(
    ...
    .configure(
        lambda builder: builder.with_icon("/assets/icons/connectors/jira.svg")
        ...
        try:
            ...
            for project, permissions in projects:
                issues = await jira_client.fetch_issues_with_permissions(
                    project.short_name, project.external_group_id, user
                )
