...
class JiraConnector(BaseConnector):
    ...
    async def _get_fresh_datasource(self) -> JiraDataSource:
        ...
        # Get current token from client using get_token() method (like Linear)
        internal_client = self.external_client.get_client()
        current_token = internal_client.get_token()

        # Update client's token if it changed (mutation) - set_token() is atomic like Linear
    ...
    async def _fetch_issues_batched(
        ...
        if modified_after:
            ...
            # Use second-level precision to avoid re-fetching issues from the same minute
            jql_conditions.append(f'updated > "{modified_dt.strftime("%Y-%m-%d %H:%M:%S")}"')
        ...
        if created_after:
            ...
            jql_conditions.append(f'created >= "{created_dt.strftime("%Y-%m-%d %H:%M:%S")}"')
    ...
    def _parse_issue_links(self, issue: Dict[str, Any]) -> List[RelatedExternalRecord]:
        ...
        Jira issue links have two sides:
        - outwardIssue: The current issue links TO this issue (use 'outward' description as tag)
        - inwardIssue: This issue links TO the current issue (use 'inward' description as tag)

        Both directions are handled automatically when each issue syncs:
        - When Issue A syncs with outwardIssue: B, we create A → B with "blocks"
        - When Issue B syncs with inwardIssue: A, we create B → A with "is blocked by"
        """
        ...
        for link in issue_links:
            ...
            try:
                ...
                if "outwardIssue" in link:
                    # Current issue → linked issue (use outward description)
                    linked_issue = link.get("outwardIssue")
                    raw_tag = link_type.get("outward", link_type.get("name", "linked to"))
                elif "inwardIssue" in link:
                    # Linked issue → current issue (use inward description)
                    linked_issue = link.get("inwardIssue")
                    raw_tag = link_type.get("inward", link_type.get("name", "linked to"))
                else:
                    continue
                ...
                # Map Jira link type description to standard enum value
                mapped_tag = map_link_relationship_tag(raw_tag)
                # Convert enum to string value if mapped, otherwise use original string
                if isinstance(mapped_tag, LinkRelationshipTag):
                    custom_tag = mapped_tag.value
                else:
                    custom_tag = mapped_tag if mapped_tag else raw_tag

                # Create RelatedExternalRecord with mapped link type as custom tag
    ...
    async def _parse_issue_to_blocks(
        ...
        - Description BlockGroup (index=0) with:
          ...
          - children_records: description inline attachments + issue-level attachments (not in comments)
        ...
        - Block objects (sub_type=COMMENT) for each comment with:
          ...
          - children_records: attachments used in that specific comment

        Per Jira API: media.attrs.id in ADF IS the attachment ID from fields.attachment[].
        We use this to map attachments to their correct location.
        """
        ...
        # Track which attachment IDs are used in comments (to exclude from description)
        comment_attachment_ids: Set[str] = set()

        # Get comments to pre-scan for attachment IDs used in comments
        ...
        for comment in comments_data:
            ...
            if comment_body_adf and isinstance(comment_body_adf, dict):
                media_nodes = extract_media_from_adf(comment_body_adf)
                for media_info in media_nodes:
                    media_id = media_info.get("id", "")
                    if media_id:
                        comment_attachment_ids.add(media_id)
        ...
        # Description children: all attachments NOT used in comments
        # This includes description inline attachments + issue-level attachments
        ...
        if attachment_children_map:
            for attachment_id, child_record in attachment_children_map.items():
                if attachment_id not in comment_attachment_ids:
                    description_children.append(child_record)

        # Create TableRowMetadata with description children
        table_row_metadata = None
        if description_children:
            table_row_metadata = TableRowMetadata(
                children_records=description_children
            )
        ...
        description_block_group = BlockGroup(
            ...
            group_subtype=GroupSubType.ISSUE_CONTENT,
        ...
        if comments_data:
            ...
            for thread_comments in sorted_threads:
                ...
                for comment in thread_comments:
                    ...
                    # Parse timestamps
                    comment_created_at = self._parse_jira_timestamp(comment.get("created", ""))
                    comment_updated_at = self._parse_jira_timestamp(comment.get("updated", ""))

                    # Convert to datetime for Block fields
                    comment_created_dt = datetime.fromtimestamp(comment_created_at / 1000, tz=timezone.utc) if comment_created_at else None
                    comment_updated_dt = datetime.fromtimestamp(comment_updated_at / 1000, tz=timezone.utc) if comment_updated_at else None

                    # Get author info
                    author = comment.get("author", {})
                    author_name = author.get("displayName", "Unknown")

                    # Get attachments used in this comment
                    # Per Jira API: media.attrs.id IS the attachment ID
                    comment_children: List[ChildRecord] = []
                    if attachment_children_map and isinstance(comment_body_adf, dict):
                        comment_media_nodes = extract_media_from_adf(comment_body_adf)
                        for media_info in comment_media_nodes:
                            media_id = media_info.get("id", "")
                            if media_id and media_id in attachment_children_map:
                                comment_children.append(attachment_children_map[media_id])

                    # Create TableRowMetadata with comment's attachments
                    comment_table_row_metadata = None
                    if comment_children:
                        comment_table_row_metadata = TableRowMetadata(
                            children_records=comment_children
                        )
                    ...
                    comment_block = Block(
                        ...
                        source_creation_date=comment_created_dt,
                        source_update_date=comment_updated_dt,
        ...
        if not block_groups:
            minimal_data = f"# {issue_title}" if issue_title else f"# Issue {issue_key or issue_id}"
            minimal_block_group = BlockGroup(
                id=str(uuid4()),
                index=0,
                name=f"{issue_key} - Description" if issue_key else "Issue Description",
                type=GroupType.TEXT_SECTION,
                group_subtype=GroupSubType.ISSUE_CONTENT,
                description=f"Description for issue {issue_key}" if issue_key else "Issue description",
                source_group_id=f"{issue_id}_description",
                ...
                weburl=weburl,
                requires_processing=True,
                table_row_metadata=TableRowMetadata(children_records=description_children) if description_children else None,
            )
