...
@(
    ...
    .with_categories(["Web"])
    .configure(
        lambda builder: builder.with_icon("/assets/icons/connectors/web.svg")
...
class WebConnector(BaseConnector):
    ...
    async def init(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to initialize web connector: {e}", exc_info=True
            )
            return False
        ...
        try:
            async with self.session.head(self.url, allow_redirects=True) as response:
                if response.status < HttpStatusCode.BAD_REQUEST.value:
                    self.logger.info(
                        f"✅ Website accessible: {self.url} (status: {response.status})"
                    )
                    return True
                self.logger.warning(
                    f"⚠️ Website returned status {response.status}: {self.url}"
                )
                return False
        except Exception as e:
    ...
    async def _crawl_single_page(self, url: str) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error crawling single page {url}: {e}", exc_info=True
            )
    ...
    async def _crawl_recursive(self, start_url: str, depth: int) -> None:
        ...
        try:
                ...
                try:
                    ...
                    if file_record:
                        ...
                        if (
                            ...
                            and file_record.mime_type == MimeTypes.HTML.value
                        ):
                        ...
                        if len(batch_records) >= self.batch_size:
                            ...
                            self.logger.info(
                                f"✅ Batch processed: {len(batch_records)} records"
                            )
                            batch_records.clear()
            ...
            if batch_records:
                ...
                self.logger.info(
                    f"✅ Final batch processed: {len(batch_records)} records"
                )
    ...
    async def _extract_links_from_content(
        ...
        try:
            ...
            async with self.session.get(
                file_record.weburl, headers=headers
            ) as response:
                if response.status >= HttpStatusCode.BAD_REQUEST.value:
                    return links
