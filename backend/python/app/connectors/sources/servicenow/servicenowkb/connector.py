...
class ServiceNowKBConnector(BaseConnector):
    ...
    async def init(self) -> bool:
        ...
        try:
            ...
            if not all([self.instance_url, self.client_id, self.client_secret, self.redirect_uri]):
                self.logger.error(
    ...
    async def run_sync(self) -> None:
        ...
        Sync order:
        1. Users and Groups
        2. Knowledge Bases (top-level containers)
        3. Categories (hierarchy within KBs)
        4. KB Articles (content with permissions)
        5. Attachments (files linked to articles)
        """
        try:
            org_id = self.data_entities_processor.org_id
            ...
            # Step 1: Sync users and groups (needed for permissions)
            self.logger.info("Step 1/4: Syncing users and groups...")
            await self._sync_users_and_groups()

            # Step 2: Sync knowledge bases (top-level containers)
            self.logger.info("Step 2/4: Syncing knowledge bases...")
            await self._sync_knowledge_bases()

            # Step 3: Sync categories (hierarchy)
            self.logger.info("Step 3/4: Syncing categories...")
            await self._sync_categories()

            # Step 4: Sync KB articles and attachments
            self.logger.info("Step 4/4: Syncing KB articles and attachments...")
    ...
    async def _sync_users_and_groups(self) -> None:
        """
        Sync users and groups from ServiceNow.
        ...
        try:
            ...
            self.logger.info("✅ Users and groups synced successfully")
        ...
        try:
            ...
            while True:
                response = await self.servicenow_datasource.get_now_table_tableName(
                    tableName="sys_user",
                    sysparm_query=query,
                    sysparm_fields="sys_id,user_name,email,first_name,last_name,title,department,active,sys_created_on,sys_updated_on",
    ...
    async def _sync_knowledge_bases(self) -> None:
        ...
        """
        try:
            # Get last sync checkpoint for delta sync
            ...
            while True:
                ...
                if kb_record_groups:
                    async with self.data_store_provider.transaction() as tx_store:
                        # Save KB RecordGroups
                        await tx_store.batch_upsert_record_groups(
                            [kb for kb, _ in kb_record_groups]
                        )

                        # Create permission edges for each KB using centralized method
                        for kb_record_group, kb_data in kb_record_groups:
                            # Prepare permissions dict for conversion
                            kb_permissions = []
                            ...
                            # Manager permissions
                            kb_managers = kb_data.get("kb_managers")
                            if kb_managers:
                                manager_sys_ids = []
                                if isinstance(kb_managers, str):
                                    manager_sys_ids = [m.strip() for m in kb_managers.split(",") if m.strip()]
                                elif isinstance(kb_managers, list):
                                    manager_sys_ids = kb_managers

                                for manager_sys_id in manager_sys_ids:
                                    kb_permissions.append({
                                        "entity_type": "USER",
                                        "source_sys_id": manager_sys_id,
                                        "role": PermissionType.WRITE.value,
                                    })

                            # Convert to Permission objects and batch upsert
                            if kb_permissions:
    ...
    async def _sync_categories(self) -> None:
        ...
        try:
            ...
            if latest_update_time:
                await self.category_sync_point.create_sync_point("categories", {"last_sync_time": latest_update_time})
    ...
    async def _sync_articles(self) -> None:
        ...
        3. For each article:
           ...
           - Fetch & create attachments (FileRecords)
           - Create all edges (hierarchy + permissions)
        ...
        try:
            ...
            while True:
                ...
                criteria_map = {}
                if all_criteria_ids:
                    criteria_map = await self._fetch_user_criteria_batch(all_criteria_ids)

                # Process each article individually
                for article_data in articles_data:
                    try:
                        attachments_count = await self._process_single_article(article_data, criteria_map)
                        total_articles_synced += 1
                        total_attachments_synced += attachments_count
                    except Exception as e:
    ...
    async def _process_single_article(
        ...
        """
        Process a single article with all its attachments and permissions.
        ...
        Returns:
            int: Number of attachments processed for this article
        """
        try:
            ...
            # Step 1: Transform article to WebpageRecord
            ...
            # Step 2: Fetch attachments for this article
            attachments_data = await self._fetch_attachments_for_article(article_sys_id)

            # Step 3: Transform attachments to FileRecords
            attachment_records = []
            self.logger.debug(f"Found {len(attachments_data)} attachments: {attachments_data}")
            for att_data in attachments_data:
                # Pass article's record_group info to attachment
            ...
            # Step 4: Extract article permissions
            article_permissions = await self._extract_article_permissions(article_data, criteria_map)

            # Step 5: Save everything in transaction
            async with self.data_store_provider.transaction() as tx_store:
                # Convert USER and GROUP permissions to Permission objects for platform handling
                all_permission_objects = await self._convert_permissions_to_objects(article_permissions, tx_store)

                # Save article record with all permissions (platform handles USER + GROUP)
                await self.data_entities_processor.on_new_records([(article_record, all_permission_objects)])

                # Save attachment records with same permissions as article
                if attachment_records:
                    await self.data_entities_processor.on_new_records([(rec, all_permission_objects) for rec in attachment_records])
                    self.logger.debug(f"Saved {len(attachment_records)} attachments for article {article_sys_id}")

                self.logger.debug(f"✅ Article {article_sys_id} with {len(attachment_records)} attachments and {len(all_permission_objects)} permissions processed")

            return len(attachment_records)
    ...
    async def _fetch_user_criteria_batch(
        ...
        try:
            ...
            response = await self.servicenow_datasource.get_now_table_tableName(
                ...
                sysparm_query=query,
                sysparm_fields="sys_id,users,groups",
                sysparm_display_value="false",
                sysparm_no_count="true",
                sysparm_exclude_reference_link="true",
            )
            ...
            for criteria_data in response.data.get("result", []):
                ...
                criteria_map[sys_id] = {"users": users, "groups": groups}
                        ...
                        permissions.append(
                            {
                                ...
                                "role": PermissionType.READER.value,
                            }
                        )
