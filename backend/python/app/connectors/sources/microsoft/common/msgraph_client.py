...
class DeltaGetResponse(BaseDeltaFunctionResponse, Parsable):
    ...
    def get_field_deserializers(self) -> dict[str, Callable[[ParseNode], None]]:
        ...
        fields: dict[str, Callable[[Any], None]] = {
            "value": lambda n: setattr(
                self, "value", n.get_collection_of_object_values(DriveItem)
            ),
        }
...
class MSGraphClient:
    ...
    async def get_all_user_groups(self) -> list[dict]:
        ...
        try:
            ...
            while result.odata_next_link:
                async with self.rate_limiter:
                    result = await self.client.groups.get_next_page(
    ...
    async def get_group_members(self, group_id: str) -> list[dict]:
        ...
        try:
            ...
            while result.odata_next_link:
                async with self.rate_limiter:
                    result = await self.client.groups.by_group_id(
                        group_id
                    ).members.get_next_page(result.odata_next_link)
                members.extend(result.value)
    ...
    async def get_all_users(self) -> list[AppUser]:
        ...
        try:
            ...
            async with self.rate_limiter:
                ...
                while result.odata_next_link:
                    async with self.rate_limiter:
                        result = await self.client.users.get_next_page(
                            result.odata_next_link
                        )
                    users.extend(result.value)
            ...
            for user in users:
                user_list.append(
                    AppUser(
                        ...
                        source_created_at=user.created_date_time.timestamp()
    ...
    async def get_delta_response_sharepoint(self, url: str) -> dict:
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved delta response with {len(response['drive_items'])} items"
            )
            return response
    ...
    async def get_file_permission(
        ...
        try:
            ...
            while (
                result and hasattr(result, "odata_next_link") and result.odata_next_link
            ):
                async with self.rate_limiter:
                    result = (
                        await self.client.drives.by_drive_id(drive_id)
                        .items.by_drive_item_id(item_id)
                        .permissions.get_next_page(result.odata_next_link)
                    )
                if result and result.value:
                    permissions.extend(result.value)

            self.logger.info(
                f"Retrieved {len(permissions)} permissions for file ID {item_id}."
            )
            return permissions
        except ODataError as e:
            self.logger.error(
                f"Error fetching file permissions for File ID {item_id}: {e}"
            )
            return []
        except Exception as ex:
            self.logger.error(
                f"Unexpected error fetching file permissions for File ID {item_id}: {ex}"
            )
            return []
        ...
        try:
            async with self.rate_limiter:
                ...
                if item and hasattr(item, "additional_data"):
                    signed_url = item.additional_data.get(
            ...
            self.logger.error(
                f"Error creating signed URL for item {item_id} in drive {drive_id}: {ex}"
            )
            return None
