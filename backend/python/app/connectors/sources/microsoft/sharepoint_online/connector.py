...
@(
    ...
    .configure(
        lambda builder: builder.with_icon("/assets/icons/connectors/sharepoint.svg")
...
class SharePointConnector(BaseConnector):
    ...
    def __init__(
        ...
        self.filters = {
            ...
            "exclude_pages": True,
            "exclude_lists": True,
            "exclude_document_libraries": True,
        }
    ...
    async def init(self) -> None:
        config = await self.config_service.get_config(
            "/services/connectors/sharepointonline/config"
        ) or await self.config_service.get_config(
            f"/services/connectors/sharepointonline/config/{self.data_entities_processor.org_id}"
        )
        if not config:
        ...
        if not all((tenant_id, client_id, client_secret, sharepoint_domain)):
            self.logger.error(
                "❌ Incomplete SharePoint Online credentials. Ensure tenantId, clientId, and clientSecret are configured."
            )
            raise ValueError(
                "Incomplete SharePoint Online credentials. Ensure tenantId, clientId, and clientSecret are configured."
            )
        ...
        self.msgraph_client = MSGraphClient(
            self.connector_name, self.client, self.logger
        )
        ...
        if "," in site_id:
            ...
            if len(parts) == SITE_ID_PARTS:
                ...
                if (
                    hostname
                    and "." in hostname
                    and len(site_guid) >= GUID_LENGTH  # GUID-like length
                    and len(web_guid) >= GUID_LENGTH
                ):  # GUID-like length
                    return True
            else:
                self.logger.warning(
                    f"⚠️ Composite site ID has {len(parts)} parts, expected 3: {site_id}"
                )
                return False
    ...
    def _normalize_site_id(self, site_id: str) -> str:
        ...
        for composite_id in self.site_cache:
            ...
            if (
                len(parts) == COMPOSITE_SITE_ID_PARTS_COUNT
                and site_id == f"{parts[1]},{parts[2]}"
            ):
                return composite_id
        ...
        if site_id.count(",") == 1 and getattr(self, "sharepoint_domain", None):
            host = (
                urllib.parse.urlparse(self.sharepoint_domain).hostname
                or self.sharepoint_domain
            )
            if host and "." in host:
    ...
    async def _safe_api_call(
        self, api_call, max_retries: int = 3, retry_delay: float = 1.0
    ) -> None:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if any(
                    ...
                    self.logger.debug(
                        f"Permission denied on API call (attempt {attempt + 1}): {e}"
                    )
                    return None
                ...
                if any(
                    term in error_str
                    for term in [HttpStatusCode.NOT_FOUND.value, "notfound"]
                ):
                    self.logger.debug(
                        f"Resource not found on API call (attempt {attempt + 1}): {e}"
                    )
                    return None
                ...
                if any(
                    ...
                    self.logger.warning(
                        f"⚠️ Bad request on API call (attempt {attempt + 1}): {e}"
                    )
                    return None
                ...
                if any(
                    ...
                    if attempt < max_retries:
                        ...
                        self.logger.warning(
                            f"⚠️ Retryable error (attempt {attempt + 1}/{max_retries + 1}): {e}. Retrying in {wait_time}s..."
                        )
                ...
                if attempt < max_retries:
                    ...
                    self.logger.warning(
                        f"⚠️ API call failed (attempt {attempt + 1}/{max_retries + 1}): {e}. Retrying in {wait_time}s..."
                    )
                    await asyncio.sleep(wait_time)
                else:
                    self.logger.error(
                        f"❌ API call failed after {max_retries + 1} attempts. Last error: {e}"
                    )
        ...
        try:
            ...
            async with self.rate_limiter:
                try:
                    ...
                    if root_site:
                        ...
                        self.logger.info(
                            f"Root site found: '{root_site.display_name or root_site.name}' - ID: '{root_site.id}'"
                        )
                        ...
                        self.logger.warning(
                            "Could not fetch root site. Continuing with site search..."
                        )
                except Exception as root_error:
                    self.logger.warning(
                        f"⚠️ Root site access failed: {root_error}. Continuing with site search..."
                    )
                ...
                try:
                    ...
                    if search_results and search_results.value:
                        for site in search_results.value:
                            self.logger.debug(
                                f"Checking site: '{site.display_name or site.name}' - URL: '{site.web_url}'"
                            )
                            self.logger.debug(
                                f"exclude_onedrive_sites: {self.filters.get('exclude_onedrive_sites')}"
                            )
                            parsed_url = urllib.parse.urlparse(site.web_url)
                            hostname = parsed_url.hostname
                            contains_onedrive = hostname is not None and re.fullmatch(
                                r"[a-zA-Z0-9-]+-my\.sharepoint\.com", hostname
                            )
                            self.logger.debug(
                                f"Hostname matches expected OneDrive pattern: {bool(contains_onedrive)}"
                            )

                            if (
                                self.filters.get("exclude_onedrive_sites")
                                and contains_onedrive
                            ):
                                self.logger.debug(
                                    f"Skipping OneDrive site: '{site.display_name or site.name}'"
                                )
                                continue

                            self.logger.debug(
                                f"Site found: '{site.display_name or site.name}' - ID: '{site.id}'"
                            )
                            self.logger.debug(f"Site URL: '{site}'")
                            # Avoid duplicates
                            if not any(
                                existing_site.id == site.id for existing_site in sites
                            ):
                                sites.append(site)
                        ...
                        self.logger.info(
                            f"Found {len(search_results.value)} additional sites from search"
                        )
                    else:
                        self.logger.info(
                            "No additional sites found from search endpoint"
                        )
                except Exception as search_error:
                    self.logger.warning(
                        f"⚠️ Site search failed: {search_error}. Continuing with available sites..."
                    )
            ...
            if self.enable_subsite_discovery and sites:
                ...
                for site in list(sites):
                    try:
                        ...
                        self.logger.debug(
                            f"Subsite discovery failed for {site.display_name or site.name}: {subsite_error}"
                        )
            ...
            for site in sites:
                if self._validate_site_id(site.id):
                    ...
                    self.logger.warning(
                        f"⚠️ Invalid site ID format, skipping: '{site.id}' ({site.display_name or site.name})"
                    )

            self.logger.info(
                f"Total valid SharePoint sites discovered: {len(valid_sites)}"
            )
            return valid_sites
    ...
    async def _sync_site_content(self, site_record_group: RecordGroup) -> None:
        ...
        try:
            ...
            async for record, permissions, record_update in self._process_site_drives(
                site_id, internal_site_record_group_id=site_record_group.id
            ):
            ...
            self.logger.info(
                f"Completed sync for site '{site_name}' - processed {total_processed} items"
            )
    ...
    async def _process_site_drives(
        self, site_id: str, internal_site_record_group_id: str
    ) -> AsyncGenerator[tuple[Record, list[Permission], RecordUpdate], None]:
        ...
        try:
            ...
            for drive in drives:
                ...
                drive_record_group = self._create_document_library_record_group(
                    drive, site_id, internal_site_record_group_id
                )
            ...
            self.logger.info(
                f"Found {len(drive_record_groups_with_permissions)} drive record groups to process."
            )
            ...
            for (
                drive_record_group,
                _permissions,
            ) in drive_record_groups_with_permissions:
                # Process items in the drive using delta
                item_count = 0
                self.logger.info(f"Drive record group: {drive_record_group}")
                async for item_tuple in self._process_drive_delta(
                    site_id, drive_record_group.external_group_id
                ):
                    yield item_tuple
                    item_count += 1
    ...
    async def _process_drive_delta(
        ...
        try:
            ...
            sync_point = await self.drive_delta_sync_point.read_sync_point(
                sync_point_key
            )
            ...
            if delta_url:
                ...
                self.logger.debug(
                    f"Parsed URL for drive_id: {drive_id} is {parsed_url}"
                )
                if not (
                    ...
                    result = await self.msgraph_client.get_delta_response_sharepoint(
                        delta_url
                    )

            if not delta_url:
                self.logger.info(
                    f"No delta URL found for drive_id: {drive_id}, starting fresh delta sync"
                )
            ...
            while result:
                ...
                for item in drive_items:
                    try:
                        ...
                        self.logger.error(
                            f"❌ Error processing drive item: {item_error}"
                        )
                        continue
            ...
            self.logger.error(
                f"❌ Error processing drive delta for drive {drive_id}: {e}"
            )
            ...
            try:
                ...
                self.logger.info(
                    f"✅ Cleared sync point for drive {drive_id} due to error"
                )
            except Exception as clear_error:
                self.logger.error(f"Failed to clear sync point: {clear_error}")

    async def _process_drive_item(
        self, item: DriveItem, site_id: str, drive_id: str, users: list[AppUser]
    ) -> RecordUpdate | None:
        ...
        try:
            ...
            if existing_record:
                ...
                if (
                    hasattr(item, "file")
                    and item.file
                    and hasattr(item.file, "hashes")
                    and item.file.hashes
                ):
    ...
    async def _create_file_record(
        self, item: DriveItem, drive_id: str, existing_record: Record | None
    ) -> FileRecord | None:
        ...
        try:
            ...
            if (
                hasattr(item, "last_modified_date_time")
            ...
            return FileRecord(
                ...
                else existing_record.record_status,
    ...
    async def _process_site_lists(
        ...
        try:
            ...
            for list_obj in lists:
                try:
                    list_name = getattr(list_obj, "display_name", None) or getattr(
                        list_obj, "name", "Unknown List"
                    )
                    ...
                    if list_record:
                        ...
                        async for item_tuple in self._process_list_items(
                            site_id, list_id
                        ):
                            yield item_tuple
                            item_count += 1
                            if item_count >= max_items_per_list:
                                self.logger.warning(
                                    f"⚠️ Reached item limit ({max_items_per_list}) for list '{list_name}'"
                                )
                                break

                        self.logger.debug(
                            f"Processed {item_count} items from list '{list_name}'"
                        )
                    ...
                    self.logger.warning(
                        f"⚠️ Error processing list '{list_name}': {list_error}"
                    )
                    continue
    ...
    async def _create_list_record(
        self, list_obj: dict, site_id: str
    ) -> SharePointListRecord | None:
        ...
        try:
            ...
            if (
                hasattr(list_obj, "last_modified_date_time")
                and list_obj.last_modified_date_time
            ):
            ...
            if hasattr(list_obj, "list") and list_obj.list:
                metadata["list_template"] = str(
                    getattr(list_obj.list, "template", None)
                )
            ...
            while page_count < max_pages:
                try:
                    async with self.rate_limiter:
                        if skip_token:
                            items_response = await self._safe_api_call(
                                self.client.sites.by_site_id(encoded_site_id)
                    ...
                    for item in items_response.value:
                        try:
                            list_item_record = await self._create_list_item_record(
                                item, site_id, list_id
                            )
                            if list_item_record:
                                permissions = await self._get_list_item_permissions(
                                    site_id, list_id, item.id
                                )
                                yield (
                                    list_item_record,
                                    permissions,
                            ...
                            self.logger.error(
                                f"❌ Error processing list item: {item_error}"
                            )
                            continue
                    ...
                    skip_token = None
                    if (
                        hasattr(items_response, "odata_next_link")
                        and items_response.odata_next_link
                    ):
                        try:
                            parsed_url = urllib.parse.urlparse(
                                items_response.odata_next_link
                            )
                    ...
                    self.logger.error(
                        f"❌ Error processing page {page_count + 1} of list items: {page_error}"
                    )
                    break
    ...
    async def _create_list_item_record(
        self, item: ListItem, site_id: str, list_id: str
    ) -> SharePointListItemRecord | None:
        ...
        try:
            ...
            try:
                if (
                    hasattr(item, "fields")
                    and item.fields
                    and hasattr(item.fields, "additional_data")
                ):
                    fields_data = dict(item.fields.additional_data)
                    title = (
                        fields_data.get("Title")
                        or fields_data.get("LinkTitle")
                        or fields_data.get("Name")
                        or title
                    )
            except Exception:
                pass
            ...
            metadata = {
                "site_id": site_id,
                "list_id": list_id,
                "content_type": getattr(item.content_type, "name", None)
        ...
        try:
            ...
            async with self.rate_limiter:
                try:
                    ...
                    if any(
                        ...
                        self.logger.debug(
                            f"Pages not accessible for site {site_id}: {pages_error}"
                        )
                        return
                    else:
                        raise pages_error
            ...
            for page in pages:
                try:
                    ...
                    self.logger.warning(
                        f"Error processing page '{page_name}': {page_error}"
                    )
                    continue
    ...
    async def _create_page_record(
        self, page: SitePage, site_id: str
    ) -> SharePointPageRecord | None:
        ...
        try:
            ...
            page_name = getattr(page, "title", None) or getattr(
                page, "name", f"Page {page_id}"
            )
            ...
            if (
                hasattr(page, "last_modified_date_time")
                and page.last_modified_date_time
            ):
            ...
            metadata = {
                "site_id": site_id,
                "page_layout": getattr(page.page_layout, "type", None)
    ...
    def _create_document_library_record_group(
        self, drive: dict, site_id: str, internal_site_record_group_id: str
    ) -> RecordGroup | None:
        ...
        try:
            ...
            if (
                hasattr(drive, "last_modified_date_time")
                and drive.last_modified_date_time
            ):
                source_updated_at = int(
                    drive.last_modified_date_time.timestamp() * 1000
                )
        ...
        try:
            ...
            try:
                ...
                if perms_response and perms_response.value:
                    graph_permissions = await self._convert_to_permissions(
                        perms_response.value
                    )
                    permissions.extend(graph_permissions)
                    self.logger.debug(
                        f"✅ Retrieved {len(graph_permissions)} Graph API permissions for site {site_id}"
                    )
            except Exception as graph_error:
                self.logger.debug(
                    f"Graph API permissions failed for site {site_id}: {graph_error}"
                )

            # Method 2: Get SharePoint site groups (the main method for standard SharePoint permissions)
            try:
                site_groups_permissions = (
                    await self._get_sharepoint_site_groups_permissions(site_id)
                )
                permissions.extend(site_groups_permissions)
                self.logger.info(
                    f"✅ Retrieved {len(site_groups_permissions)} SharePoint group permissions for site {site_id}"
                )
            except Exception as rest_error:
                self.logger.warning(
                    f"❌ SharePoint REST API permissions failed for site {site_id}: {rest_error}"
                )
            ...
            if not permissions:
                try:
                    ...
                    self.logger.info(
                        f"✅ Retrieved {len(role_permissions)} role assignment permissions for site {site_id}"
                    )
                except Exception as role_error:
                    self.logger.debug(
                        f"Role assignments failed for site {site_id}: {role_error}"
                    )

            self.logger.info(
                f"✅  Total permissions retrieved for site {site_id}: {len(permissions)}"
            )
            return permissions
    ...
    async def _get_sharepoint_access_token(self) -> str | None:
        ...
        try:
            ...
            if parsed.hostname:
                ...
                resource_host = (
                    self.sharepoint_domain.replace("https://", "")
                    .replace("http://", "")
                    .strip("/")
                )
            ...
            self.logger.debug(
                f"Requesting SharePoint token for resource: {resource}/.default"
            )
            ...
            self.logger.error(
                "   Make sure your app has SharePoint permissions (Sites.Read.All)"
            )
            self.logger.error(
    ...
    async def _get_sharepoint_site_groups_permissions(
        self, site_id: str
    ) -> list[Permission]:
        ...
        try:
            ...
            if not access_token:
                self.logger.warning(
                    "Could not get access token for SharePoint REST API"
                )
                return []
            ...
            for group in site_groups:
                try:
                    ...
                    group_permission_type = (
                        self._map_sharepoint_group_to_permission_type(
                            group_name, group_login_name
                        )
                    )

                    self.logger.debug(
                        f"Processing group '{group_name}' (ID: {group_id}, LoginName: '{group_login_name}') with permission type: {group_permission_type}"
                    )

                    # Get group members
                    group_members = await self._get_sharepoint_group_members(
                        site_url, group_id, access_token
                    )
                    ...
                    for member in group_members:
                        ...
                        member_principal_type = member.get(
                            "PrincipalType", 1
                        )  # 1 = User, 8 = SecurityGroup

                        if member_id:
                            # Determine entity type based on PrincipalType
                            entity_type = (
                                EntityType.USER
                                if member_principal_type == 1
                            ...
                            self.logger.debug(
                                f"Added permission for {entity_type.value.lower()} '{member_name}' ({member_email or member_login}) in group '{group_name}'"
                            )

                except Exception as group_error:
                    self.logger.warning(
                        f"Error processing group '{group.get('Title', 'unknown')}': {group_error}"
                    )
                    continue
            ...
            self.logger.error(
                f"❌ Error getting SharePoint site groups permissions: {e}"
            )
            return []

    def _map_sharepoint_group_to_permission_type(
        self, group_name: str, group_login_name: str = ""
    ) -> PermissionType:
        ...
        if (
            "everyone except external users" in combined_name
            or "everyone" in combined_name
            or "limited access" in combined_name
        ):
            return PermissionType.READ

        # Default to read for unknown groups
        self.logger.debug(
            f"Unknown SharePoint group pattern, defaulting to READ: '{group_name}' (LoginName: '{group_login_name}')"
        )
        return PermissionType.READ

    async def _get_sharepoint_site_groups(
        self, site_url: str, access_token: str
    ) -> list[dict]:
        ...
        try:
            ...
            async with self.rate_limiter:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    self.logger.debug(
                        f"Retrieved {len(groups)} site groups from {site_url}"
                    )

                    # Log group details for debugging
                    for group in groups:
                        self.logger.debug(
                            f"Found group: '{group.get('Title')}' (LoginName: '{group.get('LoginName')}', ID: {group.get('Id')})"
                        )
            ...
            if e.response.status_code == HttpStatusCode.FORBIDDEN.value:
                self.logger.warning(
                    f"Access denied when getting site groups from {site_url}: {e}"
                )
            elif e.response.status_code == HttpStatusCode.NOT_FOUND.value:
                self.logger.warning(
                    f"Site groups endpoint not found for {site_url}: {e}"
                )
                ...
                self.logger.error(
                    f"❌ HTTP error getting SharePoint site groups from {site_url}: {e}"
                )
            return []
        except Exception as e:
            self.logger.error(
                f"Error getting SharePoint site groups from {site_url}: {e}"
            )
            return []
    ...
    async def _get_site_role_assignments(self, site_id: str) -> list[Permission]:
        ...
        try:
                ...
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    for assignment in role_assignments:
                        try:
                            ...
                            self.logger.debug(
                                f"Added role assignment permission for {entity_type.value.lower()} '{member_title}' with type {permission_type}"
                            )

                        except Exception as assignment_error:
                            self.logger.debug(
                                f"❌ Error processing role assignment: {assignment_error}"
                            )
                            continue
            ...
            self.logger.debug(
                f"❌ Error getting role assignments for site {site_id}: {e}"
            )
            return []
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == HttpStatusCode.UNAUTHORIZED.value:
                    ...
                    self.logger.error(
                        "   This usually means the token audience is wrong."
                    )
                    self.logger.error(
                        f"   Token must be requested for '{site_url.split('/')[2]}/.default'"
                    )
                    self.logger.error(
                        "   NOT for 'https://graph.microsoft.com/.default'"
                    )
                    return []
            ...
            self.logger.error(
                f"❌ HTTP {e.response.status_code} error getting site groups: {e}"
            )
            return []
        except Exception as e:
    ...
    async def _get_sharepoint_group_members(
        self, site_url: str, group_id: int, access_token: str
    ) -> list[dict]:
        ...
        try:
            ...
            async with self.rate_limiter:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    self.logger.debug(
                        f"Retrieved {len(members)} members for group {group_id}"
                    )
                    return members
            ...
            if e.response.status_code == HttpStatusCode.FORBIDDEN.value:
                self.logger.warning(
                    f"Access denied when getting group members for group {group_id}: {e}"
                )
            elif e.response.status_code == HttpStatusCode.NOT_FOUND.value:
                self.logger.warning(
                    f"Group members endpoint not found for group {group_id}: {e}"
                )
            else:
                self.logger.error(
                    f"❌ HTTP error getting SharePoint group members for group {group_id}: {e}"
                )
            return []
        except Exception as e:
            self.logger.error(
                f"❌ Error getting SharePoint group members for group {group_id}: {e}"
            )
            return []
            ...
            async with self.rate_limiter:
                ...
                if root_item:
                    perms_response = await self._safe_api_call(
                        self.client.sites.by_site_id(encoded_site_id)
                        .drives.by_drive_id(drive_id)
                        .items.by_drive_item_id(root_item.id)
                        .permissions.get(),
                    )
                else:
                    perms_response = None
    ...
    async def _get_list_permissions(
        self, site_id: str, list_id: str
    ) -> list[Permission]:
        ...
        try:
            ...
            self.logger.debug(
                f"List permissions not directly accessible via Graph API for list {list_id}"
            )
            return []
    ...
    async def _get_list_item_permissions(
        ...
        try:
            ...
            self.logger.debug(
                f"List item permissions not directly accessible via Graph API for item {item_id}"
            )
            return []
    ...
    async def _get_page_permissions(
        ...
        try:
            ...
            self.logger.debug(
                f"Page permissions not directly accessible via Graph API for page {page_id}"
            )
            return []
    ...
    async def _convert_to_permissions(
        self, msgraph_permissions: list
    ) -> list[Permission]:
        ...
        for perm in msgraph_permissions:
            try:
                ...
                if hasattr(perm, "granted_to") and perm.granted_to:
                    if hasattr(perm.granted_to, "user") and perm.granted_to.user:
                        ...
                        permissions.append(
                            Permission(
                                external_id=user.id,
                                email=getattr(user, "mail", None)
                                or getattr(user, "user_principal_name", None),
                                type=map_msgraph_role_to_permission_type(
                                    perm.roles[0] if perm.roles else "read"
                                ),
                                entity_type=EntityType.USER,
                            )
                        )
                ...
                if (
                    hasattr(perm, "granted_to_identities")
                    and perm.granted_to_identities
                ):
    ...
    async def _sync_user_groups(self) -> None:
        ...
        try:
            ...
            try:
                ...
                for group in groups:
                    ...
                    user_group = AppUserGroup(
                        ...
                        created_at_timestamp=self._parse_datetime(
                            group.created_date_time
                        ),
                    )
                    ...
                    try:
                        ...
                        self.logger.info(
                            f"❌ Error getting members for group {group.display_name}: {member_error}"
                        )
                ...
                self.logger.error(
                    f"❌ Error getting Microsoft 365 Groups: {groups_error}"
                )
            ...
            #     continue
                ...
                self.logger.error(
                    f"❌ Error processing sites for permissions: {sites_error}"
                )

            self.logger.info(
                f"Completed SharePoint group synchronization - processed {total_groups} groups"
            )
    ...
    def _map_group_to_permission_type(self, group_name: str) -> PermissionType:
        ...
        if any(
            term in group_name_lower for term in ["owner", "admin", "full control"]
        ) or any(
            term in group_name_lower for term in ["member", "contributor", "editor"]
        ):
            return PermissionType.WRITE
        return PermissionType.READ
    ...
    async def run_sync(self) -> None:
        ...
        try:
            ...
            for site in sites:
                ...
                site_id = site.id
                source_created_at = (
                    int(site.created_date_time.timestamp() * 1000)
                    if site.created_date_time
                    else None
                )
                source_updated_at = (
                    int(site.last_modified_date_time.timestamp() * 1000)
            ...
            for i in range(
                0, len(site_record_groups_with_permissions), self.max_concurrent_batches
            ):
                batch = site_record_groups_with_permissions[
                    i : i + self.max_concurrent_batches
                ]
                batch_start = i + 1
                batch_end = min(
                    i + len(batch), len(site_record_groups_with_permissions)
                )

                self.logger.info(
                    f"📊 Processing site batch {batch_start}-{batch_end} of {len(site_record_groups_with_permissions)}"
                )

                # Process batch
                tasks = [
                    self._sync_site_content(site_record_group)
                    for site_record_group, _permissions in batch
                ]
                ...
                for idx, result in enumerate(results):
                    if isinstance(result, Exception):
                        ...
                        self.logger.info(
                            f"✅ Site sync completed: {batch[idx][0].name}"
                        )
            ...
            duration = datetime.now() - start_time
            self.logger.error(
                f"💥 Critical error in SharePoint connector after {duration}: {e}"
            )
            raise
        ...
        try:
            ...
            for site in sites:
                try:
                    ...
                    self.logger.error(
                        f"❌ Error in incremental sync for site {site.display_name or site.name}: {site_error}"
                    )
                    continue
    ...
    async def test_connection_and_access(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error testing connection and access to SharePoint: {e}"
            )
            return False
    ...
    async def get_signed_url(self, record: Record) -> str:
        ...
        try:
            ...
            # Get download URL
            signed_url = await self.msgraph_client.get_signed_url(
                drive_id, record.external_record_id
            )
            return signed_url

        except Exception as e:
            self.logger.error(
                f"❌ Error creating signed URL for record {record.id}: {e}"
            )
            raise
        ...
        try:
            ...
            if hasattr(self, "msgraph_client") and self.msgraph_client:
                try:
                    ...
                    self.logger.debug(
                        f"❌ Error cleaning up MSGraph client: {msgraph_error}"
                    )
...
class SharePointSubscriptionManager:
    ...
    async def create_site_subscription(
        self, site_id: str, notification_url: str
    ) -> str | None:
    ...
    async def create_drive_subscription(
        self, site_id: str, drive_id: str, notification_url: str
    ) -> str | None:
        ...
        try:
            ...
            if result and result.id:
                ...
                self.logger.info(
                    f"Created subscription {result.id} for drive {drive_id}"
                )
                return result.id
            ...
            self.logger.error(
                f"❌ Error creating subscription for drive {drive_id}: {e}"
            )
            return None
    ...
    async def renew_subscription(self, subscription_id: str) -> bool:
        ...
        try:
            ...
            await self.client.subscriptions.by_subscription_id(subscription_id).patch(
                subscription_update
            )
    ...
    async def delete_subscription(self, subscription_id: str) -> bool:
        ...
        try:
            ...
            resource = next(
                (k for k, v in self.subscriptions.items() if v == subscription_id), None
            )
            if resource:
