...
class OutlookConnector(BaseConnector):
    ...
    async def run_sync(self) -> None:
        ...
        try:
            ...
            async for status in self._process_users_in_batches(org_id, users_to_sync):
    ...
    async def _process_user_emails(self, org_id: str, user: AppUser) -> str:
        """Process emails from all folders for a single user."""
        try:
            ...
            # Limit concurrent folder processing to avoid API rate limits
            MAX_CONCURRENT_FOLDERS = 3
            semaphore = asyncio.Semaphore(MAX_CONCURRENT_FOLDERS)

            async def process_folder(folder: Dict) -> str:
                async with semaphore:
                    try:
                        folder_name = self._safe_get_attr(folder, 'display_name', 'Unnamed Folder')

                        result = await self._process_single_folder_messages(org_id, user, folder)
                        return f"{folder_name}: {result} messages"

                    except Exception as e:
                        self.logger.error(f"Error processing folder {folder.get('display_name', 'unknown')}: {e}")
                        return f"{folder.get('display_name', 'unknown')}: Failed"

            # Process all folders concurrently (with semaphore limit)
            tasks = [process_folder(folder) for folder in folders]
            results = await asyncio.gather(*tasks, return_exceptions=True)

            # Count total processed and collect results
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    folder_name = folders[i].get('display_name', 'unknown')
                    folder_results.append(f"{folder_name}: Error - {str(result)}")
                else:
                    folder_results.append(str(result))
                    # Try to extract number from result string
                    try:
    ...
    async def _get_all_folders_for_user(self, user_id: str) -> List[Dict]:
        ...
        try:
            ...
            self.logger.debug(f"Fetched folders for user {user_id}: {response}")
            ...
            print(f"Fetched {len(folders)} folders for user {user_id}: {folders}")
    ...
    async def _process_single_folder_messages(self, org_id: str, user: AppUser, folder: Dict) -> int:
        """Process messages for a specific folder."""
        try:
            user_id = user.source_user_id
            ...
            for i in range(0, len(messages), batch_size):
                ...
                for message in batch:
                    ...
                    # Process the email with folder information
                    update = await self._process_single_email_with_folder(org_id, user, message, folder_id, folder_name)
                    if update:
                        batch_updates.append(update)

                        # Only process attachments if the email is new or updated
                        ...
                        # Email was skipped due to no changes - also skip attachment processing
                ...
                if batch_updates:
                    ...
                    if success:
                        ...
                        self.logger.error(f"Failed to save batch for folder {folder_name}")
                        # Don't update sync point if batch save failed
                        return processed_count

                skipped_count += batch_skipped

                # Log batch progress
                if batch_updates or batch_skipped > 0:
                    self.logger.info(f"Batch {i//batch_size + 1}: Processed {len(batch_updates)}, Skipped {batch_skipped} messages")
            ...
            # Log final summary
            total_messages = processed_count + skipped_count
            self.logger.info(f"Folder '{folder_name}' completed: {processed_count} processed, {skipped_count} skipped, {total_messages} total messages")
    ...
    async def _save_records_batch(self, updates: List[RecordUpdate]) -> bool:
        ...
        Note: Transaction handling is done by data_entities_processor.on_new_records()
        ...
        try:
            ...
            # Save all records with their permissions atomically
            # The data_entities_processor.on_new_records() method handles its own transaction
    ...
    async def _save_batch_with_sync_point(self, updates: List[RecordUpdate], sync_point_key: str, sync_point_data: dict) -> bool:
        """Save a batch of record updates and sync point within a single transaction."""
        if not updates:
            # Still update sync point even if no records to save
            try:
                await self.email_delta_sync_point.update_sync_point(sync_point_key, sync_point_data)
                return True
            except Exception as e:
                self.logger.error(f"Error updating sync point: {e}")
                return False

        try:
            # Use transaction to ensure records, permissions, and sync point are all updated atomically
            async with self.data_store_provider.transaction() as tx_store:
                records_with_permissions = []
                for update in updates:
                    if update.record:
                ...
                # Update sync point in the same transaction
                await tx_store.update_sync_point(sync_point_key, sync_point_data)

                return True

        except Exception as e:
            self.logger.error(f"Error saving batch with sync point in transaction: {e}")
            return False
