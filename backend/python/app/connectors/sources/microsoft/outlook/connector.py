...
THREAD_ROOT_EMAIL_CONVERSATION_INDEX_LENGTH = (
    22  # Length (in bytes) of conversation_index for root email in a thread
)
...
@(
    ...
    .configure(
        lambda builder: builder.with_icon("/assets/icons/connectors/outlook.svg")
...
class OutlookConnector(BaseConnector):
    ...
    async def init(self) -> bool:
        ...
        try:
            ...
            external_client: ExternalMSGraphClient = (
                ExternalMSGraphClient.build_with_config(
                    MSGraphClientWithClientIdSecretConfig(
            ...
            self.external_outlook_client = OutlookCalendarContactsDataSource(
                external_client
            )
    ...
    async def test_connection_and_access(self) -> bool:
        ...
        try:
            if (
                not self.external_outlook_client
                or not self.external_users_client
            ...
            if not (
                ...
                and self.credentials.client_secret
            ):
                return False

            try:
                # Get just 1 user with minimal fields to test connection
                response: UsersGroupsResponse = (
                    await self.external_users_client.users_user_list_user(
                        top=1,
                        select=["id"],
                    )
                )
    ...
    async def _populate_user_cache(self) -> None:
        ...
        try:
            ...
            if (
                self._user_cache_timestamp
                and current_time - self._user_cache_timestamp < self._user_cache_ttl
    ...
    async def run_sync(self) -> None:
        ...
        try:
            ...
            if not self.external_outlook_client or not self.external_users_client:
                raise Exception(
                    "External API clients not initialized. Call init() first."
                )
    ...
    async def _get_all_users_external(self) -> list[AppUser]:
        ...
        try:
            ...
            for user in user_data:
                ...
                full_name = (
                    display_name if display_name else f"{given_name} {surname}".strip()
                )
                if not full_name:
                    full_name = (
                        self._safe_get_attr(user, "mail")
                        or self._safe_get_attr(user, "user_principal_name")
                        or "Unknown User"
                    )
                ...
                app_user = AppUser(
                    ...
                    email=self._safe_get_attr(user, "mail")
                    or self._safe_get_attr(user, "user_principal_name"),
                    full_name=full_name,
                )
    ...
    async def _process_users(
        self, org_id: str, users: list[AppUser]
    ...
    async def _process_user_emails(self, org_id: str, user: AppUser) -> str:
        ...
        try:
            ...
            for folder in folders:
                folder_name = self._safe_get_attr(
                    folder, "display_name", "Unnamed Folder"
                )
                try:
                    (
                        result,
                        folder_mail_records,
                    ) = await self._process_single_folder_messages(org_id, user, folder)
            ...
            try:
                thread_edges_created = await self._create_all_thread_edges_for_user(
                    org_id, user, all_mail_records
                )
                if thread_edges_created > 0:
                    self.logger.info(
                        f"Created {thread_edges_created} thread edges for user {user.email}"
                    )
            except Exception as e:
                self.logger.error(
                    f"Error creating thread edges for user {user.email}: {e}"
                )
            ...
            self.logger.error(
                f"Error processing all folders for user {user.email}: {e}"
            )
            return f"Failed to process folders for {user.email}: {e!s}"

    async def _find_parent_by_conversation_index_from_db(
        self, conversation_index: str, thread_id: str, org_id: str, user: AppUser
    ) -> str | None:
        ...
        try:
            ...
            self.logger.debug(
                f"Thread {thread_id}: Looking for parent with conversation_index={parent_index}"
            )
            ...
            self.logger.error(
                f"Error finding parent by conversation index from DB for thread {thread_id}: {e}"
            )
            return None

    async def _create_all_thread_edges_for_user(
        self, org_id: str, user: AppUser, user_mail_records: list[Record]
    ) -> int:
        ...
        try:
            ...
            for record in user_mail_records:
                if (
                    hasattr(record, "conversation_index")
                    and record.conversation_index
                    and hasattr(record, "thread_id")
                ...
                try:
                    async with self.data_store_provider.transaction() as tx_store:
                        await tx_store.batch_create_edges(
                            edges, collection=CollectionNames.RECORD_RELATIONS.value
                        )
                except Exception as e:
                    self.logger.error(
                        f"Error creating thread edges batch for user {user.email}: {e}"
                    )
                    processed_count = 0
            ...
            self.logger.error(
                f"Error creating all thread edges for user {user.email}: {e}"
            )
            return 0
    ...
    async def _get_all_folders_for_user(self, user_id: str) -> list[dict]:
        ...
        try:
            ...
            response: OutlookMailFoldersResponse = (
                await self.external_outlook_client.users_list_mail_folders(
                    user_id=user_id,
                )
            )
    ...
    async def _process_single_folder_messages(
        self, org_id: str, user: AppUser, folder: dict
        ...
        try:
            ...
            sync_point_key = generate_record_sync_point_key(
                ...
                "folders",
                f"{user_id}_{folder_id}",
            )
            sync_point = await self.email_delta_sync_point.read_sync_point(
                sync_point_key
            )
            delta_link = sync_point.get("delta_link") if sync_point else None

            # Get messages for this folder using delta sync
            result = await self._get_all_messages_delta_external(
                user_id, folder_id, delta_link
            )
            messages = result["messages"]

            self.logger.info(
                f"Retrieved {len(messages)} total message changes from folder '{folder_name}' for user {user.email}"
            )
            ...
            for message in messages:
                record_updates = await self._process_single_message(
                    org_id, user, message, folder_id, folder_name
                )
            ...
            for update in all_updates:
                if update and update.record:
                    ...
                    if (
                        hasattr(update.record, "record_type")
                        and update.record.record_type == RecordType.MAIL
                    ):
            ...
            await self.email_delta_sync_point.update_sync_point(
                sync_point_key, sync_point_data
            )

            # Log final summary
            self.logger.info(
                f"Folder '{folder_name}' completed: {processed_count} records processed from {len(messages)} messages"
            )
            ...
            self.logger.error(
                f"Error processing messages in folder '{folder_name}' for user {user.email}: {e}"
            )
            return 0, []

    async def _get_all_messages_delta_external(
        self, user_id: str, folder_id: str, delta_link: str | None = None
    ) -> dict:
        ...
        try:
            ...
            (
                messages,
                new_delta_link,
            ) = await self.external_outlook_client.fetch_all_messages_delta(
                user_id=user_id,
            ...
            self.logger.info(
                f"Delta sync completed for folder {folder_id}: retrieved {len(messages)} total messages across all pages"
            )
            ...
            self.logger.error(
                f"Error getting messages delta for folder {folder_id}: {e}",
                exc_info=True,
            )
            return {"messages": [], "delta_link": None, "next_link": None}

    async def _process_single_message(
        self, org_id: str, user: AppUser, message, folder_id: str, folder_name: str
    ) -> list[RecordUpdate]:
        ...
        try:
            ...
            if is_deleted:
                self.logger.info(
                    f"Deleting message: {message_id} and its attachments from folder {folder_name}"
                )
                async with self.data_store_provider.transaction() as tx_store:
                    await tx_store.delete_record_by_external_id(
                        Connectors.OUTLOOK, message_id, user.user_id
                    )
                return updates
            ...
            if email_update:
                ...
                if has_attachments:
                    email_permissions = await self._extract_email_permissions(
                        message, None, user
                    )
                    attachment_updates = (
                        await self._process_email_attachments_with_folder(
                            org_id,
                            user,
                            message,
                            email_permissions,
                ...
                self.logger.debug(
                    f"Skipping attachment processing for unchanged email {message_id}"
                )

        except Exception as e:
            self.logger.error(
                f"Error processing message {self._safe_get_attr(message, 'id', 'unknown')}: {e}"
            )
        ...
        try:
            ...
            if not is_new:
                ...
                if existing_folder_id and current_folder_id != existing_folder_id:
                    ...
                    is_updated = True
                    self.logger.info(
                        f"Email {message_id} moved from folder {existing_folder_id} to {current_folder_id}"
                    )
            ...
            email_record = MailRecord(
                ...
                source_created_at=self._parse_datetime(
                    self._safe_get_attr(message, "created_date_time")
                ),
                source_updated_at=self._parse_datetime(
                    self._safe_get_attr(message, "last_modified_date_time")
                ),
                ...
                or "No Subject",
                from_email=self._extract_email_from_recipient(
                    self._safe_get_attr(message, "from_", None)
                ),
                to_emails=[
                    self._extract_email_from_recipient(r)
                    for r in self._safe_get_attr(message, "to_recipients", [])
                ],
                cc_emails=[
                    self._extract_email_from_recipient(r)
                    for r in self._safe_get_attr(message, "cc_recipients", [])
                ],
                bcc_emails=[
                    self._extract_email_from_recipient(r)
                    for r in self._safe_get_attr(message, "bcc_recipients", [])
                ],
                thread_id=self._safe_get_attr(message, "conversation_id", ""),
                is_parent=False,
                internet_message_id=self._safe_get_attr(
                    message, "internet_message_id", ""
                ),
                conversation_index=self._safe_get_attr(
                    message, "conversation_index", ""
                ),
            )

            permissions = await self._extract_email_permissions(
                message, email_record.id, user
            )
            ...
            self.logger.error(
                f"Error processing email {self._safe_get_attr(message, 'id', 'unknown')}: {e!s}"
            )
            return None

    async def _extract_email_permissions(
        self, message: dict, record_id: str | None, inbox_owner: AppUser
    ) -> list[Permission]:
        ...
        try:
            ...
            for recipient in all_recipients:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to extract email from recipient {recipient}: {e}"
                    )
                    continue
    ...
    async def _process_email_attachments_with_folder(
        ...
        try:
            ...
            attachments = await self._get_message_attachments_external(
                user_id, message_id
            )
            ...
            for i, attachment in enumerate(attachments):
                ...
                attachment_record_id = (
                    existing_record.id if existing_record else str(uuid.uuid4())
                )
                ...
                attachment_record = FileRecord(
                    ...
                    connector_name=Connectors.OUTLOOK,
                    source_created_at=self._parse_datetime(
                        self._safe_get_attr(attachment, "last_modified_date_time")
                    ),
                    source_updated_at=self._parse_datetime(
                        self._safe_get_attr(attachment, "last_modified_date_time")
                    ),
            ...
            self.logger.error(
                f"Error processing attachments for email {self._safe_get_attr(message, 'id', 'unknown')}: {e}"
            )
            return []

    async def _get_message_attachments_external(
        ...
        try:
            ...
            response: OutlookCalendarContactsResponse = (
                await self.external_outlook_client.users_messages_list_attachments(
                    user_id=user_id,
            ...
            if not response.success:
                self.logger.error(
                    f"Failed to get attachments for message {message_id}: {response.error}"
                )
                return []
    ...
    async def _get_existing_record(
        self, org_id: str, external_record_id: str
    ) -> Record | None:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting existing record {external_record_id}: {e}"
            )
            return None
    ...
    async def stream_record(self, record: Record) -> StreamingResponse:
        ...
        try:
            if not self.external_outlook_client:
                raise HTTPException(
                    status_code=500, detail="External Outlook client not initialized"
                )
            ...
            async with self.data_store_provider.transaction() as tx_store:
                user_email = await tx_store.get_record_owner_source_user_email(
                    record.id
                )
                ...
                if user_id:
                    self.logger.debug(
                        f"Found Graph user ID {user_id} for record {record.id} from permission edges"
                    )
                else:
                    self.logger.warning(
                        f"Could not find owner for record {record.id} in permission edges"
                    )

            if not user_id:
                raise HTTPException(
                    status_code=400,
                    detail="Could not determine user context for this record.",
                )

            if record.record_type == RecordType.MAIL:
                message = await self._get_message_by_id_external(
                    user_id, record.external_record_id
                )
                # Extract email body content from ItemBody object
                body_obj = self._safe_get_attr(message, "body")
                email_body = (
                    self._safe_get_attr(body_obj, "content", "") if body_obj else ""
                )
            ...
            if record.record_type == RecordType.FILE:
                ...
                if not parent_message_id:
                    raise HTTPException(
                        status_code=404,
                        detail="No parent message ID stored for attachment",
                    )

                attachment_data = await self._download_attachment_external(
                    user_id, parent_message_id, attachment_id
                )
                ...
                return StreamingResponse(
                    generate_attachment(), media_type=media_type, headers=headers
                )

            raise HTTPException(
                status_code=400, detail="Unsupported record type for streaming"
            )
    ...
    async def _get_message_by_id_external(self, user_id: str, message_id: str) -> dict:
        ...
        try:
            ...
            response: OutlookCalendarContactsResponse = (
                await self.external_outlook_client.users_get_messages(
                    user_id=user_id,
                    message_id=message_id,
                )
            )
            ...
            if not response.success:
                self.logger.error(
                    f"Failed to get message {message_id}: {response.error}"
                )
                return {}
    ...
    async def _download_attachment_external(
        ...
        try:
            ...
            response: OutlookCalendarContactsResponse = (
                await self.external_outlook_client.users_messages_get_attachments(
                    user_id=user_id,
                    message_id=message_id,
            ...
            content_bytes = self._safe_get_attr(
                attachment_data, "content_bytes"
            ) or self._safe_get_attr(attachment_data, "contentBytes")
            ...
            self.logger.error(
                f"Error downloading attachment {attachment_id} for message {message_id}: {e}"
            )
            return b""
    ...
    def _extract_email_from_recipient(self, recipient) -> str:
        ...
        email_addr = self._safe_get_attr(
            recipient, "email_address"
        ) or self._safe_get_attr(recipient, "emailAddress")
        if email_addr:
