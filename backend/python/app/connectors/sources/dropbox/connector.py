...
@(
    ...
    .configure(
        lambda builder: builder.with_icon("/assets/icons/connectors/dropbox.svg")
...
class DropboxConnector(BaseConnector):
    ...
    async def init(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to initialize Dropbox client: {e}", exc_info=True
            )
            return False
    ...
    async def _process_dropbox_entry(
        ...
        try:
            ...
            shared_link_result = (
                await self.data_source.sharing_create_shared_link_with_settings(
                    path=entry.path_lower,
                    team_member_id=user_id,
                    team_folder_id=record_group_id if not is_person_folder else None,
            ...
            if shared_link_result.success:
                ...
                if "shared_link_already_exists" in error_str:
                    self.logger.info(
                        "Link already exists, making second call to retrieve it"
                    )
                    ...
                    if second_result.success:
                        ...
                        if "shared_link_already_exists" in second_error_str:
                            ...
                            if url_match:
                                ...
                                self.logger.info(
                                    "Successfully extracted URL from error: %s",
                                    preview_url,
                                )
                            else:
                                self.logger.error(
                                    "Could not extract URL from second error string"
                                )
                                self.logger.debug(
                                    "Error string: %s", second_error_str[:500]
                                )  # Log first 500 chars
                        else:
                            self.logger.error(
                                "Unexpected error on second call (not shared_link_already_exists)"
                            )
                else:
                    self.logger.error(
                        "Unexpected error type on first call (not shared_link_already_exists)"
                    )

            # Final check
            if preview_url is None:
                self.logger.error(
                    "Failed to retrieve preview URL for %s", entry.path_lower
                )
            else:
            ...
            file_record = FileRecord(
                ...
                if is_file and hasattr(entry, "content_hash")
                else None,
            )
            ...
            try:
                ...
                new_permissions = (
                    await self._convert_dropbox_permissions_to_permissions(
                ...
                self.logger.warning(
                    f"Could not fetch permissions for {entry.name}: {perm_ex}"
                )
            ...
            self.logger.error(
                f"Error processing Dropbox entry {getattr(entry, 'id', entry.path_lower)}: {ex}",
                exc_info=True,
            )
            return None
    ...
    async def _process_dropbox_items_generator(
        ...
        for entry in entries:
            try:
                ...
                self.logger.error(
                    f"Error processing item in generator: {e}", exc_info=True
                )
                continue
    ...
    async def _convert_dropbox_permissions_to_permissions(
        ...
        try:
            ...
            if not members_result.success:
                self.logger.debug(
                    f"Could not fetch permissions for {file_or_folder_id}: {members_result.error}"
                )
                return []
            ...
            if hasattr(members_result.data, "users") and members_result.data.users:
                for user_membership in members_result.data.users:
                    ...
                    permission_type = access_level_map.get(
                        access_type_tag, PermissionType.READ
                    )
                    ...
                    if not email or email.endswith("#"):
                        self.logger.debug(
                            f"Skipping user {user_info.account_id} with invalid email: {email}"
                        )
                        continue
            ...
            self.logger.debug(
                f"Error converting Dropbox permissions for {file_or_folder_id}: {e}"
            )
    ...
    def _permissions_equal(
        self, old_perms: list[Permission], new_perms: list[Permission]
    ) -> bool:
        ...
        old_set = {
            (p.external_id, p.type.value, p.entity_type.value) for p in old_perms
        }
        new_set = {
            (p.external_id, p.type.value, p.entity_type.value) for p in new_perms
        }
    ...
    async def _process_entry(
        ...
        if isinstance(entry, DeletedMetadata):
            ...
            self.logger.info(
                f"Item '{entry.name}' at path '{entry.path_lower}' was deleted. Deletion requires path-based lookup."
            )
        ...
        file_record = FileRecord(
            ...
            origin=OriginTypes.CONNECTOR.value,
            connector_name=self.connector_name,
            updated_at=int(entry.server_modified.timestamp() * 1000)
            if is_file
            else None,
            source_updated_at=int(entry.server_modified.timestamp() * 1000)
            if is_file
            else None,
    ...
    async def _get_permissions(
        self,
        entry: FileMetadata | FolderMetadata,
    ) -> list[Permission]:
        ...
        try:
            ...
            for member in all_members:
                ...
                perm_type = (
                    ...
                    if access_type in (AccessLevel.owner, AccessLevel.editor)
                ...
                entity_type = (
                    EntityType.USER
                    if hasattr(member_info, "account_id")
                    else EntityType.GROUP
                )
                external_id = (
                    member_info.account_id
                    if entity_type == EntityType.USER
                    else member_info.group_id
                )
    ...
    async def _sync_from_source(
        ...
        while has_more:
            try:
                if cursor:
                    response = await self.data_source.files_list_folder_continue(
                        cursor=cursor
                    )
                else:
                    response = await self.data_source.files_list_folder(
                ...
                await self.record_sync_point.update_sync_point(
                    sync_point_key, {"cursor": cursor}
                )

            except Exception as e:
                self.logger.error(
                    f"Error during Dropbox folder sync: {e}", exc_info=True
                )
                has_more = False
    ...
    async def _process_users_in_batches(self, users: list[User]) -> None:
        ...
        try:
            ...
            active_user_emails = {
                active_user.email.lower() for active_user in all_active_users
            }
            ...
            self.logger.info(
                f"Processing {len(users_to_sync)} active users out of {len(users)} total users"
            )
    ...
    async def _run_sync_with_yield(self, user_id: str, user_email: str) -> None:
        ...
        try:
            ...
            shared_folders = await self.data_source.sharing_list_folders(
                team_member_id=user_id
            )
            ...
            if shared_folders.success:
                self.logger.info(
                    f"Found {len(shared_folders.data.entries)} shared folders for user {user_email}"
                )
                for folder in shared_folders.data.entries:
                    self.logger.info(
                        f"  - Will sync Folder: {folder.name}, ID: {folder.shared_folder_id}"
                    )
                    folders_to_sync.append(folder.shared_folder_id)
            else:
                self.logger.warning(
                    f"Could not list shared folders for user {user_email}: {shared_folders.error}"
                )
            ...
            for folder_id in folders_to_sync:
                ...
                if folder_id is None:
                    ...
                    sync_context_id = (
                        f"{user_id}_{folder_id}"  # Safer key than using '/'
                    )
                ...
                sync_point_key = generate_record_sync_point_key(
                    RecordType.DRIVE.value, sync_group, sync_context_id
                )
                sync_point = await self.dropbox_cursor_sync_point.read_sync_point(
                    sync_point_key
                )
                ...
                while has_more:
                    ...
                    try:
                        async with self.rate_limiter:
                            if cursor:
                                self.logger.info(
                                    f"[{sync_log_name}] Calling files_list_folder_continue..."
                                )
                                result = (
                                    await self.data_source.files_list_folder_continue(
                                        cursor,
                                        team_member_id=user_id,
                        ...
                        if not result.success:
                            self.logger.error(
                                f"[{sync_log_name}] Dropbox API call failed: {result.error}"
                            )
                        ...
                        self.logger.info(
                            f"[{sync_log_name}] Got {len(result.data.entries)} entries. Has_more: {result.data.has_more}"
                        )
                        ...
                        async for (
                            ...
                            permissions,
                            record_update,
                        ) in self._process_dropbox_items_generator(
                            entries,
                            user_id,
                            user_email,
                            ...
                            if file_record:
                                ...
                                if batch_count >= self.batch_size:
                                    self.logger.info(
                                        f"[{sync_log_name}] Processing batch of {batch_count} records."
                                    )
                        ...
                        if batch_records:
                            self.logger.info(
                                f"[{sync_log_name}] Processing final batch of {len(batch_records)} records."
                            )
                            await self.data_entities_processor.on_new_records(
                                batch_records
                            )
                        ...
                        cursor = result.data.cursor
                        self.logger.info(
                            f"[{sync_log_name}] Storing new cursor for key {sync_point_key}"
                        )
                        ...
                        self.logger.error(
                            f"Dropbox API Error during sync for {sync_log_name}: {api_ex}"
                        )
                        # If path not found, stop this folder's sync and continue to the next
                        if "path/not_found" in str(api_ex):
                            self.logger.warning(
                                f"[{sync_log_name}] Path not found. Stopping sync for this folder."
                            )
                        ...
                        self.logger.error(
                            f"Error in 'while has_more' loop for {sync_log_name}: {loop_ex}",
                            exc_info=True,
                        )
            ...
            self.logger.error(
                f"Dropbox API Error during sync setup for user {user_id}: {ex}"
            )
            raise
        except Exception as ex:
            self.logger.error(
                f"Unhandled error in Dropbox sync for user {user_id}: {ex}",
                exc_info=True,
            )
            raise
    ...
    async def run_sync(self) -> None:
        ...
        try:
            ...
            member_sync_key = generate_record_sync_point_key(
                "member_events", "team_events", "global"
            )
            member_sync_point = await self.dropbox_cursor_sync_point.read_sync_point(
                member_sync_key
            )

            if not member_sync_point.get("cursor"):
                self.logger.info("Initializing cursor for member events...")
                await self._initialize_event_cursor(
                    member_sync_key, EventCategory.members
                )
            ...
            group_sync_key = generate_record_sync_point_key(
                "user_group_events", "team_events", "global"
            )
            group_sync_point = await self.dropbox_cursor_sync_point.read_sync_point(
                group_sync_key
            )
            ...
            if not group_sync_point.get("cursor"):
                ...
                await self._initialize_event_cursor(
                    group_sync_key, EventCategory.groups
                )
            ...
            record_group_sync_key = generate_record_sync_point_key(
                "record_group_events", "team_events", "global"
            )
            record_group_sync_point = (
                await self.dropbox_cursor_sync_point.read_sync_point(
                    record_group_sync_key
                )
            )
            ...
            if not record_group_sync_point.get("cursor"):
                ...
                await self._initialize_event_cursor(
                    record_group_sync_key, EventCategory.team_folders
                )
            ...
            sharing_sync_key = generate_record_sync_point_key(
                "sharing_events", "team_events", "global"
            )
            sharing_sync_point = await self.dropbox_cursor_sync_point.read_sync_point(
                sharing_sync_key
            )
            ...
            if not sharing_sync_point.get("cursor"):
                ...
                await self._initialize_event_cursor(
                    sharing_sync_key, EventCategory.sharing
                )
            else:
                self.logger.info("Running an INCREMENTAL sync for sharing events...")
                await self._sync_sharing_changes_with_cursor(
                    sharing_sync_key, sharing_sync_point.get("cursor")
                )
    ...
    async def _initialize_event_cursor(self, sync_key: str, category) -> None:
        ...
        try:
            ...
            if (
                response.success
                and hasattr(response.data, "cursor")
                and response.data.cursor
            ):
                await self.dropbox_cursor_sync_point.update_sync_point(
                    sync_key,
                ...
                self.logger.info(
                    f"✓ Initialized cursor for {category} from {current_time}"
                )
            else:
                self.logger.warning(
                    f"Could not initialize cursor for {category}: {response.error if not response.success else 'No cursor returned'}"
                )

        except Exception as e:
            self.logger.error(
                f"Could not initialize event cursor for {category}: {e}", exc_info=True
            )
        ...
        try:
            ...
            while has_more:
                try:
                    ...
                    async with self.rate_limiter:
                        response = await self.data_source.team_log_get_events_continue(
                            cursor
                        )

                    if not response.success:
                        self.logger.error(
                            f"⚠️ Error fetching team member event log: {response.error}"
                        )
                        break

                    events = response.data.events
                    self.logger.info(
                        f"Processing {len(events)} new member-related events."
                    )
                    ...
                    for event in events:
                        try:
                            ...
                            self.logger.error(
                                f"Error processing member event: {e}", exc_info=True
                            )
                            continue
                    ...
                    self.logger.error(
                        f"⚠️ Error in member sync loop: {e}", exc_info=True
                    )
                    has_more = False
            ...
            if latest_cursor_to_save:
                self.logger.info(
                    f"Storing latest member sync cursor for key {sync_point_key}"
                )
                await self.dropbox_cursor_sync_point.update_sync_point(
                    sync_point_key,
            ...
            self.logger.info(
                f"Incremental member sync completed. Processed {events_processed} events."
            )

        except Exception as e:
            self.logger.error(
                f"⚠️ Fatal error in incremental member sync: {e}", exc_info=True
            )
            raise
    ...
    async def _handle_member_change_status_event(
        self, event, app_users: list[AppUser]
    ) -> None:
        ...
        if hasattr(event.details, "get_member_change_status_details"):
            ...
            new_status = (
                status_details.new_value._tag if status_details.new_value else None
            )
            previous_status = (
                status_details.previous_value._tag
                if status_details.previous_value
                else None
            )
        ...
        try:
            ...
            if new_status == "active":
                self.logger.info(
                    f"Adding team member '{user_name}' ({user_email}, ID: {team_member_id})"
                )
            ...
            # If new status is 'removed', treat as member removed
                ...
                self.logger.info(
                    f"Removing team member '{user_name}' ({user_email}, ID: {team_member_id})"
                )
    ...
    async def _handle_member_added(
        self, user_email: str, team_member_id: str, app_users: list[AppUser]
    ) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error processing member addition for {user_email}: {e}", exc_info=True
            )
        ...
        try:
            ...
            try:
                ...
                if not groups_response.success:
                    raise Exception(
                        f"Error fetching groups list: {groups_response.error}"
                    )
                ...
                while has_more:
                    ...
                    groups_response = await self.data_source.team_groups_list_continue(
                        cursor
                    )
                    if not groups_response.success:
                        self.logger.error(
                            f"Error fetching more groups: {groups_response.error}"
                        )
                ...
                self.logger.error(
                    f"❌ Failed to fetch full group list: {e}", exc_info=True
                )
                raise  # Stop the sync if we can't get the groups

            self.logger.info(
                f"Found {len(all_groups_list)} total groups. Now processing members."
            )
            ...
            for group in all_groups_list:
                try:
                    all_members = await self._fetch_group_members(
                        group.group_id, group.group_name
                    )

                    processor_group, member_permissions = (
                        self._create_user_group_with_permissions(
                            group.group_id,
                            group.group_name,
                            all_members,
                        )
                    ...
                    self.logger.error(
                        f"❌ Failed to process group {group.group_name}: {e}",
                        exc_info=True,
                    )
            ...
            if user_groups_batch:
                self.logger.info(
                    f"Submitting {len(user_groups_batch)} user groups to the processor..."
                )
            ...
            self.logger.error(
                f"❌ Fatal error in _sync_dropbox_groups: {e}", exc_info=True
            )
            raise
    ...
    async def _sync_group_changes_with_cursor(self) -> None:
        ...
        try:
            ...
            if not cursor:
                self.logger.warning(
                    "No cursor found for incremental group sync. Running full sync instead."
                )
                await self._sync_user_groups()
                return
            ...
            while has_more:
                try:
                    ...
                    if not response.success:
                        self.logger.error(
                            f"⚠️ Error fetching team event log: {response.error}"
                        )
                        break

                    events = response.data.events
                    self.logger.info(
                        f"Processing {len(events)} new group-related events."
                    )
                    ...
                    for event in events:
                        try:
                            ...
                            self.logger.error(
                                f"Error processing group event: {e}", exc_info=True
                            )
                            continue
            ...
            if latest_cursor_to_save:
                self.logger.info(
                    f"Storing latest group sync cursor for key {sync_point_key}"
                )
                await self.dropbox_cursor_sync_point.update_sync_point(
                    sync_point_key,
                    sync_point_data={"cursor": latest_cursor_to_save},
            ...
            self.logger.info(
                f"Incremental group sync completed. Processed {events_processed} events."
            )

        except Exception as e:
            self.logger.error(
                f"⚠️ Fatal error in incremental group sync: {e}", exc_info=True
            )
            raise
    ...
    async def _process_group_event(self, event) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error processing group event of type {getattr(event, 'event_type', 'unknown')}: {e}",
                exc_info=True,
            )
    ...
    async def _handle_group_membership_event(self, event, event_type: str) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to parse participants for event {event_type}: {e}",
                exc_info=True,
            )
            return
        ...
        if not group_id or not member_email:
            self.logger.warning(
                f"Could not extract required group_id or member_email from {event_type} event. Skipping."
            )
            return
        ...
        if event_type == "group_add_member":
            self.logger.info(
                f"Adding member '{member_name}' ({member_email}) to group '{group_name}' ({group_id})"
            )

            # Determine permission type (specific to 'add' events)
            permission_type = PermissionType.WRITE  # Default permission
            if (
                hasattr(event.details, "is_group_owner")
                and event.details.is_group_owner
            ):
            ...
            self.logger.info(
                f"Removing member '{member_name}' ({member_email}) from group '{group_name}' ({group_id})"
            )
    ...
    async def _handle_group_created_event(self, event) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error processing group_create event for group {group_id}: {e}",
                exc_info=True,
            )
    ...
    async def _process_single_group(self, group_id: str, group_name: str) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Submitting newly created group {group_name} to processor..."
            )
            await self.data_entities_processor.on_new_user_groups(user_groups_batch)
            self.logger.info(
                f"Successfully processed group_create event for {group_name}"
            )

        except Exception as e:
            self.logger.error(
                f"Failed to process single group {group_name} ({group_id}): {e}",
                exc_info=True,
            )
    ...
    async def _fetch_group_members(self, group_id: str, group_name: str) -> list:
        ...
        members_response = await self.data_source.team_groups_members_list(
            group=group_id
        )
        if not members_response.success:
            raise Exception(
                f"Error fetching members for group {group_name}: {members_response.error}"
            )
        ...
        while member_has_more:
            ...
            members_response = await self.data_source.team_groups_members_list_continue(
                member_cursor
            )

            if not members_response.success:
                self.logger.error(
                    f"Error during member pagination for {group_name}: {members_response.error}"
                )
                break
    ...
    def _create_user_group_with_permissions(
        self, group_id: str, group_name: str, all_members: list
    ) -> tuple:
    ...
    async def _handle_group_renamed_event(self, event) -> None:
        ...
        for participant in event.participants:
            if participant.is_group():
                ...
                )  # This should have the updated name
                break
        ...
        try:
            ...
            self.logger.error(
                f"Error processing group_rename event for group {group_id}: {e}",
                exc_info=True,
            )

    async def _update_group_name(
        self, group_id: str, new_name: str, old_name: str = None
    ) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to update group name for {group_id}: {e}", exc_info=True
            )
            raise
    ...
    async def _handle_group_change_member_role_event(self, event) -> None:
        ...
        try:
            if hasattr(event.details, "get_group_change_member_role_details"):
                ...
                self.logger.warning(
                    f"Could not extract role details from event: {event.details}"
                )
        except Exception as e:
        ...
        new_permission_type = (
            PermissionType.OWNER if new_is_owner else PermissionType.WRITE
        )
    ...
    def _extract_folder_info_from_event(self, event) -> tuple[str | None, str | None]:
        ...
        for asset in event.assets:
            if asset.is_folder():
                ...
                if (
                    folder_info.path
                    and folder_info.path.namespace_relative
                    and folder_info.path.namespace_relative.ns_id
                ):
    ...
    async def _create_and_sync_single_record_group(
        ...
        try:
            ...
            while has_more:
                self.logger.debug(
                    f"Fetching more members for folder '{folder_name}'..."
                )

                members_continue = (
                    await self.data_source.sharing_list_folder_members_continue(
                        cursor=cursor,
                        team_member_id=team_admin_user.source_user_id,
                        as_admin=True,
                    )
            ...
            for user_info in all_users_list:
                ...
                permission_type = dropbox_to_permission_type.get(
                    access_level_tag, PermissionType.READ
                )
            ...
            await self.data_entities_processor.on_new_record_groups(
                [(record_group, permissions_list)]
            )
            self.logger.info(
                f"Successfully synced record group '{folder_name}' ({folder_id})"
            )
    ...
    async def sync_record_groups(self, users: list[AppUser]) -> None:
        ...
        self.logger.info(
            f"Using team admin user: {team_admin_user.email} (ID: {team_admin_user.source_user_id})"
        )
        ...
        if not team_folders_response.success:
            self.logger.error(
                f"Failed to fetch team folders: {team_folders_response.error}"
            )
            return
        ...
        while has_more:
            ...
            folders_continue = await self.data_source.team_team_folder_list_continue(
                cursor=cursor
            )

            if not folders_continue.success:
                self.logger.error(
                    f"Error during team folder pagination: {folders_continue.error}"
                )
                break
        ...
        for folder in all_team_folders:
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to sync folder '{folder.name}': {e}", exc_info=True
                )
                continue
    ...
    async def _sync_record_group_changes_with_cursor(
        ...
        try:
            ...
            if not cursor:
                self.logger.warning(
                    "No cursor found for incremental record group sync."
                )
                # Initialize cursor for team folder events
                await self._initialize_event_cursor(
                    sync_point_key, EventCategory.team_folders
                )
                return
            ...
            while has_more:
                try:
                    ...
                    if not response.success:
                        self.logger.error(
                            f"Error fetching team folder event log: {response.error}"
                        )
                        break
                    ...
                    for event in events:
                        try:
                            ...
                            self.logger.error(
                                f"Error processing record group event: {e}",
                                exc_info=True,
                            )
                            continue
                    ...
                    self.logger.error(
                        f"Error in record group sync loop: {e}", exc_info=True
                    )
                    has_more = False
            ...
            self.logger.error(
                f"Fatal error in incremental record group sync: {e}", exc_info=True
            )
            raise
    ...
    async def _process_record_group_event(self, event, users) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error processing record group event of type {getattr(event, 'event_type', 'unknown')}: {e}",
                exc_info=True,
            )

    async def _handle_record_group_created_event(
        self, event, users: list[AppUser]
    ) -> None:
        ...
        self.logger.info(
            f"Creating record group for team folder '{folder_name}' (ID: {folder_id})"
        )
        ...
        if not team_admin_user:
            self.logger.error(
                "No team admin user found. Cannot sync newly created folder."
            )
            return

        try:
            await self._create_and_sync_single_record_group(
                folder_id, folder_name, team_admin_user
            )
            self.logger.info(
                f"Successfully processed team_folder_create event for '{folder_name}'"
            )
        except Exception as e:
            self.logger.error(
    ...
    async def _handle_record_group_renamed_event(self, event) -> None:
        ...
        self.logger.info(
            f"Renaming record group {folder_id} from '{old_name}' to '{new_name}'"
        )

        try:
            await self.data_entities_processor.update_record_group_name(
                folder_id, new_name, old_name, self.connector_name
            )
        except Exception as e:
            self.logger.error(
    ...
    async def _handle_record_group_deleted_event(self, event) -> None:
        ...
        if not folder_id:
            self.logger.warning(
                "Could not extract folder_id from team_folder_permanently_delete event"
            )
            return
    ...
    async def _handle_record_group_status_changed_event(
        self, event, users: list[AppUser]
    ) -> None:
    ...
    async def _sync_sharing_changes_with_cursor(
        self, sync_point_key: str, cursor: str
    ) -> None:
        ...
        while has_more:
            try:
                ...
                if not response.success:
                    self.logger.error(
                        f"⚠️ Error fetching sharing event log: {response.error}"
                    )
                    break
                ...
                for event in response.data.events:
                    try:
                        ...
                        if (
                            ...
                            self.logger.info(
                                f"Processing permission change event: {event_type}"
                            )

                            # 2. Get the actor's information
                            actor = (
                                event.actor.get_admin()
                                if event.actor.is_admin()
                                else event.actor.get_user()
                            )
                            if not actor:
                                self.logger.warning(
                                    "Could not determine actor for event. Skipping."
                                )
                                continue
                            ...
                            if not event.assets:
                                self.logger.warning(
                                    f"Event {event_type} has no assets. Skipping."
                                )
                                continue
                            ...
                            for asset in event.assets:
                                if asset.is_file():
                                    ...
                                    if (
                                        file_info.path
                                        and file_info.path.namespace_relative
                                    ):
                                        ns_id_for_file = (
                                            file_info.path.namespace_relative.ns_id
                                        )
                        ...
                        self.logger.error(
                            f"Error processing a single sharing event: {e}",
                            exc_info=True,
                        )
                ...
                self.logger.error(
                    f"Fatal error in sharing sync loop: {e}", exc_info=True
                )
                has_more = False
    ...
    async def _resync_record_by_external_id(
        ...
        """
        self.logger.info(
            f"Re-syncing record (is_folder={is_folder}) due to a permission event: {external_id}"
        )
        try:
            ...
            if not is_folder:  # It's a File
                ...
                self.logger.info(
                    f"Fetching FolderMetadata with files/get_metadata for ns:{external_id}"
                )
                ...
                if not ns_metadata_result or not ns_metadata_result.success:
                    self.logger.error(
                        f"Could not fetch ns: metadata for re-sync: {external_id}. Error: {ns_metadata_result.error if ns_metadata_result else 'No response'}"
                    )
                    return
            ...
            if not metadata_result or not metadata_result.success:
                self.logger.error(
                    f"Could not fetch metadata for re-sync: {external_id}. Error: {metadata_result.error}"
                )
                return
            ...
            if isinstance(entry, FileMetadata):
                async with self.data_store_provider.transaction() as tx_store:
                    existing_record = await tx_store.get_record_by_external_id(
                        self.connector_name, external_id
                    )
                    if not existing_record:
                        self.logger.warning(
                            f"File record {external_id} not found in DB for re-sync. Cannot determine parent group."
                        )
                        return
                ...
                async with self.data_store_provider.transaction() as tx_store:
                    existing_record = await tx_store.get_record_by_external_id(
                        self.connector_name, file_id
                    )
                    if not existing_record:
                        self.logger.warning(
                            f"File record {file_id} not found in DB for re-sync. Cannot determine parent group."
                        )
                        return
                    record_group_id = existing_record.external_record_group_id
                    is_person_folder = record_group_id == team_member_id
            ...
            if record_update and record_update.is_updated:
                self.logger.info(
                    f"Permissions updated for {record_update.record.record_name}"
                )
                await self._handle_record_updates(record_update)
            else:
                self.logger.info(
                    f"Re-sync for {external_id} did not result in an update."
                )

        except Exception as e:
            self.logger.error(
                f"Error during single record re-sync for {external_id}: {e}",
                exc_info=True,
            )
    ...
    async def run_incremental_sync(self) -> None:
        ...
        sync_point_key = generate_record_sync_point_key(RecordType.DRIVE.value, "root")
        sync_point = await self.record_sync_point.dropbox_cursor_sync_point(
            sync_point_key
        )
        ...
        try:
            ...
            async with self.data_store_provider.transaction() as tx_store:
                user_with_permission = (
                    await tx_store.get_first_user_with_permission_to_node(
            ...
            if not user_with_permission:
                self.logger.warning(
                    f"No user found with permission to node: {record.id}"
                )
                return None
            if not file_record:
            ...
            team_member_info = await self.data_source.team_members_get_info_v2(
                members=members
            )
            team_member_id = (
                team_member_info.data.members_info[0]
                .get_member_info()
            ...
            if (
                ...
                and not record.external_record_group_id.startswith("dbmid:")
            ):
