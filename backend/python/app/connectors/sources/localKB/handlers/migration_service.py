...
class KnowledgeBaseMigrationService:
    ...
    def __init__(self, arango_service: BaseArangoService, logger: Logger) -> None:
        ...
        self.OLD_USER_TO_KB_EDGES = (
            LegacyCollectionNames.PERMISSIONS_TO_KNOWLEDGE_BASE.value
        )  # Direct user→kb permissions
        self.OLD_KB_TO_RECORD_EDGES = (
            LegacyCollectionNames.BELONGS_TO_KNOWLEDGE_BASE.value
        )  # KB→record relationships
    ...
    async def run_migration(self) -> dict:
        ...
        try:
            self.logger.info(
                "🚀 Starting Knowledge Base migration from old to new system"
            )
    ...
    async def _validate_migration_preconditions(self) -> dict:
        ...
        if self.OLD_KB_COLLECTION not in collection_names:
            self.logger.info(
                f"✅ Old KB collection '{self.OLD_KB_COLLECTION}' not found - no migration needed"
            )
            return {
                "success": True,
        ...
        missing_collections = [
            col for col in required_new_collections if col not in collection_names
        ]
    ...
    async def _migrate_data(self, migration_data: dict) -> dict:
        ...
        try:
            ...
            if failed_migrations:
                ...
                self.logger.error(
                    f"❌ {len(failed_migrations)} migrations failed - aborting transaction"
                )
                for failure in failed_migrations:
                    self.logger.error(
                        f"   - KB {failure.get('old_kb_id')}: {failure.get('error')}"
                    )
    ...
    async def _migrate_knowledge_bases(
        self, migration_data: dict, transaction
    ) -> list[dict]:
        ...
        for org_id, org_users in migration_data["org_data"].items():
            for user_id, user_data in org_users.items():
                try:
                    ...
                    user_key = await self._ensure_user_exists(
                        user_id, org_id, transaction
                    )
                    ...
                    for old_kb in user_data["kbs"]:
                        try:
                            ...
                            self.logger.error(
                                f"❌ Failed to migrate KB {old_kb.get('_key')}: {kb_error!s}"
                            )
                    ...
                    self.logger.error(
                        f"❌ Failed to process user {user_id} in org {org_id}: {user_error!s}"
                    )

        self.logger.info(
            f"✅ Migration completed: {len([r for r in migration_results if r.get('success')])} successful, "
            f"{len([r for r in migration_results if not r.get('success')])} failed"
        )

        return migration_results

    async def _migrate_single_kb(
        self,
        old_kb: dict,
        user_key: str,
        org_id: str,
        ...
        self.logger.info(
            f"✅ Successfully migrated KB {old_kb_name}: {migrated_records} records"
        )
    ...
    async def _migrate_kb_records(
        ...
        old_records_for_kb = [
            rel_data["record"]
            for rel_data in user_data["kb_records"]
        ...
        self.logger.info(
            f"📝 Migrated {len(record_edges)} record relationships for KB {old_kb_id}"
        )
    ...
    async def _ensure_user_exists(self, user_id: str, org_id: str, transaction) -> str:
        ...
        self.logger.warning(
            f"⚠️ Creating minimal user record for {user_id} in org {org_id}"
        )
    ...
    async def _create_new_edge_definition(self, graph: object, edge_def: dict) -> None:
        ...
        try:
            ...
            if not self.db.has_collection(edge_collection):
                self.logger.warning(
                    f"⚠️ Edge collection {edge_collection} does not exist, skipping creation"
                )
                return
            ...
            self.logger.error(
                f"❌ Failed to create edge definition {edge_def['edge_collection']}: {e!s}"
            )
    ...
    async def _create_complete_new_graph(self) -> None:
        ...
        try:
            ...
            for edge_def in edge_definitions:
                try:
                    ...
                    if self.db.has_collection(edge_collection):
                        ...
                        created_count += 1
                        self.logger.info(
                            f"✅ Created edge definition for {edge_collection}"
                        )
                    else:
                        self.logger.warning(
                            f"⚠️ Skipping edge definition for non-existent collection: {edge_collection}"
                        )
            ...
            self.logger.info(
                f"✅ Created new graph with {created_count} edge definitions"
            )
    ...
    async def _update_existing_edge_definition(
        self, graph: object, edge_collection: str, new_edge_def: dict
    ) -> None:
        ...
        try:
            ...
            if not self.db.has_collection(edge_collection):
                self.logger.warning(
                    f"⚠️ Edge collection {edge_collection} does not exist, skipping update"
                )
                return

            # Get current definition
            existing_definitions = {
                ed["edge_collection"]: ed for ed in graph.edge_definitions()
            }
            current_def = existing_definitions.get(edge_collection)

            if not current_def:
                self.logger.warning(
                    f"⚠️ Current definition not found for {edge_collection}"
                )
                return
            ...
            self.logger.error(
                f"❌ Failed to update edge definition {edge_collection}: {e!s}"
            )
            raise
    ...
    async def _update_graph_vertices(self) -> None:
        ...
        try:
            ...
            if not self.db.has_graph(self.NEW_GRAPH_NAME):
                self.logger.warning(
                    f"⚠️ Graph {self.NEW_GRAPH_NAME} not found, creating new graph"
                )
            ...
            for edge_def in new_edge_definitions:
                ...
                try:
                    if edge_collection in existing_definitions:
                        ...
                        await self._update_existing_edge_definition(
                            graph, edge_collection, edge_def
                        )
                    else:
                    ...
                    self.logger.error(
                        f"❌ Failed to process edge definition {edge_collection}: {e!s}"
                    )
                    # Continue with other edge definitions
    ...
    async def _update_graph_structure(self) -> None:
        ...
        try:
            ...
            if not self.db.has_graph(self.NEW_GRAPH_NAME):
                ...
                self.logger.info(
                    f"✅ Graph '{self.NEW_GRAPH_NAME}' already exists, ensuring vertices are updated."
                )
                await (
                    self._update_graph_vertices()
                )  # This function correctly updates/adds edge definitions.
        ...
        try:
            ...
            try:
                ...
                await asyncio.to_thread(
                    lambda: cleanup_transaction.commit_transaction()
                )
            ...
            for collection_name in collections_to_drop:
                try:
                    if self.db.has_collection(collection_name):
                        ...
                        self.logger.info(
                            f"🗑️ Successfully dropped collection: '{collection_name}'"
                        )
                    else:
                        self.logger.info(
                    ...
                    self.logger.warning(
                        f"⚠️ Failed to drop collection '{collection_name}': {drop_error!s}"
                    )
            ...
            self.logger.warning(
                "⚠️ Migration succeeded but cleanup failed - manual cleanup may be needed"
            )
    ...
    async def _verify_migration(self, migration_results: list[dict]) -> None:
        ...
        for result in successful_migrations:
            ...
            try:
                kb = await self.arango_service.get_document(
                    new_kb_id, self.NEW_KB_COLLECTION
                )
                if not kb:
        ...
        self.logger.info(
            f"✅ Migration verification completed: {len(successful_migrations)} KBs verified"
        )
    ...
    async def rollback_migration(self, backup_data: dict | None = None) -> dict:
        ...
        self.logger.warning(
            "⚠️ Rollback functionality not implemented - contact support"
        )
    ...
    try:
        ...
        if result["success"]:
            if (
                ...
                and "no migration needed" in result["message"].lower()
            ):
                logger.info(f"✅ KB Migration: {result['message']}")
            else:
                logger.info(
                    f"✅ KB Migration completed successfully: {result['migrated_count']} KBs migrated"
                )
        else:
