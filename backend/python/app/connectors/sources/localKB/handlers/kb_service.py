...
class KnowledgeBaseService:
    ...
    async def create_knowledge_base(
        ...
        try:
            self.logger.info(
                f"🚀 Creating KB '{name}' for user {user_id} in org {org_id}"
            )
    ...
    async def list_user_knowledge_bases(
        ...
        try:
            ...
            (
                ...
                total_count,
                available_filters,
            ) = await self.arango_service.list_user_knowledge_bases(
                user_id=user_key,
                org_id=org_id,
                skip=skip,
                limit=limit,
            ...
            self.logger.info(
                f"✅ Found {len(kbs)} knowledge bases (page {page}/{total_pages})"
            )
            return result
    ...
    async def update_knowledge_base(
        ...
        try:
            ...
            if user_role not in ["OWNER", "WRITER", "ORGANIZER", "FILEORGANIZER"]:
                self.logger.warning(
                    f"⚠️ User {user_id} lacks permission to update KB {kb_id}"
                )
                response = {
                    "success": False,
                    "reason": "User has no permission for KnowledgeBase",
    ...
    async def delete_knowledge_base(
        ...
        try:
            ...
            if user_role != "OWNER":
                self.logger.warning(
                    f"⚠️ User {user_name} lacks permission to delete KB {kb_id}"
                )
                return {
                    "success": False,
            ...
            self.logger.info(
                f"🔐 User {user_name} has OWNER permission - proceeding with deletion"
            )
            ...
            if result:
                self.logger.info(
                    f"✅ Knowledge base {kb_id} deleted successfully by {user_name}"
                )
                return {
                    "success": True,
    ...
    async def create_folder_in_kb(
        ...
        try:
            ...
            validation_result = await self.arango_service._validate_folder_creation(
                kb_id, user_id
            )
    ...
    async def create_nested_folder(
        ...
        try:
            self.logger.info(
                f"🚀 Creating nested folder '{name}' in folder {parent_folder_id}"
            )
            ...
            folder_valid = await self.arango_service.validate_folder_exists_in_kb(
    ...
    async def updateFolder(
        ...
        try:
            ...
            if user_role not in ["OWNER", "WRITER"]:
                self.logger.warning(
                    f"⚠️ User {user_key} lacks permission to update folder in KB {kb_id}"
                )
                response = {
                    "success": False,
                    "reason": "User has no permission for KnowledgeBase",
            ...
            folder_exists = await self.arango_service.validate_folder_in_kb(
                kb_id, folder_id
            )
            ...
            if result:
                self.logger.info(
                    f"✅ Folder updated successfully: {folder_id} by user {user_id}"
                )
                return {
                    "success": True,
                    "code": 200,
            ...
            self.logger.error(
                f"Failed to update folder {folder_id} for knowledge base {kb_id}: {e!s}"
            )
            return {
                "success": False,
                "code": 500,
    ...
    async def delete_folder(
        ...
        try:
            ...
            if user_role != "OWNER":
                self.logger.warning(
                    f"⚠️ User {user_key} lacks permission to add records in KB {kb_id}"
                )
                return {
                    "success": False,
            ...
            if result:
                self.logger.info(
                    f"🎉 Folder {folder_id} and ALL contents deleted successfully by {user_id}"
                )
                return {
                    "success": True,
    ...
    async def delete_records_in_kb(
        ...
        try:
            self.logger.info(
                f"🚀 Bulk deleting {len(record_ids)} records from KB {kb_id} root"
            )
    ...
    async def delete_records_in_folder(
        ...
        try:
            self.logger.info(
                f"🚀 Bulk deleting {len(record_ids)} records from folder {folder_id} in KB {kb_id}"
            )
            ...
            folder_exists = await self.arango_service.validate_folder_exists_in_kb(
                kb_id, folder_id
            )
            if not folder_exists:
                return {
    ...
    async def create_kb_permissions(
        ...
        try:
            self.logger.info(
                f"🚀 Creating {role} permissions for {len(user_ids)} users and {len(team_ids)} teams on KB {kb_id}"
            )
            ...
            if result.get("success"):
                self.logger.info(
                    f"✅ Permissions created: {result['grantedCount']} granted"
                )
                return result
    ...
    async def update_kb_permission(
        ...
        try:
            self.logger.info(
                f"🚀 Updating permission for {len(user_ids)} users and {len(team_ids)} teams on KB {kb_id} to {new_role}"
            )
            ...
            requester = await self.arango_service.get_user_by_user_id(
            ...
            requester_role = await self.arango_service.get_user_kb_permission(
                kb_id, requester_key
            )
            ...
            if user_ids:
                for user_id in user_ids:
                    if user_id in current_permissions["users"]:
                        ...
                        self.logger.warning(
                            f"⚠️ Skipping user {user_id} - no permission found on KB {kb_id}"
                        )
            ...
            if team_ids:
                for team_id in team_ids:
                    if team_id in current_permissions["teams"]:
                        ...
                        self.logger.warning(
                            f"⚠️ Skipping team {team_id} - no permission found on KB {kb_id}"
                        )
    ...
    async def remove_kb_permission(
        ...
        try:
            self.logger.info(
                f"🚀 Removing permission for {len(user_ids)} users and {len(team_ids)} teams from KB {kb_id}"
            )
    ...
    async def list_all_records(
        ...
        try:
            ...
            (
                ...
                total_count,
                available_filters,
            ) = await self.arango_service.list_all_records(
                user_id=user_key,
                org_id=org_id,
                skip=skip,
                limit=limit,
    ...
    async def list_kb_records(
        ...
        try:
            ...
            (
                ...
                total_count,
                available_filters,
            ) = await self.arango_service.list_kb_records(
                kb_id=kb_id,
                user_id=user_key,
                org_id=org_id,
                skip=skip,
    ...
    async def get_kb_children(
        ...
        try:
            self.logger.info(
                f"🔍 Getting KB {kb_id} children with pagination (page {page}, limit {limit})"
            )
            ...
            if not user_role:
                return self._error_response(
                    403, "User has no permission for KnowledgeBase"
                )
            ...
            self.logger.info(
                f"✅ KB children retrieved: {result['counts']['folders']} folders, {result['counts']['records']} records"
            )
            return result
    ...
    async def get_folder_children(
        ...
        try:
            self.logger.info(
                f"🔍 Getting folder {folder_id} children with pagination (page {page}, limit {limit})"
            )
            ...
            if not result.get("success"):
                return self._error_response(
                    404, result.get("reason", "Folder not found")
                )
            ...
            self.logger.info(
                f"✅ Folder children retrieved: {result['counts']['folders']} folders, {result['counts']['records']} records"
            )
            return result

        except Exception as e:
            self.logger.error(
                f"❌ Failed to get folder children with pagination: {e!s}"
            )
            return self._error_response(500, str(e))
    ...
    async def upload_records_to_kb(
        self, kb_id: str, user_id: str, org_id: str, files: list[dict]
    ) -> dict:
        """Upload to KB root"""
        return await self.arango_service.upload_records(
            kb_id, user_id, org_id, files, parent_folder_id=None
        )

    async def upload_records_to_folder(
        self, kb_id: str, folder_id: str, user_id: str, org_id: str, files: list[dict]
    ) -> dict:
        """Upload to specific folder"""
        return await self.arango_service.upload_records(
            kb_id, user_id, org_id, files, parent_folder_id=folder_id
        )
