...
class CreateKnowledgeBaseRequest(BaseModel):
    ...
    name: str = Field(
        ..., description="Name of the knowledge base", min_length=1, max_length=255
    )
...
class UpdateKnowledgeBaseRequest(BaseModel):
    ...
    groupName: str | None = Field(
        None, description="Name of the knowledge base", min_length=1, max_length=255
    )
...
class CreatePermissionRequest(BaseModel):
    ...
    requesterId: str = Field(
        ..., description="User id granting others access", min_length=1
    )
    userIds: list[str] | None = Field(
        None, description="List of user IDs to grant permissions to", min_items=0
    )
    teamIds: list[str] | None = Field(
        None, description="List of team IDs to grant permissions to", min_items=0
    )
...
class CreateRecordsRequest(BaseModel):
    ...
    records: list[dict[str, Any]] = Field(
        ..., description="List of record metadata dicts"
    )
    fileRecords: list[dict[str, Any]] = Field(
        ..., description="List of file metadata dicts (same length as records)"
    )
...
class UpdateRecordRequest(BaseModel):
    ...
    fileMetadata: dict[str, Any] | None = Field(
        None, description="Optional file metadata for file update"
    )
...
class DeleteRecordRequest(BaseModel):
    ...
    recordIds: list[str] = Field(
        ..., description="List of user IDs to grant permissions to", min_items=1
    )
...
class RecordResponse(BaseModel):
    ...
    sourceCreatedAtTimestamp: int | None = Field(
        None, description="Source creation timestamp"
    )
    sourceLastModifiedTimestamp: int | None = Field(
        None, description="Source modification timestamp"
    )
    ...
    isLatestVersion: bool | None = Field(
        ..., description="Whether this is the latest version"
    )
    webUrl: str | None = Field(None, description="Web URL")
    fileRecord: FileRecordResponse | None = Field(
        None, description="Associated file record"
    )
    folder: FolderResponse | None = Field(None, description="Folder information")
    permission: PermissionContents | None = Field(
        None, description="Permission information"
    )
...
class ListKnowledgeBaseResponse(BaseModel):
    ...
    pagination: KnowledgeBasePaginationResponse = Field(
...
class AppliedFiltersResponse(BaseModel):
    ...
    recordTypes: list[str] | None = Field(
        None, description="Applied record type filters"
    )
    ...
    indexingStatus: list[str] | None = Field(
        None, description="Applied indexing status filters"
    )
    permissions: list[str] | None = Field(
        None, description="Applied permission filters"
    )
    source: str | None = Field(None, description="Applied source filter")
    dateRange: dict[str, int | None] | None = Field(
        None, description="Applied date range"
    )
...
class UpdateRecordResponse(BaseModel):
    ...
    updatedRecord: dict[str, Any] = Field(
        ..., description="The full document of the updated record."
    )
    fileUpdated: bool = Field(
        ...,
        description="True if the associated file metadata was also updated, otherwise False.",
    )
    updatedFile: dict[str, Any] | None = Field(
        None, description="The full document of the updated file record, if applicable."
    )
    recordId: str = Field(..., description="The ID of the record that was updated.")
    timestamp: int = Field(
        ..., description="The epoch timestamp (in ms) of when the update occurred."
    )
    location: str = Field(
        ..., description="The location of the record, e.g., 'kb_root' or 'folder'."
    )
    folderId: str | None = Field(
        None, description="The ID of the parent folder, if the record is in a folder."
    )
    kb: dict[str, Any] = Field(
        ..., description="Information about the knowledge base containing the record."
    )
    userPermission: str = Field(
        ..., description="The permission role of the user who performed the update."
    )
...
class CreatePermissionsResponse(BaseModel):
    ...
    grantedUsers: list[str] = Field(
        ..., description="List of users granted permissions"
    )
    grantedTeams: list[str] = Field(
        ..., description="List of teams granted permissions"
    )
    role: str = Field(..., description="Granted role")
    kbId: str = Field(..., description="Knowledge base ID")
    details: dict[str, Any] = Field(
        ..., description="Details of the permissions created"
    )
