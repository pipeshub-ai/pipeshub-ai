...
@ConnectorBuilder("Nextcloud")\
    ...
    .with_description("Sync files and folders from Nextcloud")\
    ...
    .configure(lambda builder: builder
        ...
        .add_auth_field(CommonFields.base_url("Nextcloud"))
...
class NextcloudConnector(BaseConnector):
    def __init__(
        ...
        self.activity_sync_point = _create_sync_point(SyncDataPointType.RECORDS)
    ...
    async def init(self) -> bool:
        ...
        try:
            ...
            self.data_source = NextcloudDataSource(client)
            self.logger.info("Nextcloud client initialized successfully.")
            return True
        except Exception as e:
    ...
    async def _sync_users(self) -> List[AppUser]:
        """Fetch and sync all users from Nextcloud."""
        try:
            self.logger.info("Syncing Nextcloud users...")

            response = await self.data_source.get_users()

            if not is_response_successful(response):
                self.logger.error(f"Failed to fetch users: {get_response_error(response)}")
                return []

            body = extract_response_body(response)
            if not body:
                self.logger.error("Empty response body when fetching users")
                return []

            try:
                data = json.loads(body)
            except json.JSONDecodeError as e:
                self.logger.error(f"Failed to parse users response: {e}")
                return []

            ocs_data = data.get('ocs', {}).get('data', {})

            if isinstance(ocs_data, dict):
                user_ids = ocs_data.get('users', [])
            elif isinstance(ocs_data, list):
                user_ids = ocs_data
            else:
                self.logger.warning(f"Unexpected users response format: {type(ocs_data)}")
                return []

            if not user_ids:
                self.logger.warning("No users found in response")
                return []

            self.logger.info(f"Found {len(user_ids)} users: {user_ids}")
            ...
            for user_id in user_ids:
                try:
                    async with self.rate_limiter:
                        detail_response = await self.data_source.get_user_details(user_id)

                    if not is_response_successful(detail_response):
                        self.logger.warning(
                            f"Could not fetch details for {user_id}: {get_response_error(detail_response)}"
                        )
                        continue

                    detail_body = extract_response_body(detail_response)
                    if not detail_body:
                        continue
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to parse user details for {user_id}")
                        continue

                    user_data = detail_data.get('ocs', {}).get('data', {})
                    if not user_data:
                        continue

                    email = user_data.get('email')
                    if not email:
                         email = f"{user_id}@nextcloud.local"

                    display_name = user_data.get('displayname') or user_data.get('display-name') or user_id
                    is_active_val = user_data.get('enabled', True)
    ...
    async def _sync_user_groups(self) -> None:
        """Fetch and sync all user groups from Nextcloud."""
        try:
            self.logger.info("Syncing Nextcloud user groups...")
            ...
            if isinstance(ocs_data, dict):
                ...
                self.logger.warning("Unexpected users response format")
                return
            ...
            for user_id in user_ids:
                try:
                    ...
                    if user_groups:
                        ...
                        full_name = user_data.get('displayname') or user_data.get('display-name') or user_id
                        is_active_val = user_data.get('enabled', True)
                        ...
                        for group_id in user_groups:
                            ...
                            group_members[group_id].append(member_user)

                except Exception as e:
                    self.logger.warning(f"Error processing user {user_id} for groups: {e}")
                    continue

            self.logger.info(f"Found {len(all_groups)} unique groups")

            groups_payload = []

            for group_id in all_groups:
                try:
                    app_group = AppUserGroup(
                        app_name=self.connector_name,
                        source_user_group_id=group_id,
                        name=group_id,
                        description=f"Nextcloud Group: {group_id}"
                    )

                    members = group_members.get(group_id, [])
                    groups_payload.append((app_group, members))

                except Exception as g_ex:
                    self.logger.warning(f"Error preparing group {group_id}: {g_ex}")

            if groups_payload:
                await self.data_entities_processor.on_new_user_groups(groups_payload)
                self.logger.info(f"Successfully synced {len(groups_payload)} user groups")

        except Exception as e:
            self.logger.error(f"Error syncing user groups: {e}", exc_info=True)

    async def _sync_record_groups(self, users: List[AppUser]) -> None:
        """Create record groups for user drives."""
        try:
            self.logger.info("Syncing Nextcloud record groups...")

            record_groups = []
            for user in users:
                if not user.full_name or not user.full_name.strip():
                    self.logger.warning(f"Skipping user with empty full_name: {user.email}")
                    continue

                if not user.source_user_id or not user.source_user_id.strip():
                    self.logger.warning(f"Skipping user with empty source_user_id: {user.email}")
                    continue

                record_group = RecordGroup(
                    name=f"{user.full_name}'s Files",
                    org_id=self.data_entities_processor.org_id,
            ...
            if record_groups:
                ...
                self.logger.info(f"Created {len(record_groups)} record groups")
    ...
    async def run_sync(self) -> None:
        """
        Runs a full synchronization from the Nextcloud instance.
        """
        try:
            self.logger.info("Starting Nextcloud full sync.")
            ...
            if self.data_source is None:
                ...
                if not init_success or self.data_source is None:
                    self.logger.error(
                        ...
                        "Check your configuration (baseUrl, bearerToken)."
                    )
                    return
            ...
            self.logger.info("Step 2: Syncing user groups...")
            await self._sync_user_groups()

            # Step 3: Create record groups
            self.logger.info("Step 3: Syncing record groups...")
            await self._sync_record_groups(users)

            # Step 4: Sync files for active users
            self.logger.info("Step 4: Syncing user files...")

            users_to_sync = [user for user in users if user.is_active]

            self.logger.info(
                f"Syncing files for {len(users_to_sync)} active users "
            ...
            self.logger.info("Nextcloud full sync completed successfully.")
    ...
    async def get_signed_url(self, record: Record) -> Optional[str]:
        ...
        try:
            ...
            if not user_with_permission:
                self.logger.warning(f"No user with permission to: {record.id}")
                return None

            if not file_record or not file_record.external_record_id:
                self.logger.warning(f"No file record or external ID for: {record.id}")
                return None

            # Check if it's a folder (folders can't be downloaded)
            if file_record.mime_type == MimeTypes.FOLDER:
                self.logger.warning(f"Cannot create signed URL for folder: {record.id}")
                return None

            try:
                async with self.rate_limiter:
                    response = await self.data_source.create_direct_download_token(
                        file_id=int(file_record.external_record_id)
                    )
            except ValueError:
                self.logger.error(f"Invalid file_id format for {record.id}: {file_record.external_record_id}")
                return None

            if not is_response_successful(response):
                error_msg = get_response_error(response)
                self.logger.error(f"Failed to create download token: {error_msg}")
                return None

            body = extract_response_body(response)
            if not body:
                self.logger.warning(f"Empty response body for download token: {record.id}")
                return None

            # Log the response for debugging
            try:
                body_preview = body[:200].decode('utf-8', errors='replace') if isinstance(body, bytes) else str(body)[:200]
                self.logger.debug(f"Download token response preview: {body_preview}")
            except Exception:
                pass

            # Try to parse as XML first (Nextcloud's expected format)
            try:
                ...
                try:
                    ...
                    # Check OCS response format
                    ocs_data = data.get('ocs', {}).get('data', {})
                    if isinstance(ocs_data, dict):
                        url = ocs_data.get('url')
                        if url:
                            return url
                    ...
                    self.logger.warning(f"No URL found in JSON response: {data}")
                except (json.JSONDecodeError, AttributeError) as json_err:
                    self.logger.error(f"Failed to parse response as XML or JSON: {json_err}")

            if file_record.path and user_with_permission.source_user_id:
                base_url = self.data_source.client.get_base_url()

                # Extract relative path from full WebDAV path
                # e.g., /remote.php/dav/files/NC_Admin/Documents/file.pdf -> Documents/file.pdf
                ...
                direct_url = f"{base_url}/remote.php/dav/files/{user_with_permission.source_user_id}/{encoded_path}"

                self.logger.info(f"Using direct WebDAV URL for {record.record_name}: {direct_url}")
                return direct_url

            self.logger.error(f"Could not generate any download URL for {record.id}")
            return None

        except Exception as e:
            self.logger.error(f"Error creating signed URL for {record.id}: {e}", exc_info=True)
            return None

    async def stream_record(self, record: Record) -> StreamingResponse:
        """Stream a file's content for download."""
