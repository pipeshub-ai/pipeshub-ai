...
class BaseDriveSyncService(ABC):
    ...
    async def ensure_user_app_relation(self, user_email: str, org_id: str) -> dict | None:
...
class DriveSyncEnterpriseService(BaseDriveSyncService):
    ...
    async def setup_changes_watch(self, user_email: str) -> dict | None:
        ...
        try:
            ...
            if not page_token:
                self.logger.info("📝 No page token found for user %s, creating new watch", user_email)
                    ...
                    try:
                        ...
                        if channel_data:
                            await self.arango_service.store_page_token(
                                channel_data.get("channelId"),
                                channel_data.get("resourceId"),
                                user["email"],
                                channel_data.get("token"),
    ...
    async def resync_drive(self, org_id, user) -> bool | None:
        try:
            ...
            if not page_token:
                self.logger.warning(
                    f"No page token found for user {user['email']}. "
                ...
                try:
                    ...
                    for user in users:
                        try:
                            ...
                            self.logger.error(f"❌ Error sending event for user {user['email']}: {e!s}")
                            continue

                except Exception as e:
                    self.logger.error(f"❌ Error processing record {record['_key']}: {e!s}")
                    continue
