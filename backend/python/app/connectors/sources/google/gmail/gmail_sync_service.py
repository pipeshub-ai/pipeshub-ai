...
class BaseGmailSyncService(ABC):
    ...
    async def ensure_user_app_relation(self, user_email: str, org_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            app = await self.arango_service.get_document(self.connector_id, CollectionNames.APPS.value)
            if not app:
                self.logger.error("App document not found for %s", self.connector_id)
                return None
            ...
            app_edge_data = {
                ...
                "_to": f"{CollectionNames.APPS.value}/{self.connector_id}",
...
class GmailSyncEnterpriseService(BaseGmailSyncService):
    ...
    async def reindex_failed_records(self, org_id) -> bool | None:
        ...
        try:
            ...
            # Filter by both connector_name and connector_id to ensure we only get records from this specific connector instance
...
class GmailSyncIndividualService(BaseGmailSyncService):
    ...
    async def _should_stop(self, org_id) -> bool:
        ...
        if self._stop_requested:
            ...
            if not credential_owner_email:
                return False
            current_state = await self.arango_service.get_user_sync_state(
                credential_owner_email, Connectors.GOOGLE_MAIL.value.lower(), connector_id=self.connector_id
            )
            if current_state:
                current_state = current_state.get("syncState")
                if current_state == "IN_PROGRESS":
                    ...
                    self.logger.info("✅ Gmail sync state updated before stopping for credential owner")
                    return True
            return False
        return False
    ...
    async def connect_services(self, org_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info("📋 Initializing Gmail service from connector config")
            if not await self.gmail_user_service.connect_from_connector_config(org_id, self.connector_id):
                self.logger.error("❌ Failed to initialize Gmail service from connector config")
                return False
            ...
            self.logger.info("📋 Fetching credential owner user from Gmail API")
            fetched_users = await self.gmail_user_service.list_individual_user(org_id)
            if not fetched_users or len(fetched_users) == 0:
                self.logger.error("❌ Unable to get credential owner user from Gmail API")
                return False
            ...
            # Connect to Google Gmail using the credential owner's credentials
    ...
    async def setup_changes_watch(self, org_id: str, user_email: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not await user_service.connect_individual_user(org_id, user.user_id, self.connector_id):
        ...
        try:
            ...
            if user_info:
                ...
                if not user_id:
                    ...
                    app = await self.arango_service.get_document(self.connector_id, CollectionNames.APPS.value)
                    if not app:
                        raise Exception("Failed to get app by connector id")
                    # Create edge between user and app
                    app_edge_data = {
            ...
            user_info_list = await self.gmail_user_service.list_individual_user(org_id)
            ...
            # Get user from database (should exist from connect_services) - MUST be in users collection
            ...
            self.logger.info(f"🚀 Using credential owner user: {user_info}")
