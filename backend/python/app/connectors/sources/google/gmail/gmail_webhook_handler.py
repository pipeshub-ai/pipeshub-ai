...
class AbstractGmailWebhookHandler(ABC):
    ...
    async def _parse_pubsub_message(self, message) -> dict | None:
        ...
        try:
            ...
            self.logger.error(
                f"{self.handler_type} webhook: Failed to parse message: {e!s}",
            )
            return None
        except Exception as e:
            self.logger.error(
                f"{self.handler_type} webhook: Error processing message: {e!s}",
            )
            return None
    ...
    async def process_notification(self, headers: dict, message: str) -> bool:
        ...
        try:
            self.logger.debug(
                "%s webhook: Starting notification processing",
                self.handler_type,
            )
            ...
            if not message_data:
                self.logger.error(
                    "%s webhook: Invalid message format",
                    self.handler_type,
                )
                return False
        ...
        try:
            ...
            async with self.processing_lock:
                ...
                if not channel_history:
                    self.logger.warning(
                        f"""⚠️ No historyId found for {email_address}""",
                    )
                    return False
                ...
                if changes and isinstance(changes, dict) and changes.get("history"):
                    ...
                    self.logger.info(
                        "%s webhook: No changes to process",
                        self.handler_type,
                    )
