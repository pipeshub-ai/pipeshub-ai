...
async def test_run() -> None:
    ...
    if not bookstack_base_url:
        ...
        logger.info(
            "Example: export BOOKSTACK_BASE_URL='https://your-bookstack-instance.com'"
        )
        return
    ...
    if not bookstack_token_id:
        ...
        logger.info(
            "You can generate a token from BookStack user settings -> API Tokens"
        )
        return
    ...
    stored_config = await key_value_store.get_key(
    ...
    try:
        ...
        if await bookstack_connector.init():
            ...
            if await bookstack_connector.test_connection_and_access():
                ...
                logger.error(
                    "BookStack connection test failed. Check your API URL and token."
                )
        else:
