...
@(
    ...
    .configure(
        lambda builder: builder.with_icon("/assets/icons/connectors/bookstack.svg")
...
class BookStackConnector(BaseConnector):
    ...
    def __init__(
        ...
        self.data_source: BookStackDataSource | None = None
        self.batch_size = 100
    ...
    async def init(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to initialize BookStack client: {e}", exc_info=True
                )
                return False
    ...
    async def get_all_users(self) -> list[AppUser]:
        ...
        while True:
            ...
            if not response.success or not response.data:
                self.logger.error(
                    f"Failed to fetch users from BookStack: {response.error}"
                )
                break
        ...
        self.logger.info(
            f"Successfully fetched {len(all_bookstack_users)} users from BookStack."
        )
    ...
    async def list_roles_with_details(self) -> dict[int, dict]:
        ...
        for i, response in enumerate(detail_responses):
            ...
            if isinstance(response, Exception):
                self.logger.error(
                    f"Exception fetching details for role ID {role_id}: {response}"
                )
            elif not response.success:
                self.logger.error(
                    f"Failed to get details for role ID {role_id}: {response.error}"
                )
            else:
                roles_details_map[role_id] = response.data

        self.logger.info(
            f"Successfully fetched details for {len(roles_details_map)} roles."
        )
        return roles_details_map

    def _parse_id_and_name_from_event(
        self, event: dict
        ...
        if not detail:
            self.logger.warning(
                f"Audit log event (ID: {event.get('id')}) is missing 'detail'."
            )
            return None, None
        ...
        try:
            ...
            self.logger.error(
                f"Could not parse ID and name from audit log detail: '{detail}'"
            )
            return None, None
    ...
    async def _sync_users(self, full_sync: bool = False) -> None:
        ...
        bookstack_user_sync_key = generate_record_sync_point_key(
            "bookstack", "user_logs", "global"
        )
        bookstack_user_sync_point = await self.user_sync_point.read_sync_point(
    ...
    async def _sync_users_incremental(
        self, app_users: list[AppUser], last_sync_timestamp: str
    ) -> None:
        """Syncs only the users that have been newly created since the last sync timestamp
        by checking the audit log.
        """
        self.logger.info(
            f"Starting incremental user sync from timestamp: {last_sync_timestamp}"
        )
        ...
        if user_create_response.success and user_create_response.data.get("data"):
            await self._handle_user_create_event(
                user_create_response.data.get("data"), app_users
            )
        if user_update_response.success and user_update_response.data.get("data"):
            await self._handle_user_update_event(
                user_update_response.data.get("data"), app_users
            )
    ...
    async def _handle_user_create_event(
        self, user_create_events: list[dict], app_users: list[AppUser]
    ) -> None:
        ...
        for entry in log_entries:
            ...
            try:
                ...
                self.logger.warning(
                    f"Could not parse user ID from audit log detail: '{detail_string}'"
                )
        ...
        self.logger.info(
            f"Found {len(new_user_ids)} new user(s) in audit log with IDs: {new_user_ids}"
        )
        ...
        if newly_created_users:
            self.logger.info(
                f"Submitting {len(newly_created_users)} newly created users for processing."
            )
            ...
            self.logger.warning(
                "Found new user IDs in audit log, but no matching user objects were found in the full user list."
            )

    async def _handle_user_update_event(
        self, user_update_events: list[dict], app_users: list[AppUser]
    ) -> None:
        ...
        for event in user_update_events:
            ...
            self.logger.info(
                f"Processing role updates for user: {name} (ID: {user_id})"
            )
            ...
            try:
                ...
                if not user_response.success or not user_response.data:
                    self.logger.error(
                        f"Failed to fetch details for user ID {user_id}: {user_response.error}"
                    )
                    continue
                ...
                if not user_email:
                    self.logger.warning(
                        f"User {name} (ID: {user_id}) has no email, skipping role updates."
                    )
                    continue
                ...
                async with self.data_store_provider.transaction() as tx_store:
                    ...
                    if not user:
                        self.logger.warning(
                            f"User {name} (ID: {user_id}) not found in the database, skipping role updates."
                        )
                        continue
                    user_key = f"{CollectionNames.USERS.value}/{user.id}"
                    await tx_store.delete_edges_to_groups(
                        user_key, CollectionNames.PERMISSION.value
                    )

                if not roles:
                    self.logger.info(
                        f"User {name} (ID: {user_id}) has no roles assigned."
                    )
                    continue

                # Resync each role the user is part of
                self.logger.info(
                    f"User {name} is part of {len(roles)} role(s). Resyncing..."
                )
                ...
                for role in roles:
                    ...
                    if role_id:
                        self.logger.info(
                            f"Resyncing role '{role_name}' (ID: {role_id}) due to user update"
                        )
                        await self._handle_role_create_event(role_id, user_email_map)
                    else:
                        self.logger.warning(
                            f"Role '{role_name}' has no ID, skipping resync"
                        )

            except Exception as e:
                self.logger.error(
                    f"Error processing user update for user ID {user_id}: {e}",
                    exc_info=True,
                )

        self.logger.info("✅ Finished processing user update events and role resyncs")

    async def _handle_user_delete_event(
        self, user_delete_events: list[dict], app_users: list[AppUser]
    ) -> None:
        ...
        for event in user_delete_events:
            ...
            if user_id is None:
                self.logger.warning(
                    f"Could not parse user ID from delete event: {event}"
                )
                continue
    ...
    async def _sync_user_groups(self, full_sync: bool = False) -> None:
        current_timestamp = self._get_iso_time()
        bookstack_user_group_sync_key = generate_record_sync_point_key(
            "bookstack", "user_group_logs", "global"
        )
        bookstack_user_group_sync_point = (
            await self.user_group_sync_point.read_sync_point(
    ...
    async def _sync_user_groups_full(self) -> None:
        ...
        if not all_roles_with_details:
            self.logger.warning(
                "No roles found in BookStack. Aborting user group sync."
            )
            return
        self.logger.info(
            f"Found {len(all_roles_with_details)} total roles with details."
        )
        ...
        user_details_map = {user.get("id"): user for user in all_users_with_details}
        ...
        for role in all_roles_with_details:
            ...
            for user in role.get("users", []):
                ...
                if not user_details:
                    self.logger.warning(
                        f"No details found for user ID {user_id} in role {role.get('display_name')}"
                    )
                    continue
                ...
                app_user = AppUser(
                    ...
                    source_updated_at=self._parse_timestamp(
                        user_details.get("updated_at")
                    )
                    if user_details.get("updated_at")
                    else None,
                )
        ...
        if user_groups_batch:
            self.logger.info(
                f"Submitting {len(user_groups_batch)} groups with their users..."
            )
    ...
    async def _fetch_all_roles_with_details(self) -> list[dict]:
        ...
        self.logger.info(
            f"Fetching details for {len(basic_roles)} roles concurrently..."
        )
        tasks = [self.data_source.get_role(role["id"]) for role in basic_roles]
        ...
        self.logger.info(
            f"Successfully fetched details for {len(detailed_roles)} roles."
        )
    ...
    def _build_role_permissions_map(
        ...
        for role in all_roles_with_details:
            ...
            for user in role.get("users", []):
                ...
                if not user_id or not user_email:
                    self.logger.warning(
                        f"Skipping user in role '{role.get('display_name')}' due to missing ID or email."
                    )
                    continue
        ...
        self.logger.info(
            f"Permission map built for {len(role_to_permissions_map)} roles."
        )
    ...
    async def _fetch_all_roles(self) -> list[dict]:
        ...
        offset = 0
        while True:
            response = await self.data_source.list_roles(
                count=self.batch_size, offset=offset
            )
            if not response.success or not response.data:
    ...
    async def _fetch_all_users_with_details(self) -> list[dict]:
        ...
        self.logger.info(
            f"Fetching details for {len(all_users_summary)} users concurrently..."
        )
        ...
        self.logger.info(
            f"Successfully fetched details for {len(detailed_users)} users."
        )
    ...
    async def _sync_user_groups_incremental(self, last_sync_timestamp: str) -> None:
        ...
        if (
            roles_create_events.success
            and roles_create_events.data
            and roles_create_events.data.get("data")
        ):
        ...
        if (
            role_update_events.success
            and role_update_events.data
            and role_update_events.data.get("data")
        ):
        ...
        if (
            role_delete_events.success
            and role_delete_events.data
            and role_delete_events.data.get("data")
        ):
    ...
    async def _handle_role_create_event(
        self, role_id: int, user_email_map: dict[int, str]
    ) -> None:
        ...
        if not role_details_response.success or not role_details_response.data:
            self.logger.warning(
                f"Could not fetch details for role ID {role_id}, it may not be available yet. Skipping."
            )
            return
        ...
        if role_users:
            ...
            for i, res in enumerate(user_responses):
                if isinstance(res, Exception) or not res.success or not res.data:
                    self.logger.warning(
                        f"Failed to get details for user ID {user_ids[i]}: {res}"
                    )
                    continue
        ...
        self.logger.info(
            f"Processing newly created user group '{app_user_group.name}' with {len(app_users)} members..."
        )
        await self.data_entities_processor.on_new_user_groups(
            [(app_user_group, app_users)]
        )

    async def _handle_role_update_event(
        self, role_id: int, user_email_map: dict[int, str]
    ) -> None:
        await self._handle_role_delete_event(role_id)
        await self._handle_role_create_event(role_id, user_email_map)
    ...
    async def _sync_record_groups(self, full_sync: bool = False) -> None:
        current_timestamp = self._get_iso_time()
        bookstack_record_group_sync_key = generate_record_sync_point_key(
            "bookstack", "record_groups", "global"
        )
        bookstack_record_group_sync_point = (
            await self.record_sync_point.read_sync_point(
        ...
        if full_sync or not bookstack_record_group_sync_point.get("timestamp"):
            ...
            await self._sync_record_groups_incremental(
                last_sync_timestamp, roles_details
            )
            await self.record_sync_point.update_sync_point(
                bookstack_record_group_sync_key,
                {"timestamp": current_timestamp},
    ...
    async def _sync_record_groups_full(self, roles_details: dict[int, dict]) -> None:
        ...
        """
        self.logger.info(
            "Starting sync for shelves, books, and chapters as record groups."
        )
    ...
    async def _sync_content_type_as_record_group(
        ...
        while True:
            ...
            if not response.success or not response.data:
                self.logger.error(
                    f"Failed to fetch {content_type_name}s: {response.error}"
                )
                break
        ...
        self.logger.info(
            f"Found {len(all_items)} {content_type_name}s. Fetching permissions..."
        )
        ...
        for item in all_items:
            ...
            tasks.append(
                self._create_record_group_with_permissions(
                    item, content_type_name, roles_details, parent_external_id
                ),
            )
    ...
    async def _create_record_group_with_permissions(
    ...
    ) -> tuple[RecordGroup, list[Permission]] | None:
        ...
        try:
            ...
            if not all([item_id, item_name]):
                self.logger.warning(
                    f"Skipping {content_type_name} due to missing id or name: {item}"
                )
                return None
            ...
            if permissions_response.success and permissions_response.data:
                permissions_list = await self._parse_bookstack_permissions(
                    permissions_response.data, roles_details, content_type_name
                )

                fallback_permissions = permissions_response.data.get(
                    "fallback_permissions"
                )
                if fallback_permissions and parent_external_id:
                    # Set inherit_permissions based on the 'inheriting' flag
                    record_group.inherit_permissions = fallback_permissions.get(
                        "inheriting", False
                    )
            else:
                self.logger.warning(
    ...
    async def _parse_bookstack_permissions(
        ...
        if owner and owner.get("id"):
            try:
                ...
                self.logger.error(
                    f"Failed to fetch owner details for user ID {owner.get('id')}: {e}"
                )
        ...
        if role_permissions:
            for role_perm in role_permissions:
                ...
                if (
                    role_perm.get("update")
                    or role_perm.get("delete")
                    or role_perm.get("create")
                ):
    ...
    def _parse_bookstack_permissions_all_users(
        self, all_users: list[AppUser]
    ) -> list[Permission]:
    ...
    async def _sync_record_groups_incremental(
        self, last_sync_timestamp: str, roles_details: dict[int, dict]
    ) -> None:
        ...
        self.logger.info(
            "Starting sync for record groups (books and shelves) as record groups."
        )
    ...
    async def _sync_record_groups_events(
        ...
        tasks = {
            "create": self.data_source.list_audit_log(
                filter={
                    "type": f"{content_type}_create",
                    "created_at:gte": last_sync_timestamp,
                }
            ),
            "update": self.data_source.list_audit_log(
                filter={
                    "type": f"{content_type}_update",
                    "created_at:gte": last_sync_timestamp,
                }
            ),
            "delete": self.data_source.list_audit_log(
                filter={
                    "type": f"{content_type}_delete",
                    "created_at:gte": last_sync_timestamp,
                }
            ),
            "permissions_update": self.data_source.list_audit_log(
                filter={
        ...
        if (
            create_response
            and create_response.success
            and create_response.data.get("data")
        ):
            self.logger.info(
                f"Found {len(create_response.data['data'])} new {content_type}(s) to create."
            )
            for event in create_response.data["data"]:
                await self._handle_record_group_create_event(
                    event, content_type, roles_details
                )
        ...
        if (
            update_response
            and update_response.success
            and update_response.data.get("data")
        ):
            self.logger.info(
                f"Found {len(update_response.data['data'])} updated {content_type}(s) to update."
            )
        ...
        if (
            delete_response
            and delete_response.success
            and delete_response.data.get("data")
        ):
            self.logger.info(
                f"Found {len(delete_response.data['data'])} deleted {content_type}(s) to delete."
            )
        ...
        if (
            permissions_update_response
            and permissions_update_response.success
            and permissions_update_response.data.get("data")
        ):
            self.logger.info(
                f"Found {len(permissions_update_response.data['data'])} updated {content_type}(s) to update."
            )
    ...
    async def _handle_record_group_create_event(
        self, event: dict, content_type: str, roles_details: dict[int, dict]
    ) -> None:
        ...
        self.logger.info(
            f"{content_type} created/updated: '{item_name}' (ID: {item_id}). Fetching details..."
        )
        ...
        if not fetch_method:
            self.logger.error(
                f"Invalid content_type '{content_type}' in event handler."
            )
            return
        ...
        if not response.success or not response.data:
            self.logger.warning(
                f"Could not fetch details for new {content_type} ID {item_id}. Skipping."
            )
            return
        ...
        if record_group_tuple:
            ...
            self.logger.info(
                f"✅ Successfully processed new {content_type}: '{item_name}'."
            )

    async def _handle_record_group_delete_event(
        self, event: dict, content_type: str
    ) -> None:
    ...
    async def _sync_records(self, full_sync: bool = False) -> None:
        current_timestamp = self._get_iso_time()
        bookstack_record_sync_key = generate_record_sync_point_key(
            "bookstack", "records", "global"
        )
        bookstack_record_sync_point = await self.record_sync_point.read_sync_point(
        ...
        if full_sync or not bookstack_record_sync_point.get("timestamp"):
            ...
            await self._sync_records_incremental(
                last_sync_timestamp, roles_details, users
            )
            await self.record_sync_point.update_sync_point(
                bookstack_record_sync_key,
                {"timestamp": current_timestamp},
    ...
    async def _sync_records_full(
        self, roles_details: dict[int, dict], users: list[AppUser]
    ) -> None:
        ...
        while True:
            ...
            if response.success and response.data and "content" in response.data:
                try:
                    ...
                    self.logger.error(
                        f"Failed to decode JSON content for pages: {response.data['content']}"
                    )
                    break
            else:
                self.logger.error(
                    f"Failed to fetch pages or malformed response: {response.error}"
                )
                break
            ...
            self.logger.info(
                f"Processing page {offset + 1} to {offset + len(pages_page)}..."
            )

            # Process each page from the current API response
            for page in pages_page:
                record_update = await self._process_bookstack_page(
                    page, roles_details, users
                )
                ...
                if record_update.record:
                    ...
                    if len(batch_records) >= self.batch_size:
                        self.logger.info(
                            f"Processing batch of {len(batch_records)} new page records."
                        )
        ...
        if batch_records:
            self.logger.info(
                f"Processing final batch of {len(batch_records)} new page records."
            )
            await self.data_entities_processor.on_new_records(batch_records)

        self.logger.info("✅ Finished syncing all page records.")

    async def _process_bookstack_page(
        self, page: dict, roles_details: dict[int, dict], users: list[AppUser]
    ) -> RecordUpdate | None:
        ...
        try:
            ...
            if existing_record:
                ...
                if existing_record.external_revision_id != str(
                    page.get("revision_count")
                ):
            ...
            if permissions_response.success and permissions_response.data:
                ...
                new_permissions = self._parse_bookstack_permissions_all_users(
                    all_users=users
                )
                ...
                if fallback_permissions:
                    # Set inherit_permissions based on the 'inheriting' flag
                    file_record.inherit_permissions = fallback_permissions.get(
                        "inheriting", True
                    )
            else:
                self.logger.warning(
            ...
            self.logger.error(
                f"Error processing BookStack page ID {page.get('id')}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def _handle_record_updates(self, record_update: RecordUpdate) -> None:
        ...
        try:
            if record_update.is_deleted:
                ...
                self.logger.info(
                    f"New record detected: {record_update.record.record_name}"
                )
            elif record_update.is_updated:
                if record_update.content_changed:
                    self.logger.info(
                        f"Content changed for record: {record_update.record.record_name}"
                    )
                ...
                if record_update.metadata_changed:
                    self.logger.info(
                        f"Metadata changed for record: {record_update.record.record_name}"
                    )
                ...
                if record_update.permissions_changed:
                    self.logger.info(
                        f"Permissions changed for record: {record_update.record.record_name}"
                    )
    ...
    async def _sync_records_incremental(
        ...
        """
        self.logger.info(
            f"Starting incremental record (page) sync from: {last_sync_timestamp}"
        )

        # 1. Concurrently fetch all page-related events
        tasks = {
            "create": self.data_source.list_audit_log(
                filter={"type": "page_create", "created_at:gte": last_sync_timestamp}
            ),
            "update": self.data_source.list_audit_log(
                filter={"type": "page_update", "created_at:gte": last_sync_timestamp}
            ),
            "permissions_update": self.data_source.list_audit_log(
                filter={
                    "type": "permissions_update",
            ...
            "delete": self.data_source.list_audit_log(
                filter={"type": "page_delete", "created_at:gte": last_sync_timestamp}
            ),
            "move": self.data_source.list_audit_log(
                filter={"type": "page_move", "created_at:gte": last_sync_timestamp}
            ),
        }
        results = await asyncio.gather(*tasks.values(), return_exceptions=True)
        event_responses = dict(zip(tasks.keys(), results))
        ...
        if (
            ...
            self.logger.info(
                f"Found {len(create_response.data['data'])} new page(s) to create."
            )
            for event in create_response.data["data"]:
        ...
        if (
            ...
            self.logger.info(
                f"Found {len(update_response.data['data'])} page(s) to update."
            )
            for event in update_response.data["data"]:
                await self._handle_page_upsert_event(event, roles_details, users)
        ...
        if (
            ...
            self.logger.info(
                f"Found {len(permissions_update_response.data['data'])} page(s) to update."
            )
            for event in permissions_update_response.data["data"]:
        ...
        if (
            ...
            self.logger.info(
                f"Found {len(delete_response.data['data'])} page(s) to delete."
            )
            # for event in delete_response.data['data']:
        ...
        if move_response and move_response.success and move_response.data.get("data"):
            self.logger.info(
                f"Found {len(move_response.data['data'])} page(s) to move."
            )
            self.logger.warning("!! method not implemented yet !!")
    ...
    async def _handle_page_upsert_event(
        self, event: dict, roles_details: dict[int, dict], users: list[AppUser]
    ) -> None:
        ...
        if not page_response.success or not page_response.data:
            self.logger.warning(
                f"Could not fetch details for page ID {page_id}. Skipping."
            )
            return
        ...
        if not json_content_str:
            self.logger.warning(
                f"API response for page ID {page_id} is empty. Skipping."
            )
            return
        ...
        try:
            ...
            if not pages_list:
                self.logger.warning(
                    f"No page data found in the response for ID {page_id}. Skipping."
                )
                return
            ...
            self.logger.error(
                f"Failed to decode JSON for page ID {page_id}. Content: {json_content_str}"
            )
            return

        # Process the page to determine if it's new or updated
        record_update = await self._process_bookstack_page(
            page_details, roles_details, users
        )
        ...
        if record_update.is_new:
            self.logger.info(
                f"New record detected from event: {record_update.record.record_name}"
            )
            new_record_batch = [
                (record_update.record, record_update.new_permissions or [])
            ]
    ...
    async def run_incremental_sync(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error in BookStack incremental sync: {ex}", exc_info=True
            )
            raise
