# Folder Sync (server-side / backend-visible path)

Folder Sync ingests files from a **local directory on the host where the Python connector service runs**. The web app stores `sync_root_path` in connector config; **`run_sync`** walks that path and creates graph records, then publishes **`record-events`** (`newRecord`) for the indexing pipeline.
...
1. Create or open a **personal** Folder Sync connector instance.
...
4. Run a **manual sync** — Folder Sync uses **MANUAL** strategy; turning “Active” on does **not** crawl the folder by itself.
...
- **Logs**: connector logs show `Folder Sync: finished sync from ...`; indexing logs should show consumption of `newRecord` events for those record IDs.
...
The app’s **Socket.IO** service is used for **real-time notifications** (e.g. upload/processing progress) with a normal user JWT. It is **not** the transport for folder crawl or file bytes. Folder Sync does **not** require WebSocket for ingestion on a backend-visible path.
...
2. **`pipeshub setup`** — pick or create a personal Folder Sync instance and **PUT** `sync_root_path` / `include_subfolders` (writes `daemon.json` locally).
