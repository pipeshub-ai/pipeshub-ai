...
class EventService:
    ...
    async def process_event(self, event_type: str, payload: dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Unknown {connector_name.capitalize()} connector event type: {event_type}"
            )
            return False

        except Exception as e:
            self.logger.error(
                f"Error handling {connector_name.capitalize()} connector event {event_type}: {e}",
                exc_info=True,
            )
            return False
    ...
    async def _handle_init(self, connector_name: str, payload: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not org_id:
                self.logger.error(
                    f"'orgId' is required in the payload for '{connector_name}.init' event."
                )
                return False

            self.logger.info(
                f"Initializing {connector_name} init sync service for org_id: {org_id}"
            )
            ...
            if hasattr(self.app_container, connector_key):
                getattr(self.app_container, connector_key).override(
                    providers.Object(connector)
                )
            else:
            ...
            self.logger.error(
                f"Failed to initialize event service connector {connector_name} for org_id %s: %s",
                org_id,
                e,
                exc_info=True,
            )
    ...
    async def _handle_start_sync(
        ...
        try:
            ...
            self.logger.info(
                f"Starting {connector_name} sync service for org_id: {org_id}"
            )
            connector_name = connector_name.replace(" ", "").lower()
            ...
            try:
                ...
                if connector:
                    ...
                    return True
                self.logger.error(
                    f"{connector_name.capitalize()} connector not initialized"
                )
                return False
            except Exception as e:
                self.logger.error(
                    f"Failed to get {connector_name.capitalize()} connector: {e!s}"
                )
                return False
        except Exception as e:
            self.logger.error(
                f"Failed to queue {connector_name.capitalize()} sync service start: {e!s}"
            )
            return False
