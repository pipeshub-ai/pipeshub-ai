...
class BaseArangoService:
        ...
        try:
            ...
            for edge_def in EDGE_DEFINITIONS:
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to create edge definition for {edge_def['edge_collection']}: {e!s}"
                    )
    ...
    async def reindex_single_record(
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"❌ Failed to publish reindex event: {event_error!s}"
                )
                return {
                    "success": False,
                    "code": 500,
    ...
    async def reindex_failed_connector_records(
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"❌ Failed to publish reindexFailed event: {event_error!s}"
                )
                return {
                    "success": False,
                    "code": 500,
    ...
    async def delete_record_by_external_id(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to delete record {external_id} from {connector_name}: {e!s}"
            )
            raise
    ...
    async def remove_user_access_to_record(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to remove user access {external_id} from {connector_name}: {e!s}"
            )
            raise
    ...
    async def _get_kb_context_for_record(self, record_id: str) -> dict | None:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to get KB context for record {record_id}: {e!s}"
            )
            return None
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to publish KB deletion event: {event_error!s}"
                    )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to publish Drive deletion event: {event_error!s}"
                    )
    ...
    async def _delete_drive_specific_edges(self, transaction, record_id: str) -> None:
        ...
        for edge_collection in drive_edge_collections:
            try:
                ...
                self.logger.error(
                    f"❌ Failed to delete edges from {edge_collection}: {e!s}"
                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to publish Gmail deletion event: {event_error!s}"
                    )
    ...
    async def _create_deleted_record_event_payload(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to create deleted record event payload: {e!s}"
            )
            return {}
            ...
            if record.get("origin") == OriginTypes.UPLOAD.value:
                ...
                if record.get("recordType") == "MAIL":
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to decode file content as UTF-8: {decode_error!s}"
                        )
    ...
    async def _publish_kb_deletion_event(
        self, record: dict, file_record: dict | None
    ) -> None:
    ...
    async def _publish_gmail_deletion_event(
        self, record: dict, mail_record: dict | None, file_record: dict | None
    ) -> None:
    ...
    async def get_record_by_path(
        ...
        try:
            ...
            if result:
                self.logger.info(
                    "✅ Successfully retrieved file record for path: %s",
                    path,
                )
            ...
            self.logger.error(
                "❌ Failed to retrieve record for path %s: %s",
                path,
                str(e),
            )
            return None
    ...
    async def get_record_by_id(
        ...
        try:
            self.logger.info(
                "🚀 Retrieving internal key for id %s",
                id,
            )
            ...
            if result:
                self.logger.info(
                    "✅ Successfully retrieved internal key for id %s",
                    id,
                )
                return Record.from_arango_base_record(result)
            self.logger.warning(
                "⚠️ No internal key found for id %s",
                id,
            )
            return None
            ...
            self.logger.error(
                "❌ Failed to retrieve internal key for id %s: %s",
                id,
                str(e),
            )
            return None
    ...
    async def get_user_by_email(
        ...
        try:
            self.logger.info(
                "🚀 Retrieving internal key for email %s",
                email,
            )
            query = f"""
            ...
            if result:
                self.logger.info(
                    "✅ Successfully retrieved internal key for email %s",
                    email,
                )
                return User.from_arango_user(result)
            self.logger.warning(
                "⚠️ No internal key found for email %s",
                email,
            )
            return None
        except Exception as e:
            self.logger.error(
                "❌ Failed to retrieve internal key for email %s: %s",
                email,
                str(e),
            )
            return None
    ...
    async def get_app_user_by_email(
        ...
        try:
            self.logger.info(
                "🚀 Retrieving user for email %s and app %s",
                email,
                app_name,
            )
            ...
            if result:
                self.logger.info(
                    "✅ Successfully retrieved user for email %s and app %s",
                    email,
                    app_name,
                )
                return AppUser.from_arango_user(result)
            self.logger.warning(
                "⚠️ No user found for email %s and app %s", email, app_name
            )
            return None
        except Exception as e:
            self.logger.error(
                "❌ Failed to retrieve user for email %s and app %s: %s",
                email,
                app_name,
                str(e),
            )
    ...
    async def get_user_by_source_id(
        ...
        try:
            ...
            if user_doc:
                self.logger.info(
                    "✅ Successfully retrieved user by source_id %s", source_user_id
                )
    ...
    async def get_app_users(self, org_id, app_name: Connectors) -> list[dict]:
        ...
        try:
            ...
            users = [
                AppUser.from_arango_user(user_data) for user_data in user_data_list
            ]
            self.logger.info(
                f"✅ Successfully fetched {len(users)} users for {app_name.value}"
            )
            return users
    ...
    async def get_user_groups(
        ...
        try:
            self.logger.info(
                "🚀 Retrieving user groups for connector %s and org %s",
                app_name.value,
                org_id,
            )
            ...
            groups = [
                AppUserGroup.from_arango_base_user_group(group_data)
                for group_data in cursor
            ]

            self.logger.info(
                "✅ Successfully retrieved %d user groups for connector %s",
                len(groups),
                app_name.value,
            )
            return groups
            ...
            self.logger.error(
                "❌ Failed to retrieve user groups for connector %s: %s",
                app_name.value,
                str(e),
            )
            return []
    ...
    async def get_sync_point(
        self, key: str, collection: str, transaction: TransactionDatabase | None = None
    ) -> dict | None:
        """Get a node by key"""
        try:
    ...
    async def get_all_documents(
        self, collection: str, transaction: TransactionDatabase | None = None
    ) -> list[dict]:
    ...
    async def get_app_by_name(
        self, name: str, transaction: TransactionDatabase | None = None
    ) -> dict | None:
    ...
    async def delete_edges_to_groups(
        ...
        try:
            self.logger.info(
                "🚀 Deleting edges from %s to groups collection in %s",
                from_key,
                collection,
            )
            ...
            if count > 0:
                self.logger.info(
                    "✅ Successfully deleted %d edges from %s to groups",
                    count,
                    from_key,
                )
            else:
                self.logger.warning(
                    "⚠️ No edges found from %s to groups in collection: %s",
                    from_key,
                    collection,
                )
            ...
            self.logger.error(
                "❌ Failed to delete edges from %s to groups in %s: %s",
                from_key,
                collection,
                str(e),
            )
    ...
    async def get_file_parents(self, file_key: str, transaction) -> list[dict]:
        try:
            ...
            self.logger.error(
                "❌ Error getting parents for record %s: %s",
                file_key,
                str(e),
            )
            return []
    ...
    async def get_entity_id_by_email(
        ...
        try:
            ...
            self.logger.error(
                "❌ Failed to get entity ID for email %s: %s",
                email,
                str(e),
            )
            return None
    ...
    async def bulk_get_entity_ids_by_email(
        ...
        try:
            ...
            try:
                users = list(
                    db.aql.execute(user_query, bind_vars={"emails": unique_emails})
                )
                for user in users:
            ...
            if remaining_emails:
                ...
                try:
                    groups = list(
                        db.aql.execute(
                            group_query, bind_vars={"emails": remaining_emails}
                        )
                    )
                    for group in groups:
            ...
            if remaining_emails:
                ...
                try:
                    people = list(
                        db.aql.execute(
                            people_query, bind_vars={"emails": remaining_emails}
                        )
                    )
    ...
    async def store_permission(
        ...
        try:
            self.logger.info(
                "🚀 Storing permission for file %s and entity %s",
                file_key,
                entity_key,
            )
            ...
            try:
                ...
                if not existing_edge:
                    ...
                    self.logger.info(
                        "✅ No update needed for permission edge: %s",
                        edge_key,
                    )
    ...
    async def store_membership(
        ...
        try:
            self.logger.info(
                "🚀 Storing membership for group %s and user %s",
                group_id,
                user_id,
            )
            edge = {
    ...
    async def process_file_permissions(
        ...
        try:
            ...
            if permissions_to_remove:
                self.logger.info(
                    "🗑️ Removing %d obsolete permissions",
            ...
            self.logger.info(
                "✅ Successfully processed all permissions for file %s",
                file_key,
            )
            return True
    ...
    async def get_file_access_history(
        ...
        try:
            self.logger.info(
                "🚀 Getting historical access information for file %s",
                file_id,
            )
            query = """
            FOR perm IN permissions
    ...
    async def delete_records_and_relations(
        ...
        try:
            ...
            if not record:
                self.logger.warning(
                    "⚠️ Record %s not found in Records collection",
                    node_key,
                )
                return False
    ...
    async def save_to_people_collection(
        self, entity_id: str, email: str
    ) -> dict | None:
        """Save an entity to the people collection if it doesn't already exist"""
        try:
            self.logger.info(
                "🚀 Checking if entity %s exists in people collection",
                entity_id,
            )
            ...
            self.logger.info(
                "⏩ Entity %s already exists in people collection",
                entity_id,
            )
            return exists[0]
        except Exception as e:
    ...
    async def get_key_by_external_file_id(
        ...
        try:
            self.logger.info(
                "🚀 Retrieving internal key for external file ID %s",
                external_file_id,
            )
            ...
            if result:
                ...
                return result
            self.logger.warning(
                "⚠️ No internal key found for external file ID %s",
                external_file_id,
            )
            return None
    ...
    async def get_key_by_attachment_id(
        ...
        try:
            ...
            cursor = db.aql.execute(
                query,
                bind_vars={"external_attachment_id": external_attachment_id},
            )
            result = next(cursor, None)
    ...
    async def update_user_sync_state(
        ...
        try:
            ...
            self.logger.error(
                "❌ Failed to update user %s sync state: %s",
                service_type,
                str(e),
            )
            return None
    ...
    async def get_user_sync_state(
        ...
        try:
            self.logger.info(
                "🔍 Getting %s sync state for user %s",
                service_type,
                user_email,
            )
            ...
            self.logger.error(
                "❌ Failed to get user %s sync state: %s",
                service_type,
                str(e),
            )
            return None
    ...
    async def update_drive_sync_state(
        ...
        try:
            self.logger.info(
                "🚀 Updating sync state for drive %s to %s",
                drive_id,
                state,
            )
    ...
    async def get_drive_sync_state(self, drive_id: str) -> str | None:
        ...
        try:
            ...
            if result:
                self.logger.debug(
                    "✅ Found sync state for drive %s: %s",
                    drive_id,
                    result[0],
                )
                return result[0]
            ...
            self.logger.debug(
                "No sync state found for drive %s, assuming NOT_STARTED",
                drive_id,
            )
            return "NOT_STARTED"
        ...
        try:
            ...
            for file_data in created_files_data:
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to publish event for record: {event_error!s}"
                    )
                    failed_events += 1
            ...
            self.logger.error(
                f"❌ Critical error in event publishing for KB {kb_id}: {e!s}",
                exc_info=True,
            )
    ...
    async def _create_records(
        ...
        for folder_id, folder_file_list in folder_files.items():
            try:
                ...
                self.logger.error(
                    f"❌ Failed to create files in subfolder {folder_id}: {e!s}"
                )
            ...
            try:
                ...
                if creation_result["total_created"] > 0 or len(folder_map) > 0:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"❌ Event publishing failed (records still created): {event_error!s}"
                        )
                ...
                if transaction:
                    try:
                        ...
                        self.logger.error(
                            f"❌ Failed to abort transaction: {abort_error!s}"
                        )
    ...
    async def get_user_kb_permission(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to validate knowledge base permission for user {user_id}: {e!s}"
            )
            raise
    ...
    async def list_user_knowledge_bases(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to list knowledge bases with pagination: {e!s}"
            )
            return (
                [],
                0,
    ...
    async def get_folder_record_by_id(
        self, folder_id: str, transaction: TransactionDatabase | None = None
    ) -> dict | None:
        try:
            db = transaction if transaction else self.db
            query = """
            ...
            try:
                ...
                if should_commit:
                    try:
                        ...
                        self.logger.error(
                            f"❌ Transaction abort failed: {abort_error!s}"
                        )
                raise inner_error
    ...
    async def validate_record_in_folder(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to validate record {record_id} in folder {folder_id}: {e!s}"
            )
            return False

    async def validate_record_in_kb(
        self, kb_id: str, record_id: str, transaction: TransactionDatabase | None = None
    ) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to validate record {record_id} in KB {kb_id}: {e!s}"
            )
            return False
            ...
            try:
                ...
                if file_metadata:
                    ...
                    original_name = self._normalize_name(
                        file_metadata.get("originalname", "")
                    )
                ...
                if file_metadata and current_file_record:
                    ...
                    if "originalname" in file_metadata:
                        file_updates["name"] = self._normalize_name(
                    ...
                    if file_updates:
                        ...
                        try:
                            ...
                            self.logger.error(
                                f"❌ Failed to update file metadata: {file_error!s}"
                            )
                ...
                if should_commit:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"❌ Transaction commit failed: {commit_error!s}"
                        )
                        try:
                            await asyncio.to_thread(
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to publish update event: {event_error!s}"
                    )
                ...
                self.logger.error(
                    f"❌ Database error during record update: {db_error!s}"
                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to publish delete events: {event_error!s}"
                    )
                    # Don't fail the main operation for event publishing errors
                ...
                self.logger.error(
                    f"❌ Database error during bulk deletion: {db_error!s}"
                )
                if should_commit and transaction:
                    try:
                        await asyncio.to_thread(lambda: transaction.abort_transaction())
    ...
    async def update_kb_permission(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to update KB permission optimistically: {e!s}"
            )
            return {
                "success": False,
                "reason": str(e),
    ...
    async def get_kb_children(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to get KB children with folders_first pagination: {e!s}"
            )
    ...
    async def get_folder_children(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to get folder children with folders_first pagination: {e!s}"
            )
            return {"success": False, "reason": str(e)}
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to publish KB deletion events: {event_error!s}"
                    )
                    # Don't fail the main operation for event publishing errors
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ Failed to publish folder deletion events: {event_error!s}"
                    )
                    # Don't fail the main operation for event publishing errors
                ...
                self.logger.error(
                    f"❌ Database error during folder deletion: {db_error!s}"
                )
                if should_commit and transaction:
                    try:
                        await asyncio.to_thread(lambda: transaction.abort_transaction())
            ...
            self.logger.error(
                f"❌ Failed to delete folder {folder_id} completely: {e!s}"
            )
            return False
    ...
    async def get_records_by_virtual_record_id(
        ...
        try:
            self.logger.info(
                "🔍 Finding records with virtualRecordId: %s",
                virtual_record_id,
            )
    ...
    async def get_file_record_by_id(
        ...
        try:
            ...
            self.logger.error(
                "❌ Failed to retrieve file record for id %s: %s",
                id,
                str(e),
            )
            return None
