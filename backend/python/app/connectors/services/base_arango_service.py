...
class BaseArangoService:
    ...
    async def get_connector_stats(
        ...
        try:
            ...
            if result:
                connector_display = result["connector"]["name"]
    ...
    async def get_records(
        ...
        try:
            ...
            LET allNewPermissionKeys = APPEND(connectorRecordsNewPermission, groupConnectorRecordsNewPermission)
            LET uniqueNewPermissionKeys = UNIQUE(allNewPermissionKeys)

            LET mergeRecords = APPEND(kbRecords, connectorRecords)

            // Get unique records from new permission
            LET uniqueNewPermissionRecords = (
                FOR key IN uniqueNewPermissionKeys
                    LET record = DOCUMENT(CONCAT("records/", key))
                    FILTER record != null
                    RETURN record
            )

            LET allRecords = APPEND(mergeRecords, uniqueNewPermissionRecords)

            // Filter out records where the associated file has isFile == false
            LET filteredCount = LENGTH(
