...
class BaseArangoService:
    ...
    async def _delete_gmail_specific_edges(self, transaction, record_id: str) -> None:
        ...
        edge_deletion_strategies = {
            ...
            CollectionNames.RECORD_RELATIONS.value: {
                "filter": "(edge._from == @record_from OR edge._to == @record_to) AND edge.relationshipType IN @relation_types",
                "bind_vars": {
                    "record_from": f"records/{record_id}",
                    "record_to": f"records/{record_id}",
    ...
    async def batch_upsert_record_relations(
        ...
        relationship types (e.g., FOREIGN_KEY and DEPENDS_ON) or different constraint names
