...
async def get_drive_webhook_handler(
    request: Request,
) -> AbstractDriveWebhookHandler | None:
    try:
        container: ConnectorAppContainer = request.app.container
...
async def get_gmail_webhook_handler(
    request: Request,
) -> AbstractGmailWebhookHandler | None:
    try:
        container: ConnectorAppContainer = request.app.container
...
async def handle_record_deletion(
    record_id: str,
    arango_service=Depends(Provide[ConnectorAppContainer.arango_service]),
) -> dict | None:
    try:
...
async def stream_record_internal(
    request: Request,
    record_id: str,
    arango_service: BaseArangoService = Depends(
        Provide[ConnectorAppContainer.arango_service]
    ),
    ...
    try:
        ...
        container: ConnectorAppContainer = request.app.container
        if (
            connector_name == Connectors.KNOWLEDGE_BASE.value.lower()
        ...
        raise HTTPException(
            status_code=HttpStatusCode.BAD_REQUEST.value, detail="Invalid token payload"
        )
    except Exception as e:
...
@router.get(
    "/api/v1/index/{org_id}/{connector}/record/{record_id}", response_model=None
)
@inject
async def download_file(
    ...
    google_token_handler: GoogleTokenHandler = Depends(
        Provide[ConnectorAppContainer.google_token_handler]
    ),
    config_service: ConfigurationService = Depends(
        Provide[ConnectorAppContainer.config_service]
    ),
    ...
    try:
        ...
        if payload.record_id != record_id:
            logger.error(
                f"""Token does not match requested file: {payload.record_id} != {
                    record_id
                }""",
            )
            raise HTTPException(
                status_code=HttpStatusCode.UNAUTHORIZED.value,
                detail="Token does not match requested file",
            )
        ...
        creds = None
        if (
            connector.lower() == Connectors.GOOGLE_DRIVE.value.lower()
            or connector.lower() == Connectors.GOOGLE_MAIL.value.lower()
        ):
            if org["accountType"] in [
                AccountType.ENTERPRISE.value,
                AccountType.BUSINESS.value,
            ]:
                # Use service account credentials
                creds = await get_service_account_credentials(
                    org_id,
                    user_id,
                    logger,
                    arango_service,
                ...
                async def attachment_stream() -> AsyncGenerator[bytes, None]:
                    try:
                        ...
                        if messages and messages[0]:
                            ...
                            logger.warning(
                                "Related message not found, returning empty buffer"
                            )
                            yield b""
                            return
                        ...
                        try:
                            ...
                            if "_" in file_id:
                                try:
                                    ...
                                    try:
                                        ...
                                        if (
                                            hasattr(access_error, "resp")
                                            and access_error.resp.status
                                            == HttpStatusCode.NOT_FOUND.value
                                        ):
                                            logger.info(
                                                f"Message not found with ID {message_id}, searching for related messages..."
                                            )

                                            # Get messageIdHeader from the original mail
                                            file_key = await arango_service.get_key_by_external_message_id(
                                                message_id
                                            )
                                            aql_query = """
                                            FOR mail IN mails
                                            ...
                                            for related_key in related_mail_keys:
                                                related_mail = (
                                                    await arango_service.get_document(
                                                        related_key,
                                                        CollectionNames.RECORDS.value,
                                                    )
                                                )
                                                related_message_id = related_mail.get(
                                                    "externalRecordId"
                                                )
                                                try:
                                                    message = (
                                                    ...
                                                    if message:
                                                        logger.info(
                                                            f"Found accessible message with ID: {related_message_id}"
                                                        )
                                                        message_id = related_message_id  # Update message_id to use the accessible one
                                                        break
                                                except Exception as e:
                                                    logger.warning(
                                                        f"Failed to fetch message with ID {related_message_id}: {e!s}"
                                                    )
                                                    continue
                                    ...
                                    for part in parts:
                                        if part.get("partId") == part_id:
                                            ...
                                            logger.info(
                                                f"Found attachment ID: {actual_attachment_id}"
                                            )
                                            break
                                    else:
                                    ...
                                    logger.error(
                                        f"Error extracting attachment ID: {e!s}"
                                    )
                                    raise HTTPException(
                                        status_code=HttpStatusCode.BAD_REQUEST.value,
                            ...
                            try:
                                ...
                                if (
                                    hasattr(attachment_error, "resp")
                                    and attachment_error.resp.status
                                    == HttpStatusCode.NOT_FOUND.value
                                ):
                                    raise HTTPException(
                                        status_code=HttpStatusCode.NOT_FOUND.value,
                    ...
                    try:
                        ...
                        try:
                            ...
                            if (
                                ...
                                logger.info(
                                    f"Message not found with ID {file_id}, searching for related messages..."
                                )

                                # Get messageIdHeader from the original mail
                                file_key = (
                                    await arango_service.get_key_by_external_message_id(
                                        file_id
                                    )
                                )
                                aql_query = """
                                ...
                                message = None
                                for related_key in related_mail_keys:
                                    related_mail = await arango_service.get_document(
                                        related_key, CollectionNames.RECORDS.value
                                    )
                                    ...
                                    try:
                                        ...
                                        if message:
                                            logger.info(
                                                f"Found accessible message with ID: {related_id}"
                                            )
                                            break
                                    except Exception as e:
                                        logger.warning(
                                            f"Failed to fetch message with ID {related_id}: {e!s}"
                                        )
                                        continue
                ...
                try:
                    ...
                    if "_" in file_id:
                        try:
                            ...
                            for part in parts:
                                if part.get("partId") == part_id:
                                    actual_attachment_id = part.get("body", {}).get(
                                        "attachmentId"
                                    )
                                    if not actual_attachment_id:
                                        raise Exception(
...
async def handle_admin_webhook(
    ...
    try:
        ...
        if not events:
            raise HTTPException(
                status_code=HttpStatusCode.BAD_REQUEST.value,
                detail="No events found in webhook body",
            )
        ...
        if not event_type:
            raise HTTPException(
                status_code=HttpStatusCode.BAD_REQUEST.value,
                detail="Missing event name in webhook body",
            )
...
async def get_service_account_credentials(
    ...
    try:
        ...
        async with service_creds_lock:
            ...
            logger.info(
                f"Service account cache miss: {cache_key}. Creating new credentials."
            )
            ...
            credentials_json = await google_token_handler.get_enterprise_token(
                org_id, app_name=connector
            )
...
async def get_user_credentials(
    org_id: str, user_id: str, logger, google_token_handler, container, connector: str
) -> google.oauth2.credentials.Credentials:
    ...
    try:
        ...
        async with user_creds_lock:
            ...
            if cache_key in container.user_creds_cache:
                ...
                try:
                    ...
                    if expiry and (expiry - buffer_time) > now:
                        ...
                        return creds
                    logger.info(
                        f"User credentials expired or expiring soon for {cache_key}"
                    )
            ...
            logger.info(
                f"User credentials cache miss: {cache_key}. Creating new credentials."
            )

            # Create new credentials
            SCOPES = await google_token_handler.get_account_scopes(app_name=connector)
            # Refresh token
            await google_token_handler.refresh_token(
                org_id, user_id, app_name=connector
            )
            ...
            token_expiry = datetime.fromtimestamp(
                ...
                tzinfo=None
            )  # Convert to naive UTC for Google client compatibility
            new_creds.expiry = token_expiry

            # Cache the credentials
            container.user_creds_cache[cache_key] = new_creds
            logger.info(
                f"Cached new user credentials for {cache_key} with expiry: {new_creds.expiry}"
            )
...
async def get_records(
    ...
    record_types: str | None = Query(
        None, description="Comma-separated list of record types"
    ),
    origins: str | None = Query(None, description="Comma-separated list of origins"),
    connectors: str | None = Query(
        None, description="Comma-separated list of connectors"
    ),
    indexing_status: str | None = Query(
        None, description="Comma-separated list of indexing statuses"
    ),
    permissions: str | None = Query(
        None, description="Comma-separated list of permissions"
    ),
...
async def reindex_failed_records(
    ...
    try:
        ...
        logger.info(
            f"🔄 Attempting to reindex failed {request_body.get('connector')} records"
        )
        ...
        if result["success"]:
            logger.info(
                f"✅ Successfully initiated reindex for failed {request_body.get('connector')} records"
            )
            return {
                "success": True,
...
async def get_connector_config(
    ...
    try:
        ...
        if not registry_entry:
            raise HTTPException(
                status_code=404, detail=f"Connector {app_name} not found in registry"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to load config from etcd for {app_name}",
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get connector config for {app_name}"
        )
...
async def get_connector_schema(
    ...
    connector_registry = request.app.state.connector_registry
    result: dict[str, Any] | None = await connector_registry.get_connector_by_name(
        app_name
    )
    if not result:
...
async def get_oauth_authorization_url(
    ...
    try:
        ...
        if not registry_entry:
            raise HTTPException(
                status_code=404, detail=f"Connector {app_name} not found"
            )

        # Check if it's an OAuth connector
        if (registry_entry.get("authType") or "").upper() not in [
            "OAUTH",
            "OAUTH_ADMIN_CONSENT",
        ]:
            raise HTTPException(
                status_code=400, detail=f"Connector {app_name} does not support OAuth"
            )
        ...
        if not config or not config.get("auth"):
            raise HTTPException(
                status_code=400, detail=f"OAuth configuration not found for {app_name}"
            )
        ...
        authorize_url = auth_config.get("authorizeUrl") or connector_auth_config.get(
            "authorizeUrl", ""
        )
        token_url = auth_config.get("tokenUrl") or connector_auth_config.get(
            "tokenUrl", ""
        )

        if not redirect_uri:
            raise HTTPException(
                status_code=400, detail=f"Redirect URI not configured for {app_name}"
            )

        if not authorize_url or not token_url:
            raise HTTPException(
                status_code=400, detail=f"OAuth URLs not configured for {app_name}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate OAuth URL: {e!s}"
        )
...
async def handle_oauth_callback(
    ...
    try:
        ...
        async def _get_settings_base_path() -> str:
            ...
            """
            try:
                orgs = await arango_service.get_all_documents(
        ...
        if error:
            ...
            return {
                "success": False,
                "error": error,
                "redirect_url": f"{base_url}/connectors/oauth/callback/{connector_name}?oauth_error={error}",
            }
        ...
        if not code or not state:
            ...
            return {
                ...
                "redirect_url": f"{base_url}/connectors/oauth/callback/{connector_name}?oauth_error=missing_parameters",
            }
        ...
        if not registry_entry:
            ...
            return {
                ...
                "redirect_url": f"{base_url}/connectors/oauth/callback/{connector_name}?oauth_error=connector_not_found",
            }
        ...
        if not config or not config.get("auth"):
            ...
            return {
                ...
                "redirect_url": f"{base_url}/connectors/oauth/callback/{connector_name}?oauth_error=config_not_found",
            }
        ...
        if not redirect_uri:
            return {
                ...
                "redirect_url": f"{base_url}/connectors/oauth/callback/{connector_name}?oauth_error=redirect_uri_not_configured",
            }

        if not authorize_url or not token_url:
            return {
                "success": False,
                "error": "oauth_urls_not_configured",
                "redirect_url": f"{base_url}/connectors/oauth/callback/{connector_name}?oauth_error=oauth_urls_not_configured",
            }
        ...
        if not token or not token.access_token:
            ...
            return {
                "success": False,
                "error": "invalid_token",
                "redirect_url": f"{base_url}/connectors/oauth/callback/{connector_name}?oauth_error=invalid_token",
            }
        ...
        try:
            ...
            updated_config = await kv_store.get_key(
                f"/services/connectors/{filtered_app_name}/config"
            )
            if isinstance(updated_config, dict):
                await config_service.set_config(
                    f"/services/connectors/{filtered_app_name}/config", updated_config
                )
                logger.info(
                    f"Refreshed config cache for {app_name} after OAuth callback"
                )
        except Exception as cache_err:
            logger.warning(
                f"Could not refresh config cache for {app_name}: {cache_err}"
            )
        ...
        try:
            ...
            refresh_service = TokenRefreshService(
                container.key_value_store(), arango_service
            )
            await refresh_service.schedule_token_refresh(
                _sanitize_app_name(app_name), token
            )
            logger.info(f"Scheduled token refresh for {app_name}")
        except Exception as sched_err:
            logger.warning(
                f"Could not schedule token refresh for {app_name}: {sched_err}"
            )
        ...
        try:
            ...
            account_type = (
                str((orgs[0] or {}).get("accountType", "")).lower()
            ...
            settings_base_path = (
                ...
                if account_type in ["business", "organization", "enterprise"]
                else "/account/individual/settings/connector"
            )
        except Exception:
        ...
        await connector_registry.update_connector(app_name, updates)
        logger.info(
            f"Connector 'isAuthenticated' status for {app_name} set to False due to an error."
        )
        return {"success": False, "error": "server_error", "redirect_url": error_url}


async def get_connector_filter_options_from_config(
    app_name: str,
    connector_config: dict[str, Any],
    token_or_credentials: OAuthToken | dict[str, Any],
    config_service,
) -> dict[str, Any]:
    ...
    try:
        ...
        filter_endpoints = (
            connector_config.get("config", {}).get("filters", {}).get("endpoints", {})
        )
            ...
            try:
                if endpoint == "static":
                    ...
                    filter_options[filter_type] = await _get_static_filter_options(
                        app_name, filter_type
                    )
                else:
                    # Call dynamic API endpoint
                    options = await _fetch_filter_options_from_api(
                        endpoint, filter_type, token_or_credentials, app_name
                    )
                    if options:
...
def _parse_filter_response(
    data: dict[str, Any], filter_type: str, app_name: str
) -> list[dict[str, str]]:
    ...
    try:
        if app_name.upper() == "GMAIL" and filter_type == "labels":
            ...
            for label in labels:
                if (
                    label.get("type") == "user"
                ):  # Only user-created labels, not system labels
                    options.append(
                        {
                            "value": label["id"],
                            "label": label["name"],
...
async def get_connector_filters(
    ...
    try:
        ...
        connector_registry = request.app.state.connector_registry
        connector_config = await connector_registry.get_connector_by_name(
            app_name_upper
        )
        if not connector_config:
            raise HTTPException(
                status_code=404, detail=f"Connector {app_name_upper} not found"
            )
        ...
        if auth_type == "OAUTH":
            ...
            if not config or not config.get("credentials"):
                raise HTTPException(
                    status_code=400,
                    detail=f"OAuth credentials not found for {app_name}. Please authenticate first.",
                )
            ...
            if not config or not config.get("auth"):
                raise HTTPException(
                    status_code=400,
                    detail=f"Connector configuration not found for {app_name}. Please configure first.",
                )
            ...
            if not config or not config.get("auth"):
                raise HTTPException(
                    status_code=400,
                    detail=f"API token configuration not found for {app_name}. Please configure first.",
                )
            token_or_credentials = config.get("auth", {})
            ...
            if not config or not config.get("auth"):
                raise HTTPException(
                    status_code=400,
                    detail=f"Authentication configuration not found for {app_name}. Please configure first.",
                )
            token_or_credentials = config.get("auth", {})

        else:
            raise HTTPException(
                status_code=400, detail=f"Unsupported authentication type: {auth_type}"
            )

        # Get filter options from configured endpoints
        filter_options = await get_connector_filter_options_from_config(
            app_name, connector_config, token_or_credentials, config_service
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get filter options: {e!s}"
        )
...
async def save_connector_filters(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to save filter selections: {e!s}"
        )
...
async def update_connector_config(
    ...
    try:
        ...
        for section in ["auth", "sync", "filters"]:
            if (
                section in body_dict and isinstance(body_dict[section], dict)
            ) or section in body_dict:
        ...
        try:
            ...
            logger.info(
                f"App created in database for {app_name} (if not already exists)"
            )
        except Exception as e:
        ...
        merged_config["auth"]["authorizeUrl"] = merged_config["auth"].get(
            "authorizeUrl"
        ) or auth_meta.get("authorizeUrl", "")
        merged_config["auth"]["tokenUrl"] = merged_config["auth"].get(
            "tokenUrl"
        ) or auth_meta.get("tokenUrl", "")
...
async def toggle_connector(
    ...
    try:
        ...
        org = await arango_service.get_document(
            user_info["orgId"], CollectionNames.ORGS.value
        )
        if not org:
        ...
        try:
            if not current_status:  # enabling
                ...
                custom_google_business_logic = (
                    org_account_type == "enterprise"
                    and app_name.upper() in ["GMAIL", "DRIVE"]
                )
                if auth_type == "OAUTH":
                    if custom_google_business_logic:
                        auth_creds = cfg.get("auth", {})
                        if not auth_creds or not (
                            auth_creds.get("client_id") and auth_creds.get("adminEmail")
                        ):
                            logger.error(
                                f"Connector {app_name} cannot be enabled until OAuth authentication is completed"
                            )
                            raise HTTPException(
                                status_code=HttpStatusCode.BAD_REQUEST.value,
                    ...
                    if not app.get("isConfigured", False):
                        logger.error(
                            f"Connector {app_name} must be configured before enabling"
                        )
                        raise HTTPException(
                            status_code=HttpStatusCode.BAD_REQUEST.value,
            ...
            logger.error(
                f"Failed to validate enable preconditions for {app_name}: {prereq_err}"
            )
            raise HTTPException(
                status_code=500, detail="Failed to validate connector state"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to update connector {app_name}"
            )
        ...
        return {
            "success": True,
            "message": f"Connector {app_name} toggled successfully",
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to toggle connector {app_name}"
        )
