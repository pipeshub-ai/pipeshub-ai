...
class Neo4jProvider(IGraphDBProvider):
                    ...
                    AND (
                        ...
                        (
                            ...
                            AND (
                                ...
                                OR ('RecordGroup' IN labels(c) AND EXISTS {{ (u)-[:USER_APP_RELATION]->(:App {{id: $appId}})<-[:INHERIT_PERMISSIONS|BELONGS_TO]-(c) }})
                            )
                        )
                        OR
    ...
    async def get_knowledge_hub_search_v3(
        ...
        try:
            ...
            if parent_id:
                # Get connector context from parent
                parent_connector_query = """
                MATCH (n {id: $parentId})
                RETURN n.connectorId AS connectorId
                LIMIT 1
                """
                parent_conn_result = await self.client.execute_query(
                    parent_connector_query,
                    parameters={"parentId": parent_id},
                    txn_id=transaction
                )

                if parent_conn_result and parent_conn_result[0]:
                    app_id = parent_conn_result[0].get("connectorId")
                else:
                    app_id = None
            else:
    ...
    def _build_filter_where_clause_v3(
        ...
        if record_types:
            conditions.append(f"({node_var}.nodeType IN ['record', 'folder'] AND {node_var}.recordType IN $recordTypes)")
            params["recordTypes"] = record_types

        if indexing_status:
            conditions.append(f"({node_var}.nodeType IN ['record', 'folder'] AND {node_var}.indexingStatus IN $indexingStatus)")
...
# ======================================================= #
    ...
    def _build_permission_qpp_core(
        ...
        Required query parameters:
            ...
            (App-linked RecordGroup branch uses USER_APP_RELATION to (:App {id: $appId}), not $hasAppRelation.)
                    ...
                    AND (
                        ...
                        (
                            ...
                            AND (
                                ...
                                OR (
                                    'RecordGroup' IN labels({child_var})
                                    AND EXISTS {{ ({user_var})-[:USER_APP_RELATION]->(:App {{id: $appId}})<-[:INHERIT_PERMISSIONS|BELONGS_TO]-({child_var}) }}
                                )
                            )
                        )
                        OR
