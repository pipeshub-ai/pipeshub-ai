...
class ArangoService(IGraphService):
    def __init__(
        self, logger: Logger, config_service: ArangoConfig | ConfigurationService
    ) -> None:
        self.logger = logger
    ...
    @classmethod
    async def create(
        cls, logger: Logger, config_service: ArangoConfig | ConfigurationService
    ) -> "ArangoService":
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not sys_db.has_database(arango_db):
                self.logger.info(
                    f"Database '{arango_db}' does not exist, creating it..."
                )
    ...
    async def get_edge(
        self, edge_type: str, from_node: str, to_node: str
    ) -> Edge | None:
        """Get an edge"""
    ...
    async def upsert_document(
        ...
        try:
            ...
            if result is not None:
                ...
                return True
            self.logger.error(
                f"Upsert operation failed for document {document['_key']} in {collection_name}"
            )
            return False
    ...
    async def upsert_document_with_merge(
        ...
        try:
            ...
            if result is not None and len(result) > 0:
                self.logger.debug(
                    f"Upserted document {document['_key']} in {collection_name} with strategy '{merge_strategy}'"
                )
    ...
    async def batch_upsert_documents(
        ...
        try:
            ...
            if result is not None:
                self.logger.debug(
                    f"Batch upserted {len(documents)} documents in {collection_name} with strategy '{merge_strategy}'"
                )
                return True
            self.logger.error(
                f"Batch upsert operation failed for {len(documents)} documents in {collection_name}"
            )
            return False
    ...
    async def create_index(
        self, collection_name: str, fields: list[str], index_type: str = "persistent"
    ) -> bool:
