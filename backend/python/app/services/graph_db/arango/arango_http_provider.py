...
class ArangoHTTPProvider(IGraphDBProvider):
            ...
            FOR kb IN @@recordGroups_collection
                ...
                LET folders = (
                    FOR edge IN @@kb_to_folder_edges
                        ...
                        // Folders are now RECORDS documents, so edge._from should be records/folder_key
    ...
    async def list_kb_records(
        ...
        try:
            ...
            # Execute queries
            records = await self.http_client.execute_aql(main_query, bind_vars=main_bind_vars, txn_id=transaction)
            count_results = await self.http_client.execute_aql(count_query, bind_vars=count_bind_vars, txn_id=transaction)
            count = count_results[0] if count_results else 0
            available_filters_results = await self.http_client.execute_aql(filters_query, bind_vars=filters_bind_vars, txn_id=transaction)
            available_filters = available_filters_results[0] if available_filters_results else {}
    ...
    async def list_user_knowledge_bases(
        ...
        try:
            ...
            # Execute queries
            kbs = await self.http_client.execute_aql(main_query, bind_vars=main_bind_vars, txn_id=transaction)
            count_result = await self.http_client.execute_aql(count_query, bind_vars=count_bind_vars, txn_id=transaction)
            filter_data = await self.http_client.execute_aql(filters_query, bind_vars=filters_bind_vars, txn_id=transaction)

            # Get results
            kbs = list(kbs) if kbs else []
            total_count = count_result[0] if count_result else 0
            filter_data = list(filter_data) if filter_data else []
