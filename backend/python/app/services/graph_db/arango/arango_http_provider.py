        ...
        try:
            ...
            for arango_doc in raw_docs:
                ...
                if record.get("isPlaceholder"):
                    continue
                upsert_doc: dict = {"_key": rid, "indexingStatus": status}
                if status == ProgressStatus.QUEUED.value:
                    upsert_doc.update(build_indexing_progress(IndexingStage.QUEUED))
                to_upsert.append(upsert_doc)
