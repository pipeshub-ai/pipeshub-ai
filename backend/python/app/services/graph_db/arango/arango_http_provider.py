...
# KB ``parent_type`` / ``node_type`` → Arango vertex collection (generic ``id`` == ``_key``)
_KB_NODE_TYPE_TO_COLLECTION: dict[str, str] = {
    ...
    "kb": CollectionNames.RECORD_GROUPS.value,
...
class ArangoHTTPProvider(IGraphDBProvider):
    ...
    async def get_node_connector_id(
        ...
        For ``app``, returns ``node_id`` (app key is the scope id). For other mapped
        types, returns the document's ``connectorId`` when present.
        """
        if not node_id:
            return None
        if node_type == "app":
        ...
        if not node_type:
            return None
        collection = _KB_NODE_TYPE_TO_COLLECTION.get(node_type)
        if not collection:
            return None
        try:
            doc = await self.get_document(
                document_key=node_id,
                collection=collection,
                transaction=transaction,
            )
    ...
    async def _get_connector_for_parent_v2(
        ...
        """
        Get the connector ID for a given parent node (delegates to ``get_node_connector_id``).
        """
        if parent_type not in ("app", "recordGroup", "kb", "record", "folder"):
            return None
        return await self.get_node_connector_id(parent_id, parent_type, transaction)
