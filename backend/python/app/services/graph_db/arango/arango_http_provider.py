...
class ArangoHTTPProvider(IGraphDBProvider):
    ...
    async def get_user_apps(self, user_key: str) -> List[Dict]:
        """Get all apps (connectors) associated with a user: direct User->App and via User->Team->App."""
        try:
            user_from = f"{CollectionNames.USERS.value}/{user_key}"
            query = f"""
            LET user_apps = (
    ...
    async def get_knowledge_hub_filter_options(
        ...
        // Get connector apps the user has access to: direct User->App and User->Team->App
        LET user_apps_raw = (
            FOR app IN OUTBOUND user_from userAppRelation
                FILTER app != null
                RETURN app
        ...
        LET team_apps_raw = (
            FOR perm IN permission
                ...
                FOR app IN OUTBOUND perm._to userAppRelation
                    FILTER app != null
                    RETURN app
        )
        LET combined_apps = APPEND(user_apps_raw, team_apps_raw)
        LET apps = (
            FOR app IN combined_apps
                COLLECT key = app._key INTO grouped KEEP app
                LET app = grouped[0].app
