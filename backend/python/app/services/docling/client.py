...
class DoclingClient:
    ...
    def __init__(self, service_url: str | None = None, timeout: float = 2400.0) -> None:
        self.service_url = (
            service_url or os.getenv("DOCLING_SERVICE_URL", "http://localhost:8081")
        ).rstrip("/")
    ...
    async def process_pdf(
        self, record_name: str, pdf_binary: bytes, org_id: str | None = None
        ...
        if len(pdf_binary) > 100 * 1024 * 1024:  # 100MB limit
            self.logger.error(
                f"❌ PDF too large for processing: {len(pdf_binary)} bytes (max: 100MB)"
            )
            return None
        ...
        async with httpx.AsyncClient(
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    self.logger.info(
                        f"🚀 Sending PDF processing request for: {record_name} (attempt {attempt + 1}/{self.max_retries})"
                    )
                    ...
                    if response.status_code == HttpStatusCode.SUCCESS.value:
                        ...
                        self.logger.error(
                            f"❌ Docling service returned error for {record_name}: {error_msg}"
                        )
                        return None
                    self.logger.error(
                        f"❌ Docling service HTTP error {response.status_code}: {response.text}"
                    )
                    ...
                    self.logger.error(
                        f"❌ Timeout processing PDF {record_name} (attempt {attempt + 1}): {e!s}"
                    )
                    if attempt < self.max_retries - 1:
                        self.logger.info(
                            f"🔄 Retrying in {self.retry_delay} seconds..."
                    ...
                    self.logger.error(
                        f"❌ Connection error processing PDF {record_name} (attempt {attempt + 1}): {e!s}"
                    )
                    if attempt < self.max_retries - 1:
                        self.logger.info(
                            f"🔄 Retrying in {self.retry_delay} seconds..."
                    ...
                    self.logger.error(
                        f"❌ Write error processing PDF {record_name} (attempt {attempt + 1}): {e!s}"
                    )
                    if "write could not complete without blocking" in str(e):
                        self.logger.warning(
                            "⚠️ Network buffer issue detected, retrying with exponential backoff..."
                        )
                        if attempt < self.max_retries - 1:
                    ...
                    self.logger.error(
                        f"❌ Request error processing PDF {record_name} (attempt {attempt + 1}): {e!s}"
                    )
                    if attempt < self.max_retries - 1:
                        self.logger.info(
                            f"🔄 Retrying in {self.retry_delay} seconds..."
                    ...
                    self.logger.error(
                        f"❌ Unexpected error processing PDF {record_name} (attempt {attempt + 1}): {e!s}"
                    )
                    if attempt < self.max_retries - 1:
                        self.logger.info(
                            f"🔄 Retrying in {self.retry_delay} seconds..."
            ...
            self.logger.error(
                f"❌ Failed to process PDF {record_name} after {self.max_retries} attempts"
            )
            return None
    ...
    def _parse_blocks_container(self, block_containers_data) -> BlocksContainer:
        ...
        try:
            ...
            self.logger.error(f"❌ Failed to parse blocks container: {e!s}")
            raise
