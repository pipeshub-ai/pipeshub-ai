...
class EntityEventService(BaseEventService):
    ...
    async def __get_or_create_knowledge_base(
        ...
        try:
            ...
            await self.graph_provider.batch_upsert_nodes([kb_data], CollectionNames.APPS.value)
            await self.graph_provider.batch_create_edges([permission_edge], CollectionNames.PERMISSION.value)
            await self.graph_provider.batch_create_edges([org_app_edge], CollectionNames.ORG_APP_RELATION.value)
            await self.graph_provider.batch_create_edges([user_app_edge], CollectionNames.USER_APP_RELATION.value)
