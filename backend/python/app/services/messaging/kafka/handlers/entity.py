...
class EntityEventService(BaseEventService):
        ...
        try:
            ...
            if enabled_apps:
                ...
                if user_type == AccountType.ENTERPRISE.value:
                    active_users = await self.graph_provider.get_users(
                        org_id, active=True
                    )
    ...
    async def __get_or_create_knowledge_base(
        ...
        try:
            ...
            folder_edge = {
                "from_id": folder_id,
                "from_collection": CollectionNames.FILES.value,
                "to_id": kb_key,
                "to_collection": CollectionNames.RECORD_GROUPS.value,
            ...
            await self.graph_provider.batch_upsert_nodes([root_folder_data], CollectionNames.FILES.value)
            await self.graph_provider.batch_create_edges([permission_edge], CollectionNames.PERMISSION.value)
            await self.graph_provider.batch_create_edges([folder_edge], CollectionNames.BELONGS_TO.value)
