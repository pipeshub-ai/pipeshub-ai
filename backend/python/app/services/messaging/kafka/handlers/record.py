...
class RecordEventHandler(BaseEventService):
    def __init__(
        ...
        if scheduler:
            ...
            self.scheduler: Scheduler = self.__buildDefaultScheduler(
                logger, config_service
            )
        self.scheduled_update_task = asyncio.create_task(
            self.scheduler.process_scheduled_events(event_processor)
        )
        self.event_processor: EventProcessor = event_processor

    def __buildDefaultScheduler(
        ...
        return SchedulerFactory.scheduler(
            "redis", redis_url, logger, config_service, delay_hours=1
        )
    ...
    async def process_event(self, event_type: str, payload: dict) -> bool:
        ...
        try:
            ...
            if (
                (extension is None or extension == "unknown")
            ...
            if payload and payload.get("signedUrlRoute"):
                ...
                try:
                    response = await make_signed_url_api_call(
                ...
                try:
                    ...
                    response = await make_api_call(
                        route=f"{connector_url}/api/v1/internal/stream/record/{record_id}",
                        token=token,
                    )
    ...
    async def __create_scheduler(
        self, scheduler_type: str, logger: Logger, config_service: ConfigurationService
    ) -> Scheduler:
        ...
        return SchedulerFactory.scheduler(
            scheduler_type, redis_url, logger, config_service, delay_hours=1
        )
