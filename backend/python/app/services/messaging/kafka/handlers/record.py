            ...
            if error_occurred and record_id:
                ...
                if is_final:
                    ...
                    if record is not None:
                        virtual_record_id = record.get("virtualRecordId")
                        self.logger.info(f"🔄 Current record {record_id} has failed (final), triggering next queued duplicate")
                        await self._trigger_next_queued_duplicate(record_id, virtual_record_id)
                    else:
                        self.logger.warning(f"Record {record_id} not found, skipping duplicate trigger")
                else:
