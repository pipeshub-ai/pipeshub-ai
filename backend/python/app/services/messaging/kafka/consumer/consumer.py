...
class KafkaMessagingConsumer(IMessagingConsumer):
    ...
    # implementing abstract methods from IMessagingConsumer
    async def stop(self, message_handler: Optional[Callable[[Dict[str, Any]], AsyncGenerator[Dict[str, Any], None]]] = None) -> None:
                    ...
                    for topic_partition, messages in message_batch.items():
                        for message in messages:
                            try:
                                ...
                                # Always use dual semaphore processing
    ...
    async def __process_message_wrapper(self, message, topic_partition: TopicPartition) -> None:
        ...
        try:
            ...
            if parsed_message is None:
                ...
                # Mark as processed and commit offset for malformed messages to avoid infinite retry loop
            ...
            # Iterate over events yielded by the message handler
            if self.message_handler:
                ...
                # Mark as processed and commit offset on success
                self.__mark_message_processed(message_id)
                if self.consumer:
                    await self.consumer.commit({topic_partition: message.offset + 1})
