...
class KafkaUtils:
    ...
    @staticmethod
    async def create_entity_kafka_consumer_config(
        app_container: ConnectorAppContainer,
    ) -> KafkaConsumerConfig:
    ...
    @staticmethod
    async def create_sync_kafka_consumer_config(
        app_container: ConnectorAppContainer,
    ) -> KafkaConsumerConfig:
    ...
    @staticmethod
    async def create_record_kafka_consumer_config(
    ...
    @staticmethod
    async def create_aiconfig_kafka_consumer_config(
    ...
    async def create_entity_message_handler(
        ...
        async def handle_entity_message(message: dict[str, Any]) -> bool:
            ...
            try:
                if message is None:
                    logger.warning(
                        "Received a None message, likely during shutdown. Skipping."
                    )
                    return True
        ...
        async def handle_sync_message(message: dict[str, Any]) -> bool:
            ...
            try:
                ...
                if event_type == "connectorPublicUrlChanged":
                    logger.info(
                        f"Processing connectorPublicUrlChanged event: {payload}"
                    )
                    ...
                    return await google_drive_event_service.process_event(
                        event_type, payload
                    )
                    return True
                ...
                logger.info(
                    f"Processing sync event: {event_type} for connector {connector}"
                )
    ...
    async def create_aiconfig_message_handler(
        ...
        async def handle_aiconfig_message(message: dict[str, Any]) -> bool:
            ...
            try:
                ...
                logger.error(
                    f"Error processing AI config message: {e!s}", exc_info=True
                )
                return False
