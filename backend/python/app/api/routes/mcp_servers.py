...
@router.post("/instances", status_code=HttpStatusCode.CREATED.value)
...
@router.post("/instances/{instance_id}/authenticate")
...
@router.put("/instances/{instance_id}/credentials")
...
@router.delete("/instances/{instance_id}/credentials")
async def remove_credentials(request: Request, instance_id: str) -> dict[str, Any]:
    ...
    await config_service.delete_config(get_mcp_credentials_path(instance_id, user_id))

    from app.connectors.core.base.token_service.startup_service import startup_service

    refresh_service = startup_service.get_mcp_token_refresh_service()
    if refresh_service:
        refresh_service.cancel_refresh_task(get_mcp_credentials_path(instance_id, user_id))

    return {"success": True}


@router.post("/instances/{instance_id}/auto-authenticate")
...
@router.post("/instances/{instance_id}/reauthenticate")
    ...
    if discovered and discovered.registration_endpoint:
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Dynamic client registration failed: {e}",
            ) from e
...
@router.get("/instances/{instance_id}/oauth/authorize")
...
async def handle_oauth_callback(
    ...
    try:
        ...
        return {"success": False, "error": "client_config_changed", "errorMessage": str(e)}
    ...
    try:
        ...
        return {"success": False, "error": "token_exchange_failed", "errorMessage": str(e)}
...
@router.post("/instances/{instance_id}/oauth/refresh")
async def refresh_oauth_token(request: Request, instance_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=HttpStatusCode.UNAUTHORIZED.value, detail=f"Refresh token rejected: {e}") from e
    except oauth_client_module.MCPOAuthError as e:
        raise HTTPException(status_code=HttpStatusCode.BAD_GATEWAY.value, detail=f"Token refresh failed: {e}") from e

    return {"success": True}


@router.get("/instances/{instance_id}/oauth-config")
...
@router.put("/instances/{instance_id}/oauth-config")
...
@router.get("/instances/{instance_id}/tools")
