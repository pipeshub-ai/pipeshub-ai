"""OSS toolset edition resolvers — own-org lookups; EE-only seams are no-ops."""
...
# OSS never emits this; kept so base can compare without edition branching.
...
async def get_oauth_credentials_for_toolset(
    ...
    """Fetch OAuth config for a toolset (own-org only). Extra kwargs ignored on OSS."""
...
async def get_toolset_by_id(
    ...
    """Fetch a toolset instance by ID (org_id ignored on OSS)."""
    del org_id
    try:
        instances = await config_service.get_config(DEFAULT_TOOLSET_INSTANCES_PATH, default=[])
        if isinstance(instances, list):
...
async def check_user_is_admin(
    ...
    """Admin check via Node.js CM backend (forwards request auth headers)."""
    del org_id
    if request is None:
        return False
    try:
...
def mask_oauth_secrets(
    ...
    *,
    ...
    """OSS: return config unchanged (admins see secrets)."""
    del is_inherited
    return dict(cfg_data)


def is_redacted_placeholder(value: Any) -> bool:
    """OSS never redacts — always False."""
    del value
    return False
...
async def load_instances_for_mutation(
    ...
    """Load instances for create/update/delete (org-filtered on OSS)."""
    try:
