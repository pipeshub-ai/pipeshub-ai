            ...
            try:
                ...
                entity_records.append(
                    EntityRecord(
                        ...
                        connector_ids=[raw["connectorId"]] if raw.get("connectorId") else [],
                        record_group_ids=[raw["recordGroupId"]] if raw.get("recordGroupId") else [],
                    )
                )
            except Exception as exc:
...
async def entity_sync_status(
    request: Request,
    page_cap: int = Query(20, ge=1, le=10_000, description="Scroll pages (100 points each) to count before reporting an estimate"),
) -> JSONResponse:
    """Return collection info, per-org entity count (including how many
    points have empty or legacy-layout membership), and the persisted status
    of the most recent sync run (idle if none has ever run)."""
    services = await _get_services(request)
    logger = services["logger"]
    graph_provider = services["graph_provider"]
    entity_vector_store = services["entity_vector_store"]
    ...
    try:
        ...
        org_counts = await entity_vector_store.count_org_entities(org_id, page_cap=page_cap)
        info = {
            "collection": entity_vector_store.collection_name,
            "org_entity_count": org_counts["count"],
            "is_estimate": org_counts["is_estimate"],
            "empty_connector_ids_count": org_counts["empty_connector_ids"],
            "legacy_nested_membership_count": org_counts["legacy_nested_membership"],
        }
    except Exception as exc:
