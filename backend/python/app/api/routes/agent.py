...
async def stream_response(
    ...
    initial_state = build_initial_state(
        query_info, user_info, llm, logger,
        retrieval_service, arango_service, reranker_service, config_service, org_info
    )
...
async def askAIStream(request: Request, query_info: ChatQuery) -> StreamingResponse:
    ...
    try:
        ...
        return StreamingResponse(
            stream_response(
                query_info.model_dump(), enriched_user_info, services["llm"],
                services["logger"], services["retrieval_service"], services["arango_service"],
                services["reranker_service"], services["config_service"], org_info
            ),
...
async def chat(request: Request, agent_id: str, chat_query: ChatQuery) -> JSONResponse:
    ...
    try:
        ...
        # Execute graph
        initial_state = build_initial_state(
            query_info, enriched_user_info, services["llm"], logger,
            services["retrieval_service"], services["arango_service"],
            services["reranker_service"], services["config_service"], org_info
        )
...
async def chat_stream(request: Request, agent_id: str) -> StreamingResponse:
    ...
    try:
        ...
        return StreamingResponse(
            stream_response(
                query_info, enriched_user_info, llm, logger,
                services["retrieval_service"], services["arango_service"],
                services["reranker_service"], services["config_service"], org_info
            ),
            media_type="text/event-stream",
