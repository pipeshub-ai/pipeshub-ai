...
async def _create_mcp_server_edges(
    ...
    # Batch create agent -> mcpServer edges
    try:
    ...
    if tool_nodes:
        try:
            ...
            if not result:
                logger.warning("Failed to create MCP tool nodes")
        except Exception as e:
            logger.error(f"Failed to batch create MCP tool nodes: {e}")

    # Batch create mcpServer -> tool edges
...
async def get_assistant_agent(
    ...
    """
    from app.agents.mcp.service import get_authenticated_mcp_servers
