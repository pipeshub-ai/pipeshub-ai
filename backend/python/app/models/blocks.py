...
class CitationMetadata(BaseModel):
    ...
    def validate_bounding_boxes(cls, v: list[Point]) -> list[Point]:
        ...
        if len(v) != COORDINATE_COUNT:
            raise ValueError(
                f"bounding_boxes must contain exactly {COORDINATE_COUNT} points"
            )
        return v
...
class Block(BaseModel):
    ...
    parent_index: int | None = Field(
        default=None, description="Index of the parent block group"
    )
    type: BlockType
    name: str | None = None
...
class BlockGroup(BaseModel):
    ...
    parent_index: int | None = Field(
        description="Index of the parent block group", default=None
    )
    description: str | None = Field(
        description="Description of the block group", default=None
    )
    source_group_id: str | None = Field(
        description="Source group identifier", default=None
    )
    citation_metadata: CitationMetadata | None = None
    list_metadata: ListMetadata | None = None
