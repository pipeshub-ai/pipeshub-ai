...
class Processor:
    ...
    async def _get_llm_for_role(self, role: str):
        """Resolve LLM for a role. EE overrides to use inherited multi-tenant config."""
        return await get_llm_for_role(self.config_service, role)

    def _convert_record(self, record_dict: dict) -> Record:
        """Map a record document to a Record model. EE overrides for EE Record."""
