...
class EventProcessor:
    ...
    async def on_event(self, event_data: dict) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"📥 Processing event: {event_type}: for record {record_id} with virtual_record_id {virtual_record_id}"
            )
            ...
            if (
                ...
                if virtual_record_id is None:
                    raise Exception(
                        f"❌ Virtual record ID not found for record {record_id} for event {event_type}"
                    )

                self.logger.info(
                    f"""🔄 Deleting existing embeddings for record {record_id} for event {event_type}""",
                )
                await self.processor.indexing_pipeline.delete_embeddings(
            ...
            origin = event_data.get(
                "origin", "CONNECTOR" if connector != "" else "UPLOAD"
            )
            ...
            self.logger.debug(
                f"file_content type: {type(file_content)} length: {len(file_content)}"
            )
            ...
            if record_type == RecordTypes.FILE.value:
                try:
                    ...
                    if md5_checksum is None:
                        ...
                        await self.arango_service.batch_upsert_nodes(
                            [file_doc], CollectionNames.FILES.value
                        )
                    ...
                    if duplicate_files:
                        ...
                        for attempt in range(60):  # Wait up to 60 seconds
                            processed_duplicate = next(
                                (
                                    ...
                                    if f.get("summaryDocumentId")
                                    and f.get("virtualRecordId")
                                ),
                                None,
                            )
                            ...
                            if processed_duplicate:
                                ...
                                doc.update(
                                    {
                                        "isDirty": False,
                                        "summaryDocumentId": processed_duplicate.get(
                                            "summaryDocumentId"
                                        ),
                                        "virtualRecordId": processed_duplicate.get(
                                            "virtualRecordId"
                                        ),
                            ...
                            if in_progress:
                                self.logger.info(
                                    f"🚀 Duplicate file {in_progress.get('_key')} is being processed, waiting..."
                                )
                                ...
                                duplicate_files = (
                                    await self.arango_service.find_duplicate_files(
                                        file_doc.get("_key"),
                                        md5_checksum,
                                        size_in_bytes,
                                    )
                        ...
                        self.logger.info(
                            f"🚀 No processed duplicate found, proceeding with processing for {record_id}"
                        )
                    else:
                        self.logger.info(
                            f"🚀 No duplicate files found for record {record_id}"
                        )
            ...
            if mime_type == MimeTypes.GOOGLE_SLIDES.value:
                ...
                if isinstance(file_content, bytes):
                    try:
                        ...
                        self.logger.error(
                            f"Failed to decode Google Slides content: {e!s}",
                        )
                        raise
            ...
            if mime_type == MimeTypes.GOOGLE_DOCS.value:
                ...
                if isinstance(file_content, bytes):
                    try:
                        ...
                        self.logger.error(
                            f"Failed to decode Google Docs content: {e!s}",
                        )
                        raise
            ...
            if mime_type == MimeTypes.GOOGLE_SHEETS.value:
                ...
                if isinstance(file_content, bytes):
                    try:
                        ...
                        self.logger.error(
                            f"Failed to decode Google Sheets content: {e!s}",
                        )
                        raise
            ...
            if (
                extension == ExtensionTypes.PDF.value
                or mime_type == MimeTypes.PDF.value
            ):
                ...
                or mime_type == MimeTypes.DOCX.value
            ):
                ...
                extension == ExtensionTypes.DOC.value
                or mime_type == MimeTypes.DOC.value
            ):
                ...
                or mime_type == MimeTypes.XLSX.value
            ):
                ...
                extension == ExtensionTypes.XLS.value
                or mime_type == MimeTypes.XLS.value
            ):
                ...
                extension == ExtensionTypes.CSV.value
                or mime_type == MimeTypes.CSV.value
            ):
                ...
                extension == ExtensionTypes.HTML.value
                or mime_type == MimeTypes.HTML.value
            ):
                result = await self.processor.process_html_document(
                    recordName=record_name,
                    recordId=record_id,
                ...
                extension == ExtensionTypes.PPTX.value
                or mime_type == MimeTypes.PPTX.value
            ):
                ...
                extension == ExtensionTypes.PPT.value
                or mime_type == MimeTypes.PPT.value
            ):
                ...
                extension == ExtensionTypes.MD.value
                or mime_type == MimeTypes.MARKDOWN.value
            ):
                ...
                extension == ExtensionTypes.MDX.value
                or mime_type == MimeTypes.MDX.value
            ):
                ...
                or mime_type == MimeTypes.PLAIN_TEXT.value
            ):
                result = await self.processor.process_txt_document(
                    recordName=record_name,
                    recordId=record_id,
