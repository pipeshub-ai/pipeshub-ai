...
async def main():
    ...
    if not tenant_id or not client_id or not client_secret:
        raise Exception(
            "OUTLOOK_CLIENT_TENANT_ID, OUTLOOK_CLIENT_ID, and OUTLOOK_CLIENT_SECRET must be set"
        )
    ...
    print(client)
    print("****************************")
    outlook_data_source: OutlookCalendarContactsDataSource = (
        OutlookCalendarContactsDataSource(client)
    )
    ...
    response: OutlookCalendarContactsResponse = (
        await outlook_data_source.users_list_messages(user_id=user_id_or_upn)
    )
    print(response.data)
    ...
    response: OutlookCalendarContactsResponse = (
        await outlook_data_source.users_list_messages(
            user_id=user_id_or_upn, select=["id", "subject", "from", "receivedDateTime"]
        )
    )
    print(response.data)
    print(response.error)
