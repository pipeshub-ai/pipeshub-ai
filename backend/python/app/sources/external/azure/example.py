...
async def main():
    ...
    ACCESS_KEY = os.getenv("AZURE_BLOB_ACCOUNT_NAME")
    ACCOUNT_KEY = os.getenv("AZURE_BLOB_ACCOUNT_KEY")
    BUCKET = os.getenv("AZURE_BLOB_CONTAINER_NAME")
    # Create client
    config = AzureBlobAccountKeyConfig(accountName=ACCESS_KEY, accountKey=ACCOUNT_KEY, containerName=BUCKET)
    ...
    print("Running exists...")
    response: AzureBlobResponse = await azure_blob_data_source.exists()
