...
async def main() -> None:
    ...
    config = ZammadTokenConfig(token=API_TOKEN, base_url=BASE_URL)
    ...
    if response.success and response.data:
        user = (
            ...
            else response.data.get("user", {})
        )
        print(
            f"User: {user.get('firstname', '')} {user.get('lastname', '')} ({user.get('email', 'N/A')})"
        )
    ...
    if tickets_response.success and tickets_response.data:
        tickets = (
            ...
            if isinstance(tickets_response.data, list)
            else tickets_response.data.get("tickets", [])
        )
    ...
    if create_response.success and create_response.data:
        new_ticket = (
            create_response.data if isinstance(create_response.data, dict) else {}
        )
