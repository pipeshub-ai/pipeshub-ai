...
Scopes (OAuth2):
files:read, file_content:read, file_comments:read, file_comments:write,
file_variables:read, file_dev_resources:read, webhooks:read
"""
...
async def main() -> None:
    ...
    if CLIENT_ID and CLIENT_SECRET:
        ...
        try:
            ...
            # Figma uses Basic Auth (header method) for token exchange
    ...
    try:
        ...
        user_resp = await data_source.get_me()
        ...
        if not team_id:
            ...
            projects_resp = await data_source.get_team_projects(team_id=team_id)
            ...
            if project_id:
                ...
                files_resp = await data_source.get_project_files(project_id=project_id)
                ...
                if file_key:
                    ...
                    meta_resp = await data_source.get_file_meta(file_key=file_key)
                    print_result("Get File Metadata", meta_resp)

                    # 7. List Comments
                    print_section("File Comments")
                    comments_resp = await data_source.get_comments(file_key=file_key)
                    print_result("List Comments", comments_resp)

                    # 8. List File Versions
                    print_section("File Versions")
                    versions_resp = await data_source.get_file_versions(file_key=file_key)
                    print_result("List File Versions", versions_resp)

                    # 9. List File Components
                    print_section("File Components")
                    components_resp = await data_source.get_file_components(file_key=file_key)
                    print_result("List File Components", components_resp)

                    # 10. List File Styles
                    print_section("File Styles")
                    styles_resp = await data_source.get_file_styles(file_key=file_key)
                    print_result("List File Styles", styles_resp)

                    # 11. Get Local Variables
                    print_section("Local Variables")
                    vars_resp = await data_source.get_local_variables(file_key=file_key)
                    print_result("Get Local Variables", vars_resp)

                    # 12. Get Dev Resources
                    print_section("Dev Resources")
                    dev_resp = await data_source.get_dev_resources(file_key=file_key)
                    print_result("Get Dev Resources", dev_resp)

            # 13. List Team Components
            print_section("Team Components")
            team_components_resp = await data_source.get_team_components(
                team_id=team_id, page_size=5
            )
            print_result("List Team Components", team_components_resp)

            # 14. List Team Styles
            print_section("Team Styles")
            team_styles_resp = await data_source.get_team_styles(
                team_id=team_id, page_size=5
            )
            ...
            webhooks_resp = await data_source.get_webhooks()
