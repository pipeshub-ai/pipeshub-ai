...
async def main() -> None:
    ...
    google_calendar_data_source = GoogleDocsDataSource(
        enterprise_google_client.get_client()
    )
    created_document = await google_calendar_data_source.documents_create(
        title="Test Document by Agent with body"
    )
