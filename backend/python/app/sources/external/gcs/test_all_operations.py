"""
Comprehensive test script for all GCS DataSource operations.
Tests all 15 implemented methods against fake-gcs-server emulator.
"""
import asyncio
import os
import sys
...
async def test_all_operations() -> None:
    ...
    try:
        ...
        if resp.success:
            ...
            # Bucket already exists is acceptable (idempotent behavior)
            print("   ⚠️  Bucket already exists (409), treating as success for idempotency")
            results["passed"] += 1
            results["failed"] -= 1  # Remove from failed count
    ...
    try:
        ...
        check_result("patch_bucket", resp)  # Now uses REST API
    except Exception as e:
        results["failed"] += 1
    ...
    try:
        ...
        check_result("get_object_metadata", resp)  # Now uses REST API
    except Exception as e:
        results["failed"] += 1
    ...
    try:
        ...
        check_result("copy_object", resp)  # Available in gcloud-aio-storage
        if resp.success:
            # Clean up
    ...
    try:
        ...
        check_result("compose_object", resp)  # Available in gcloud-aio-storage
        if resp.success:
            # Clean up
    ...
    try:
        ...
        check_result("rewrite_object", resp)  # Now uses REST API
        if resp.success:
    ...
    try:
        ...
        check_result("update_object_metadata", resp)  # Now uses REST API
    except Exception as e:
        results["failed"] += 1
