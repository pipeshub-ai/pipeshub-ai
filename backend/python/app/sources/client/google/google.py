...
try:
    ...
    print(
        "Google API client libraries not found. Please install them using 'pip install google-api-python-client google-auth-httplib2 google-auth-oauthlib'"
    )
    raise
...
class GoogleAuthConfig:
    ...
    is_individual: bool | None = (
        False  # Flag to indicate if authentication is for an individual user.
    )
...
class GoogleClient(IClient):
    ...
    @staticmethod
    def _get_optimized_scopes(
        service_name: str, additional_scopes: list[str] | None = None
    ) -> list[str]:
    ...
    async def build_from_services(
        ...
        version: str
        | None = "v3",  # Version of the service to build the client for [v3, v1]
        ...
        if is_individual:
            try:
                ...
                saved_credentials = await GoogleClient.get_individual_token(
                    service_name, logger, config_service
                )
                if not saved_credentials:
                    raise ValueError("Failed to get individual token")

                # Get optimized scopes for the service
                optimized_scopes = GoogleClient._get_optimized_scopes(
                    service_name, scopes
                )

                google_credentials = Credentials(
                    token=saved_credentials.get(CredentialKeys.ACCESS_TOKEN.value),
                    refresh_token=saved_credentials.get(
                        CredentialKeys.REFRESH_TOKEN.value
                    ),
                    token_uri="https://oauth2.googleapis.com/token",
                    client_id=saved_credentials.get(CredentialKeys.CLIENT_ID.value),
                    client_secret=saved_credentials.get(
                        CredentialKeys.CLIENT_SECRET.value
                    ),
            ...
            try:
                saved_credentials = await GoogleClient.get_enterprise_token(
                    service_name, logger, config_service
                )
                if not saved_credentials:
    ...
    async def _get_connector_config(
        service_name: str, logger: logging.Logger, config_service: ConfigurationService
    ) -> dict:
    ...
    @staticmethod
    async def get_account_scopes(
        service_name: str, logger: logging.Logger, config_service: ConfigurationService
    ) -> list:
        """Get account scopes for a specific connector (gmail/drive)."""
        config = await GoogleClient._get_connector_config(
            service_name, logger, config_service
        )
