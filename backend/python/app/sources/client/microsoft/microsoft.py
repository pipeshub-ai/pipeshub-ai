...
try:
    ...
    raise ImportError(
        "azure-identity is not installed. Please install it with `pip install azure-identity`"
    )
...
class MSGraphUsernamePasswordConfig:
    ...
    def create_client(
        self, mode: GraphMode = GraphMode.APP
    ) -> MSGraphClientViaUsernamePassword:
        return MSGraphClientViaUsernamePassword(
            self.username, self.password, self.client_id, self.tenant_id, mode=mode
        )
...
class MSGraphClientWithClientIdSecretConfig:
    ...
    def create_client(
        ...
        return MSGraphClientWithClientIdSecret(
            self.client_id, self.client_secret, self.tenant_id, mode=mode
        )
...
class MSGraphClientWithCertificatePathConfig:
    ...
    def create_client(
        ...
        return MSGraphClientWithCertificatePath(
            self.certificate_path, self.tenant_id, self.client_id, mode=mode
        )
...
class MSGraphClient(IClient):
    ...
    def build_with_config(
        cls,
        config: MSGraphUsernamePasswordConfig
        | MSGraphClientWithClientIdSecretConfig
        | MSGraphClientWithCertificatePathConfig,  # type:ignore
        mode: GraphMode = GraphMode.APP,
    ) -> "MSGraphClient":
    ...
    async def build_from_services(
        ...
        try:
            ...
            config = await cls._get_connector_config(
                service_name.replace(" ", "").lower(), logger, config_service
            )

            if not config:
                raise ValueError(
                    "Failed to get Microsoft Graph connector configuration"
                )
            auth_config = config.get("auth", {}) or {}
            # Extract configuration values
            auth_type = auth_config.get(
                "authType", "OAUTH_ADMIN_CONSENT"
            )  # client_secret, username_password, certificate
            tenant_id = auth_config.get("tenantId", "")
            client_id = auth_config.get("clientId", "")

            if not tenant_id or not client_id:
                raise ValueError(
                    "Tenant ID and Client ID are required for Microsoft Graph authentication"
                )
            ...
            if auth_type == "USERNAME_PASSWORD":
                ...
                client = MSGraphClientViaUsernamePassword(
                    username, password, client_id, tenant_id, mode
                )
            ...
            #     client = MSGraphClientWithCertificatePath(certificate_path, tenant_id, client_id, mode)
                ...
                if not client_secret:
                    raise ValueError(
                        "Client secret required for client_secret auth type"
                    )
                scopes = auth_config.get(
                    "scopes", ["https://graph.microsoft.com/.default"]
                )
                client = MSGraphClientWithClientIdSecret(
                    client_id, client_secret, tenant_id, scopes, mode
                )
