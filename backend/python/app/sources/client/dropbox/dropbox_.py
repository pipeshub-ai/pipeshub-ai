...
class DropboxRESTClientViaToken:
    ...
    def __init__(self, access_token: str, refresh_token: str, timeout: Optional[float] = None, is_team: bool = False) -> None:
    ...
    def create_client(self) -> Dropbox: # type: ignore[valid-type]
        ...
        if self.is_team:
            self.dropbox_client = DropboxTeam(oauth2_access_token=self.access_token, timeout=self.timeout, oauth2_refresh_token=self.refresh_token) # type: ignore[valid-type]
        else:
            self.dropbox_client = Dropbox(oauth2_access_token=self.access_token, timeout=self.timeout, oauth2_refresh_token=self.refresh_token) # type: ignore[valid-type]
...
class DropboxTokenConfig:
    ...
    async def create_client(self, is_team: bool = False) -> DropboxRESTClientViaToken:
        ...
        return DropboxRESTClientViaToken(self.token, self.refresh_token, timeout=self.timeout, is_team=is_team)
