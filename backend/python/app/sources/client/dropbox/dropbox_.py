...
class DropboxClient(IClient):
    ...
    def __init__(
        self,
        client: DropboxRESTClientViaToken | DropboxRESTClientWithAppKeySecret,
    ) -> None:
        self.client = client

    def get_client(
        self,
    ) -> DropboxRESTClientViaToken | DropboxRESTClientWithAppKeySecret:
    ...
    async def build_with_config(
        cls,
        config: DropboxTokenConfig | DropboxAppKeySecretConfig,
        is_team: bool = False,
    ) -> "DropboxClient":
    ...
    async def build_from_services(
        ...
        try:
            ...
            if auth_type == "APP_KEY_SECRET":
                ...
                client = await DropboxTokenConfig(token=access_token).create_client(
                    is_team=is_team
                )
            ...
            logger.error(f"Failed to build Dropbox client from services: {e!s}")
            raise
