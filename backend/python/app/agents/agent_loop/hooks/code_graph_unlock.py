...
def record_is_code_file(record: Any) -> bool:
    """Public because `repo_instructions` gates on the same predicate; keeping
    a second copy there would let the two drift apart."""
...
def virtual_records_include_code(virtual_records: dict[str, Any] | None) -> bool:
    if not virtual_records:
        return False
    return any(record_is_code_file(r) for r in virtual_records.values())
