...
class GoogleClientFactory(ClientFactory):
    ...
    async def create_client(
        self, config_service, logger, state: ChatState | None = None
    ) -> GoogleClient:
        ...
        if state and isinstance(state, dict):
            ...
            if not is_individual:
                user_email = (
                    str(state.get("user_info", {}).get("userEmail", "")).strip() or None
                )
        ...
        logger.info(
            f"Created Google client for service {self.service_name} with user email {user_email}"
        )
