...
def get_tool_results_summary(state: "ChatState") -> str:
    ...
    for result in all_results:
        ...
        tool_summary[tool_name][status] = tool_summary[tool_name].get(status, 0) + 1
    ...
    for tool_name, stats in tool_summary.items():
        ...
        if stats["results"]:
            ...
            result_preview = str(last_result.get("result", ""))[
...
"""
    ...
    for category, tools in sorted(by_category.items()):
        ...
        if len(tools) > MAX_TOOLS_PER_CATEGORY_DISPLAY:
            guidance += (
                f"  ... and {len(tools) - MAX_TOOLS_PER_CATEGORY_DISPLAY} more\n"
            )
