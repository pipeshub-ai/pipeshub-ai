...
class ToolsDBManager:
    ...
    @classmethod
    async def create(
        cls, logger: logging.Logger, config_service: ArangoConfig | ConfigurationService
    ) -> "ToolsDBManager":
        """Create and initialize ToolsDBManager"""
        graph_service = await GraphDBFactory.create_service(
            "arango", logger, config_service
        )
    ...
    async def initialize_collections(self) -> None:
        ...
        try:
            ...
            if not await self.graph_service.create_collection(self.collection_name):
                self.logger.warning(
                    f"Collection {self.collection_name} already exists or failed to create"
                )

            # Create ctag collection
            if not await self.graph_service.create_collection(
                self.ctag_collection_name
            ):
                self.logger.warning(
                    f"Collection {self.ctag_collection_name} already exists or failed to create"
                )
        ...
        content = json.dumps(
            {
                ...
                "parameters": [
                    param.to_json_serializable_dict() for param in tool.parameters
                ],
    ...
    async def _sync_single_tool(self, tool: Tool) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to sync tool {tool.app_name}.{tool.tool_name}: {e}"
            )
            raise
    ...
    async def _update_connector_ctag(self, connector_name: str, ctag: str) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to update connector ctag for {connector_name}: {e}"
            )
            raise
