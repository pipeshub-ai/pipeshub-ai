...
from app.sandbox.artifact_upload import _read_file_bytes, create_artifact_record
...
class ExecutePythonInput(BaseModel):
    ...
    requirements: list[str] = Field(
        ...
        description="pip packages to install before execution (e.g. ['pandas', 'matplotlib'])",
    )
...
class ExecuteTypeScriptInput(BaseModel):
    ...
    packages: list[str] = Field(
        default_factory=list,
        description="npm packages to install before execution (e.g. ['chart.js', 'docx'])",
    )
...
class CodingSandbox:
    ...
    async def _upload_artifacts(
        ...
        Returns a list of upload info dicts (each has ``fileName``, ``mimeType``,
        ``signedUrl``/``downloadUrl``, ``sizeBytes``, and ``recordId``).
        """
    ...
    async def execute_python(self, code: str, requirements: list[str] | None = None) -> tuple[bool, str]:
        ...
        logger.info(
            "[execute_python] INPUT | code_length=%d requirements=%s\n--- CODE ---\n%s\n--- END CODE ---",
            len(code), requirements or [], code,
        )
        try:
            ...
            return self._result(False, {"success": False, "error": str(e)})
    ...
    async def execute_typescript(self, code: str, packages: list[str] | None = None) -> tuple[bool, str]:
        ...
        logger.info(
            "[execute_typescript] INPUT | code_length=%d packages=%s\n--- CODE ---\n%s\n--- END CODE ---",
            len(code), packages or [], code,
        )
        try:
            executor = get_executor()
