...
class GoogleMeet:
    ...
    def start_instant_meeting(
        ...
        try:
            ...
            if title or description:
                result["note"] = (
                    "Title and description are not supported for instant meetings. Use schedule_meeting_with_calendar for meetings with custom titles."
                )
    ...
    def join_meeting_by_code(self, meeting_code: str) -> tuple[bool, str]:
        ...
        try:
            ...
            try:
                space = self._run_async(
                    self.client.spaces_get(name=f"spaces/{clean_code}")
                )
                result = {
            ...
            logger.error(
                f"Failed to generate join link for meeting {meeting_code}: {e}"
            )
            return False, json.dumps({"error": str(e)})
    ...
    def schedule_meeting_with_calendar(
        ...
        try:
            ...
            start_dt = datetime.fromtimestamp(
                start_timestamp / 1000, tz=dt_timezone.utc
            )
            ...
            if recurrence:
                event_config["recurrence"] = [
                    recurrence.get("rule", "RRULE:FREQ=WEEKLY")
                ]

            calendar_event = self._run_async(
                calendar_client.events_insert(
                    calendarId="primary",
    ...
    def find_available_time(
        ...
        try:
            # Prepare time range
            start_iso, end_iso = prepare_iso_timestamps(
            ...
            freebusy_result = self._run_async(
                calendar_client.freebusy_query(body=freebusy_query)
            )
            ...
            while current_time + timedelta(minutes=duration_minutes) <= end_time:
                ...
                for busy_period in busy_times_sorted:
                    ...
                    if current_time < busy_end and slot_end > busy_start:
                        ...
                        next_time = (
                            busy_end if next_time is None else max(next_time, busy_end)
                        )
                        break
    ...
    def update_scheduled_meeting(
        ...
        try:
            ...
            # Get current event
            current_event = self._run_async(
                calendar_client.events_get(
                    calendarId="primary",
                    eventId=event_id,
                )
            )
            ...
            # Update the event
            updated_event = self._run_async(
                calendar_client.events_patch(
                    calendarId="primary",
                    eventId=event_id,
                    body=update_data,
                )
    ...
    def cancel_meeting(
        self, event_id: str, notify_attendees: bool | None = None
    ) -> tuple[bool, str]:
    ...
    def list_upcoming_meetings(
        ...
        try:
            ...
            # Get events
            events_response = self._run_async(
                calendar_client.events_list(**query_params)
            )
    ...
    def create_meeting_space(
        ...
        try:
            ...
            if title or description:
                result["note"] = (
                    "Title and description are not supported for meeting spaces. Use schedule_meeting_with_calendar for meetings with custom titles."
                )
            ...
            if create_calendar_event and start_time and duration_minutes:
                try:
                    ...
                    event_config = {
                        ...
                        or f"Join the meeting: {meeting_uri}",
                        "start": {
                            "dateTime": start_time_iso,
                            "timeZone": timezone,
                        },
                    ...
                    result["warning"] = (
                        f"Meet space created but calendar event failed: {e!s}"
                    )
    ...
    def update_meeting_space(
        ...
        try:
            ...
            if not space_config:
                return False, json.dumps(
                    {
                        "error": "No updates provided. Only space_config updates are supported."
                    }
                )

            # Use GoogleMeetDataSource method - only update spaceConfig
            updated_space = self._run_async(
                self.client.spaces_patch(
                    name=space_name,
    ...
    def end_active_conference(self, space_name: str) -> tuple[bool, str]:
        ...
        try:
            ...
            result = self._run_async(
                self.client.spaces_end_active_conference(name=space_name)
            )
    ...
    def get_conference_records(
        ...
        try:
            ...
            records = self._run_async(
                self.client.conference_records_list(
                    pageSize=page_size,
            ...
            enhanced_response = {
                ...
                "summary": {
                    "active_conferences": len(
                        [
                            r
                            for r in records.get("conferenceRecords", [])
                            if not r.get("endTime")
                        ]
                    ),
                    "completed_conferences": len(
                        [
                            r
                            for r in records.get("conferenceRecords", [])
                            if r.get("endTime")
    ...
    def get_conference_record_details(self, conference_record: str) -> tuple[bool, str]:
        ...
        try:
            ...
            record = self._run_async(
                self.client.conference_records_get(name=conference_record)
            )
    ...
    def get_conference_participants(
        ...
        try:
            ...
            # Use GoogleMeetDataSource method
            participants = self._run_async(
                self.client.conference_records_participants_list(
            ...
            enhanced_response = {
                ...
                "summary": {
                    "active_participants": len(
                        [p for p in participant_list if not p.get("latestEndTime")]
                    ),
                    "completed_participants": len(
                        [p for p in participant_list if p.get("latestEndTime")]
                    ),
                },
            }
    ...
    def get_conference_recordings(
        ...
        try:
            # Use GoogleMeetDataSource method
            recordings = self._run_async(
                self.client.conference_records_recordings_list(
                    parent=conference_record,
                    pageSize=page_size,
                    pageToken=page_token,
                )
    ...
    def get_conference_transcripts(
        ...
        try:
            # Use GoogleMeetDataSource method
            transcripts = self._run_async(
                self.client.conference_records_transcripts_list(
                    parent=conference_record,
                    pageSize=page_size,
                    pageToken=page_token,
                )
                    ...
                    if transcript_name:
                        try:
                            entries = self._run_async(
                                self.client.conference_records_transcripts_entries_list(
                                    parent=transcript_name,
                                    pageSize=100,  # Get all entries for each transcript
                                )
                            )
                            transcript_entries[transcript_name] = entries.get(
                                "transcriptEntries", []
                            )
                        except Exception as e:
                            logger.warning(
                                f"Failed to get entries for transcript {transcript_name}: {e}"
                            )
        ...
        try:
            ...
            if record.get("startTime") and record.get("endTime"):
                ...
                start_time = datetime.fromisoformat(
                    record["startTime"].replace("Z", "+00:00")
                )
                end_time = datetime.fromisoformat(
                    record["endTime"].replace("Z", "+00:00")
                )
                duration = end_time - start_time
                summary["meeting_info"]["duration_minutes"] = int(
