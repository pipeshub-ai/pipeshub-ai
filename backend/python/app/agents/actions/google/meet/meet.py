...
class GoogleMeet:
    ...
    def start_instant_meeting(self, title: Optional[str] = None, description: Optional[str] = None) -> tuple[bool, str]:
        ...
        try:
            ...
            # Update space with title/description if provided
            if title or description:
                ...
                if update_body:
                    try:
                        ...
                        result["updated_space"] = updated_space
                        result["message"] += " with title and description"
                    except Exception as e:
                        logger.warning(f"Failed to update space with title/description: {e}")
                        result["warning"] = f"Space created but title/description update failed: {str(e)}"
    ...
    def schedule_meeting_with_calendar(
        ...
        try:
            ...
            calendar_client = GoogleCalendarDataSource(self.client.client)
    ...
    def update_meeting_space(
        ...
        Args:
            ...
            title: New meeting title/display name
            description: New meeting description
        ...
        try:
            ...
            if not update_mask_parts and not space_config:
                return False, json.dumps({"error": "No updates provided"})

            # Use GoogleMeetDataSource method
            updated_space = self._run_async(self.client.spaces_patch(
                name=space_name,
                updateMask=",".join(update_mask_parts) if update_mask_parts else "*",
                body=update_body
            ))
            ...
            return True, json.dumps({
                ...
                "message": "Meeting space updated successfully"
            })
        except Exception as e:
