...
# How many blocks a free-text or glob selector may consider before ranking.
...
def _looks_like_path(select: str) -> bool:
    return "/" in select or "*" in select or select.endswith((".py", ".ts", ".tsx", ".js", ".jsx"))
...
async def query_code_graph_impl(
    *,
    ...
    if _looks_like_path(select):
        matched = await _select_by_path(graph_provider, org_id, select, connector_id)
        how = "path"
    else:
    ...
    if not matched:
        return {"select": select, "resolved_as": how, "matches": 0,
    ...
    result: dict[str, Any] = {
        ...
        "truncated": total_matched > len(seeds),
    }
...
async def _grouped_result(
    *,
    ...
    return {
        ...
        # Silence reads as absence: a model that thinks it saw everything will
        # state a wrong conclusion confidently.
        "truncated": total_edges > limit or len(rows) >= _ROLLUP_ROW_LIMIT,
        **result,
    }
