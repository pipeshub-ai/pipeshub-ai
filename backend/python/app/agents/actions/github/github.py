...
class GitHub:
    ...
    @tool(
        ...
        description="Get files changed in a pull request (diff summary).",
        llm_description="Returns list of changed files in a PR (filename, status, additions, deletions, patch). Use when user wants to 'review this PR', 'see what changed', 'what files in this PR', or 'diff for PR #N'. Not for PR metadata (get_pull_request) or line-level comments (list_pull_request_comments).",
        args_schema=GetPullRequestInput,
        returns="JSON array of changed files with filename, status, additions, deletions, and patch info",
        primary_intent=ToolIntent.SEARCH,
        category=ToolCategory.CODE_MANAGEMENT,
        when_to_use=[
        ...
        typical_queries=["Review this PR", "What changes in PR #5?", "What files changed in this PR?", "Show me the diff for PR #7"],
    )
    async def get_pull_request_file_changes(self, owner: str, repo: str, number: int) -> Tuple[bool, str]:
        """Get file changes (diff summary) for a pull request."""
        try:
            logger.info("github.get_pull_request_file_changes called with args: %s", {"owner": owner, "repo": repo, "number": number})
            response = self.client.get_pull_file_changes(owner=owner, repo=repo, number=number)
            return self._handle_response(response, "Pull request file changes fetched successfully")
        except Exception as e:
