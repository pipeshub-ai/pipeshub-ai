...
from app.utils.chat_helpers import (
    ...
    llm_likely_accepts_vision_content,
)
...
class Retrieval:
    ...
    async def search_internal_knowledge(
        ...
        try:
            ...
            # Match Assistant /chat: do not drop image blocks when the model supports
            # vision. :data:`build_initial_state` now sets ``is_multimodal_llm`` from
            # :func:`llm_likely_accepts_vision_content` (replaces a stale hard-coded
            # ``False`` and a tiny substring list that missed e.g. ``gemini-2.5-flash``).
            llm_instance = self.state.get("llm")
            is_multimodal_llm = bool(
                self.state.get("is_multimodal_llm")
            ) or llm_likely_accepts_vision_content(llm_instance)
