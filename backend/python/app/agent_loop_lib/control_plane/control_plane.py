...
class _LazyTransport:
    ...
    same deferred-transport convention as `_make_llm_summarizer` below,
    needed anywhere a Phase 5+ feature (e.g. RubricGrader) wants "the
    configured LLM" without ControlPlane.start() paying for a real
    transport client it may never end up using."""

    def __init__(self, transport_registry, provider: str) -> None:
...
class ControlPlane:
    ...
    async def start(self) -> None:
        ...
        # 7. RoleRegistry (builtin roles: assistant, planner, explorer, researcher, critic, verifier, writer)
        ...
        if smc.skills_dir is not None:
            ...
            if smc.require_passing_grade:
                ...
                skill_grader = RubricGrader(
                    _LazyTransport(transport_registry, cfg.transport), pass_threshold=smc.pass_threshold,
                )
            skill_evaluator = RubricSkillEvaluator(grader=skill_grader, index=skill_index)
            skill_extractor = (
                LLMSkillExtractor(_LazyTransport(transport_registry, cfg.transport))
