...
def shape_tool_result_clearing(
    ...
    `protected_tool_names`, when given, prevents clearing of any TOOL
    message whose originating tool call name is in the set — regardless
    of age. Used by the orchestrator to keep create_plan/critique_plan
    results visible throughout the Phase 1 plan-critique loop.
    """
    ...
    async def _middleware(ctx: ModelCallContext, next_fn) -> None:
        ...
        if protected_tool_names and clearable:
            ...
            for i in clearable:
                ...
                if tool_call_id and call_id_to_name.get(tool_call_id) in protected_tool_names:
