...
__all__ = ["SystemPromptBuilder", "DefaultPromptBuilder", "build_system_prompt"]
...
class DefaultPromptBuilder:
    ...
    def build(
        ...
        template.set("skills_overview", _render_skills_overview(runtime))
...
def _render_toolset_overview(runtime: "AgentRuntime") -> str:
    ...
    if registry is None or not registry.has_toolsets():
        return ""
    lines = ["Additional capability groups are available via fetch_tools(toolset):"]
    for ts in registry.toolset_overview():
        lines.append(f"- {ts['name']}: {ts['description']} ({ts['tool_count']} tool(s))")
    return "\n".join(lines)


def _render_skills_overview(runtime: "AgentRuntime") -> str:
    ...
    stay pure string assembly, no I/O."""
