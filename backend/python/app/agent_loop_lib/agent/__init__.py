...
class Agent:
    ...
    def __init__(self, spec: "AgentSpec", runtime: "AgentRuntime", *, session_id: str | None = None) -> None:
        ...
        # warning/truncation-recovery/supervisor-confidence) — dispatched
        # through the SAME kernel as PRE_TOOL_USE/POST_TOOL_USE, idempotent
        # per kernel instance (see hooks/middleware/builtin/turn_guards.py).
        install_turn_guards(
            self._hooks,
    ...
    async def inject_user_message(self, text: str) -> None:
        ...
        context internals."""
        await self._context.add(UserMessage(content=text))
    ...
    async def step(self, goal: Goal, turn_index: int) -> StepOutcome:
        ...
        async def _run_one_tool_call(call: ToolCall) -> ToolCallOutcome:
            ...
            return await execute_tool_call(
                ...
                self.started_at, seen_tool_calls, response_msg, turn_scope,
            )

        outcomes_by_id: dict[str, ToolCallOutcome] = {}
        parallel_calls = [c for c in tool_calls if c.name != "spawn_agent"]
        if parallel_calls:
            results = await asyncio.gather(*(_run_one_tool_call(c) for c in parallel_calls))
            outcomes_by_id.update(zip((c.id for c in parallel_calls), results))
        for c in tool_calls:
            if c.name == "spawn_agent":
                outcomes_by_id[c.id] = await _run_one_tool_call(c)
