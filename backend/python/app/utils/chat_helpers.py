...
async def get_flattened_results(result_set: List[Dict[str, Any]], blob_store: BlobStorage, org_id: str, is_multimodal_llm: bool, virtual_record_id_to_result: Dict[str, Dict[str, Any]],virtual_to_record_map: Dict[str, Dict[str, Any]]=None,from_tool: bool = False,from_retrieval_service: bool = False) -> List[Dict[str, Any]]:
    ...
    # Log batch fetch timing
    if records_to_fetch:
        batch_fetch_duration_ms = (time.time() - batch_fetch_start_time) * 1000
        record_count = len(records_to_fetch)
        avg_duration_ms = batch_fetch_duration_ms / record_count if record_count > 0 else 0
        logger.info("⏱️ Batch fetch completed: %d records in %.0fms (avg: %.0fms/record)", record_count, batch_fetch_duration_ms, avg_duration_ms)
