...
async def _try_blobstore_fetch(
    blob_store: BlobStorage, org_id: str, record_id: str
) -> dict[str, Any] | None:
    ...
    """
    try:
        rec = await blob_store.get_record_from_storage(
            org_id=org_id, virtual_record_id=record_id
        )
        if rec:
            return rec
    except Exception:
...
async def _fetch_full_record_using_vrid(
    vrid: str, blob_store: BlobStorage, org_id: str
) -> dict[str, Any]:
...
async def _fetch_full_record_impl(
    ...
    return {
        "ok": False,
        "error": f"Record '{record_id}' not found via blob store or arango.",
    }
...
async def _fetch_multiple_records_impl(
    ...
    return {
        "ok": False,
        "error": f"None of the requested records were found: {', '.join(record_ids)}",
    }
    ...
    async def fetch_full_record_tool(
        ...
        try:
            result = await _fetch_multiple_records_impl(
                record_ids, virtual_record_id_to_result
            )
            return result
        except Exception as e:
            # Return error as dict
            return {"ok": False, "error": f"Failed to fetch records: {e!s}"}

    return fetch_full_record_tool


def create_fetch_block_group_tool(
    blob_store: BlobStorage, final_results: list[dict[str, Any]], org_id: str
) -> Callable:
    """Factory function to create the tool with runtime dependencies injected."""

    @tool("fetch_block_group", args_schema=FetchBlockGroupArgs)
    async def fetch_block_group_tool(
        block_group_number: str,
        reason: str = "Fetching block group for additional context",
    ) -> str:
        ...
        if vrid:
            ...
            if record:
                ...
                if block_groups and block_group_index < len(block_groups):
                    ...
                    record = create_record_for_fetch_block_group(
                        record, block_group, result_blocks
                    )
                    return {"ok": True, "record": record}
            else:
                return {
                    "ok": False,
                    "error": f"Block group '{block_group_number}' not found in record with vrid '{vrid}'.",
                }
        else:
            return {
                "ok": False,
                "error": f"Block group '{block_group_number}' not found.",
            }

    return fetch_block_group_tool


def create_record_for_fetch_block_group(
    record: dict[str, Any], block_group: dict[str, Any], blocks: list[dict[str, Any]]
) -> dict[str, Any]:
    block_container = {
        "blocks": blocks,
