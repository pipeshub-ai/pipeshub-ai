...
class RetrievalService:
    # Base64 alphabet, including URL-safe variants and padding. Kept at class
    # scope so membership checks don't rebuild a set on every call.
    _BASE64_ALPHABET = frozenset(
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=-_"
    )
    # Only treat a string as a bare-base64 image blob when it's at least this
    # long AND passes every structural check below. Tuned to sit comfortably
    # above real-world JWTs (~1–2 KB) without missing small inline thumbnails.
