...
class MarkdownParser:
    ...
    def extract_and_replace_images(
        ...
        modified_content = re.sub(
            reference_usage_pattern, replace_reference_image, md_content
        )
        modified_content = re.sub(
            markdown_img_pattern, replace_markdown_image, modified_content
        )
    ...
    for i, line in enumerate(lines):
        ...
        if i < len(lines) - 1:
            ...
            if (
                ...
                and len(next_line) >= MIN_UNDERLINE_LENGTH
            ):
