...
class MDXParser:
    ...
    def __init__(self) -> None:
        ...
        self.jsx_block_pattern = (
            r"<(?P<tag>[A-Za-z][A-Za-z0-9]*)[^>]*>(?P<content>.*?)</\1>"
        )
    ...
    def convert_mdx_to_md(self, binary_content: bytes) -> bytes:
        try:
            ...
            def handle_allowed_tags(match) -> str:
                ...
                if tag == "Info":
                    return f"\n\n> **Note:**\n>\n> {inner}\n"
                if tag == "CodeGroup" or tag == "AccordionGroup":
                    return f"\n\n{inner}\n"
                # Default passthrough
                return inner
            ...
            content = re.sub(
                self.jsx_block_pattern,
                lambda m: handle_allowed_tags(m)
                if m.group("tag") in self.allowed_tags
                else "",
                content,
                flags=re.DOTALL,
            )
            ...
            raise Exception(f"Error converting MDX to Markdown: {e!s}") from e
