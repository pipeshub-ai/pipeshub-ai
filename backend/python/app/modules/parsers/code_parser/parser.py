...
def extract_blocks(source: bytes, language: str, merge_gap: int = 1) -> list[Block]:
    ...
    Args:
        ...
        language: key into LANG_CONFIG (e.g. "python", "typescript").
        merge_gap: maximum line-gap that still counts as "adjacent" when merging
            small blocks (statements + comments). 1 means strictly consecutive
            (no blank line between); 2 allows one blank line. 0 disables merging.
    """
