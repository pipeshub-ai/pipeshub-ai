...
def serialize_block_for_graph(
    ...
    *,
    ...
    doc: dict[str, Any] = {
        ...
        "typeTable": None,
        # Filled on one block per file by `write_code_file_blocks_to_graph`.
...
async def write_code_file_blocks_to_graph(
    *,
    ...
    summary_id = block_key_for(context.record_id, FILE_SUMMARY_QUALIFIED_NAME)
    carrier = next((doc for doc in docs if doc["id"] == summary_id), docs[0])
    carrier["referencedNames"] = _referenced_names(block_containers)
