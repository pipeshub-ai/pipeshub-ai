"""Connector-specific tool-usage guidance blocks, extracted verbatim from
`modules/agents/qna/nodes.py` (Phase 0 of the agent-loop migration).

Each `*_GUIDANCE` constant is injected into the ReAct/planner system prompt
only when the corresponding connector's tools are active for the request
(see `modules/agents/context/connector_detection.py`). `GUIDANCE_MAP` keys
match the lowercase toolset `name` field used throughout `agent_toolsets`.

NOTE: `nodes.py` previously defined `JIRA_GUIDANCE` twice (lines 1596 and
3074 in the pre-extraction file); Python's normal name-rebinding meant the
second definition silently shadowed the first, which was therefore dead
code. This module keeps only the second (more complete) definition.
"""
...
GUIDANCE_MAP: dict[str, str] = {
    "jira": JIRA_GUIDANCE,
    "confluence": CONFLUENCE_GUIDANCE,
    "slack": SLACK_GUIDANCE,
    "onedrive": ONEDRIVE_GUIDANCE,
    "outlook": OUTLOOK_GUIDANCE,
    "teams": TEAMS_GUIDANCE,
    "github": GITHUB_GUIDANCE,
    "clickup": CLICKUP_GUIDANCE,
    "mariadb": MARIADB_GUIDANCE,
    "redshift": REDSHIFT_GUIDANCE,
    "zoom": ZOOM_GUIDANCE,
    "salesforce": SALESFORCE_GUIDANCE,
    "sharepoint": SHAREPOINT_GUIDANCE,
}
