...
class PerformanceTracker:
    ...
    def track_llm_call(self, duration_ms: float, tokens: Optional[int] = None) -> None:
    ...
    def track_tool_execution(self, tool_name: str, duration_ms: float, success: bool) -> None:
    ...
    def _identify_bottlenecks(self, step_breakdown: List[Dict], total_ms: float) -> List[Dict[str, Any]]:
        ...
        for step in step_breakdown:
            ...
            _BOTTLENECK_PERCENTAGE_THRESHOLD = 25
            _BOTTLENECK_DURATION_MS_THRESHOLD = 1000
            _HIGH_SEVERITY_PERCENTAGE = 40
            if percentage > _BOTTLENECK_PERCENTAGE_THRESHOLD or duration > _BOTTLENECK_DURATION_MS_THRESHOLD:
                bottlenecks.append({
                    "step": step["step"],
                    "duration_ms": duration,
                    "percentage": percentage,
                    "severity": "high" if percentage > _HIGH_SEVERITY_PERCENTAGE else "medium"
                })
