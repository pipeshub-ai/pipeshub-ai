...
_POSITION_FAST_START = 0.3
_POSITION_FAST_FINISH = 0.8
_SHORT_CONTENT_THRESHOLD = 200
_MEDIUM_CONTENT_THRESHOLD = 1000
...
class StreamingOptimizer:
    ...
    async def stream_response(
        self,
        content: str,
        writer: Callable[[dict[str, Any]], None],  # noqa: ANN401
    ...
    def _calculate_delay(self, chunk_index: int, total_chunks: int) -> float:
        ...
        if position < _POSITION_FAST_START:
            # Fast start
            return self.min_delay_ms
        elif position > _POSITION_FAST_FINISH:
            # Fast finish
            return self.min_delay_ms
        else:
    ...
    def optimize_for_length(self, content_length: int) -> None:
        ...
        if content_length < _SHORT_CONTENT_THRESHOLD:
            # Short content: larger chunks, faster streaming
            self.chunk_size = 100
            self.min_delay_ms = 5
            self.max_delay_ms = 15
        elif content_length < _MEDIUM_CONTENT_THRESHOLD:
