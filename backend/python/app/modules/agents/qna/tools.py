...
class WriteToFileInput(BaseModel):
    ...
    filename: str | None = Field(
        default=None, description="Optional filename, will use default if not provided"
    )
    append: bool = Field(
        default=False, description="Whether to append to existing file or overwrite"
    )
...
class WebSearchTool(BaseTool):
                ...
                try:
                    ...
                    try:
                        ...
                        self.state["logger"].debug(
                            "Using requests-based search as fallback"
                        )
                    except ImportError:
                        self._search_tool = None
    ...
    def _search_with_ddgs(self, query: str, max_results: int) -> list:
        ...
        try:
            ...
            try:
                ...
                self.state["logger"].debug(
                    f"First DDGS method failed: {e}, trying alternative"
                )
                ...
                try:
                    ...
                    self.state["logger"].debug(
                        f"Second DDGS method failed: {e2}, trying basic"
                    )
    ...
    def _run(self, query: str, max_results: int | None = 5) -> str:
        ...
        try:
            ...
            if formatted_results:
                self.state["logger"].info(
                    f"Web search successful: {len(formatted_results)} results"
                )
                return json.dumps(
                    {
                        "status": "success",
            ...
            error_msg = f"Web search error: {e!s}"
...
class WriteToFileTool(BaseTool):
    ...
    def _run(
        self, content: str, filename: str | None = None, append: bool = False
    ) -> str:
        ...
        try:
            ...
            error_msg = f"Error writing to file: {e!s}"
            self.state["logger"].error(error_msg)
            return json.dumps(
                {
                    "status": "error",
