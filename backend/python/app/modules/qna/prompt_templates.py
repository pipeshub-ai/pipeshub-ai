...
<instructions>
  ...
  - **CITATION PLACEMENT RULE: Every factual claim or piece of information derived from blocks MUST be immediately followed by its citation in the SAME sentence where the information appears.**
  ...
  - There must be exactly one citation number inside each pair of square brackets. DO NOT CLUB MULTIPLE citations like [1, 2]
  - **WRONG: Placing all citations at the end like "The system works well. [R1-1][R1-2][R2-3]"**
  - **CORRECT: "The system is secure [R1-1]. It processes data quickly [R1-2]. Users report high satisfaction [R2-3]."**
  - Ensure the assigned numbers map to actual block numbers in the final output using the `blockNumbers` mapping
  - **When a code block ends, the closing line with ``` MUST stand alone. Put any citation (e.g. [3]) on the *next* line, never on the same line as the fence in the code block.**
  - **FOLLOW-UP CONVERSATION RULE: Even in follow-up responses, you MUST maintain the same citation discipline. Do not skip citations just because it's a continuation.**
  ...
  7. Multi-question handling:
      ...
      ii. For any question that cannot be answered with current blocks:
          ...
          - Do NOT skip questions
      iii. Ensure all questions receive equal attention
      iv. **Each answer to each question MUST include proper citations**
</instructions>

<output_format>
  Output format:
  {
    "answer": "<Answer the query in markdown with citations like [R1-1][R2-3] placed immediately after each relevant claim. If based only on user data, say 'User Information'. If enhanced with full record data, indicate appropriately>",
