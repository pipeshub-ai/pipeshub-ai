    ...
    async def index_documents(
        ...
        try:
            ...
            for block in blocks:
                ...
                elif block_type == "record_summary" and isinstance(block.data, dict) and block.data.get("kind") == "file_summary":
                    # File-level symbol summary generated by CodeFileParser
                    code_blocks.append(block)
                elif (
            ...
            # ── Code blocks (BlockType.CODE from tree-sitter parser) ──────────────────
            # Embedding strategy: prepend a structured contextual header before the raw
            # code body (BP2 from the SOTA review).  This "semantic bridge" lets natural-
            # language queries match code chunks (+15-25% retrieval quality).
            # The raw code is stored in blob/block.data["text"]; only page_content carries
            # the prefix so citations always show the original source.
            #
            # Chunking: embed each semantic block (function/class/import) as a single unit.
            # Large blocks (>MAX_CODE_NONWS_CHARS non-whitespace chars) are split via a
            # sliding window so the embedding model never receives a truncated context.
            # The window step is 80% of the budget (20% overlap) to keep continuity.
            MAX_CODE_NONWS_CHARS = 4000   # ~2000 non-whitespace chars → ~4000 raw chars budget
            MAX_CODE_WINDOW_OVERLAP = 0.2  # 20% overlap for sliding-window splits

            if code_blocks:
                file_path = (
                    record.record_name
                    if record is not None and hasattr(record, "record_name")
                    else ""
                )
                # Also embed CODE block groups (classes/structs) from block_groups
                code_block_groups = [
                    bg for bg in block_groups
                    if (
                        str(bg.type.value).lower() if hasattr(bg.type, "value") else str(bg.type).lower()
                    ) == "code"
                ]
                ...
                def _build_code_prefix(
                    ...
                    """Build the contextual header prepended before code text."""
                    lines = []
                    if file_path:
                ...
                def _sliding_window_split(text: str, max_chars: int, overlap: float) -> list[str]:
                    """Split text into overlapping windows of at most max_chars each."""
                    if len(text) <= max_chars:
                        return [text]
                    step = max(1, int(max_chars * (1.0 - overlap)))
                    chunks = []
                    start = 0
                ...
                for block in code_blocks:
                    ...
                    # summaries and surface entire files for broad queries.
                    if block_kind == "file_summary":
                        documents_to_embed.append(
                            Document(
                    ...
                    code_meta = block.code_metadata
                    block_language = (code_meta.language or "") if code_meta else ""
                    signature = (code_meta.signature or "") if code_meta else ""
                    docstring = (code_meta.docstring or "") if code_meta else ""

                    # Build scope chain: parent block group name if available
                    scope: str | None = None
                    if block.parent_index is not None and 0 <= block.parent_index < len(block_groups):
                        parent_bg = block_groups[block.parent_index]
                        if parent_bg.name:
                            scope = parent_bg.name

                    prefix = _build_code_prefix(block_language, file_path, scope, block_kind, signature, docstring)
                    base_metadata = {
                        "virtualRecordId": virtual_record_id,
                        "blockId": block.id,
                        "orgId": org_id,
                    ...
                    # Use subtokenised text if available (improves BM25 sparse recall)
                    embed_text = block_data.get("subtokens") or code_text

                    # Split large blocks via sliding window; embed small/medium blocks as-is
                    chunks = _sliding_window_split(embed_text, MAX_CODE_NONWS_CHARS, MAX_CODE_WINDOW_OVERLAP)
                    for i, chunk in enumerate(chunks):
                        page_content = f"{prefix}\n\n{chunk}" if prefix else chunk
                        chunk_metadata = {**base_metadata}
                        if len(chunks) > 1:
                            chunk_metadata["chunkIndex"] = i
                        documents_to_embed.append(
                            Document(page_content=page_content, metadata=chunk_metadata)
                        )

                    # For functions/methods: also embed a lightweight signature+docstring
                    # summary document so natural-language queries can surface the function
                    # even when the full body is large and split.
                    if block_kind in ("function", "method") and (signature or docstring):
                        summary_text_parts = []
                        if prefix:
                            summary_text_parts.append(prefix)
                        if signature:
                            summary_text_parts.append(signature)
                        if docstring:
                            summary_text_parts.append(docstring[:500])
                        summary_text_content = "\n\n".join(summary_text_parts)
                        documents_to_embed.append(
                            Document(
                                page_content=summary_text_content,
                                metadata={**base_metadata, "isCodeSummary": True},
                            )
                        )
                ...
                for bg in code_block_groups:
                    ...
                    bg_prefix = _build_code_prefix(
                        ...
                        (bg_code_meta.signature or "") if bg_code_meta else None,
                        (bg_code_meta.docstring or "") if bg_code_meta else None,
                    )
                    bg_base_metadata = {
                        "virtualRecordId": virtual_record_id,
                        "blockId": bg.id,
                    ...
                    # Use subtokenised text if available
                    bg_embed_text = bg_data.get("subtokens") or bg_text
                    chunks = _sliding_window_split(bg_embed_text, MAX_CODE_NONWS_CHARS, MAX_CODE_WINDOW_OVERLAP)
                    for i, chunk in enumerate(chunks):
                        page_content = f"{bg_prefix}\n\n{chunk}" if bg_prefix else chunk
                        chunk_meta = {**bg_base_metadata}
                        if len(chunks) > 1:
                            chunk_meta["chunkIndex"] = i
                        documents_to_embed.append(
                            Document(page_content=page_content, metadata=chunk_meta)
                        )

                self.logger.info(
                    f"📊 Added {len(code_blocks)} code block(s) and "
                    f"{len(code_block_groups)} code block group(s) for embedding"
                )
