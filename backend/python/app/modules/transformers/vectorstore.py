...
# flag (see :func:`_provider_has_native_image_path`). This set only
...
def _normalize_azure_ai_endpoint_for_image_client(endpoint: Optional[str]) -> Optional[str]:
    """Convert an Azure AI Foundry endpoint URL into the base URL for
    ``azure-ai-inference``'s ``ImageEmbeddingsClient``.

    ``ImageEmbeddingsClient.embed`` appends ``/images/embeddings`` to the
    provided endpoint, so the endpoint must be:
    - Hub-based (``*.services.ai.azure.com``): ``https://<resource>.services.ai.azure.com/models``
    - Serverless (``*.models.ai.azure.com``): ``https://<deployment>.<region>.models.ai.azure.com``

    Users most often paste the OpenAI-compatible URL from the Azure Portal
    (``https://<resource>.services.ai.azure.com/openai/v1/``). Strip the
    well-known OpenAI suffixes, then append ``/models`` for hub-based
    endpoints so the Azure AI Inference API routes are reachable.

    The Cohere SDK's native ``/v2/embed`` route returns HTTP 404 on
    hub-based ``*.services.ai.azure.com`` endpoints — those only serve
    the Azure AI Inference API at ``/models/images/embeddings``.
    """
    if not endpoint:
        return endpoint
    ...
    for suffix in ("/openai/v1", "/openai"):
        if url.endswith(suffix):
            ...
            break
    url = url.rstrip("/")
    # Hub-based Azure AI Foundry endpoints expose the inference API under
    # /models.  ImageEmbeddingsClient then calls /models/images/embeddings.
    if "services.ai.azure.com" in url and not url.endswith("/models"):
        url = f"{url}/models"
...
class VectorStore(Transformer):
    # Cap for in-memory download when normalizing https/blob ``uri`` to base64.
    _MAX_HTTPS_IMAGE_BYTES = 25 * 1024 * 1024
    ...
    async def _normalize_image_with_mime(
        ...
        ``http`` / ``https`` URLs (typical blob-store ``uri`` from block
        data) are downloaded **in this service** and converted to base64
        so embedding APIs (Cohere, Gemini, etc.) are not asked to fetch
        private or expiring presigned URLs — which was the common cause
        of "indexing success but 0 image vectors in Qdrant".
        ...
        try:
            ...
            if uri.lower().startswith(("http://", "https://")):
                async with httpx.AsyncClient(
                    ...
                    if len(body) > self._MAX_HTTPS_IMAGE_BYTES:
                        self.logger.warning(
                            "Skipping https image: size %d bytes exceeds cap %d",
                            len(body),
                            self._MAX_HTTPS_IMAGE_BYTES,
                        )
                        return None
                ...
                if ct.lower().startswith("image/"):
                    ...
                    # Many blob gateways omit image/*; default is safe for embed APIs.
    ...
    async def _process_image_embeddings_cohere(
        ...
        async def embed_single_image(i: int, image_base64: str) -> Optional[PointStruct]:
            ...
            Always passes a ``data:...;base64,...`` URL. Block ``uri`` values
            are often https blob or presigned URLs: Cohere's servers cannot
            reach private object storage, which previously produced all
            per-image failures and *zero* Qdrant image points while text still
            indexed "successfully".
            """
            normalized = await self._normalize_image_with_mime(image_base64)
            if not normalized:
                self.logger.warning(
                    "Skipping Cohere image index=%s: could not load or decode "
                    "image URI (https/data/base64).",
                    i,
                )
                return None
    ...
    async def _process_image_embeddings_azure_ai(
        self, image_chunks: List[dict], image_base64s: List[str]
    ) -> List[PointStruct]:
        """Process image embeddings via Azure AI Inference ``ImageEmbeddingsClient``.

        Azure AI Foundry hub-based endpoints (``*.services.ai.azure.com``) host
        Cohere Embed v4 (and similar multimodal models) through the **Azure AI
        Inference API** at ``/models/images/embeddings``, not Cohere's native
        ``/v2/embed`` route.  Using ``cohere.ClientV2`` against these endpoints
        returns HTTP 404 because Azure APIM has no route registered for
        ``/v2/embed``.

        ``azure-ai-inference``'s ``ImageEmbeddingsClient`` targets the correct
        route and works with both hub-based (``*.services.ai.azure.com``) and
        serverless (``*.models.ai.azure.com``) Azure AI Foundry deployments.
        """
        from azure.ai.inference import ImageEmbeddingsClient
        from azure.ai.inference.models import ImageEmbeddingInput
        from azure.core.credentials import AzureKeyCredential

        base_url = _normalize_azure_ai_endpoint_for_image_client(self.endpoint)
        if not base_url:
            raise EmbeddingError(
                "Azure AI endpoint is not configured; cannot embed images. "
                "Set the embedding configuration's 'endpoint' field to the "
        ...
        async def embed_single_image(i: int, image_uri: str) -> Optional[PointStruct]:
            normalized = await self._normalize_image_with_mime(image_uri)
            if not normalized:
                self.logger.warning(
                    "Skipping Azure AI image index=%s: could not load or decode "
                    "image URI (https/data/base64).",
                    i,
                )
                return None
            b64, mime_type = normalized
            data_url = f"data:{mime_type};base64,{b64}"
            image_input = ImageEmbeddingInput(image=data_url)
            loop = asyncio.get_running_loop()
            response = await loop.run_in_executor(
                None,
                lambda: client.embed(input=[image_input]),
            )
            chunk = image_chunks[i]
        ...
        async def limited_embed(i: int, image_uri: str) -> Optional[PointStruct]:
            async with semaphore:
                return await embed_single_image(i, image_uri)
        ...
        for result in results:
            if isinstance(result, PointStruct):
                ...
                self.logger.warning(
                    "Failed to embed image via Azure AI Inference: %s", str(result)
                )
    ...
    async def _process_image_embeddings(
        ...
        if self.embedding_provider == EmbeddingProvider.COHERE.value:
            ...
            # Azure AI Foundry exposes Cohere Embed v4 (and other multimodal
            # models) through the Azure AI Inference API, not Cohere's native
            # /v2/embed route.  hub-based (*.services.ai.azure.com) endpoints
            # return HTTP 404 for /v2/embed; the correct route is
            # /models/images/embeddings served by azure-ai-inference's
            # ImageEmbeddingsClient.
            return await self._process_image_embeddings_azure_ai(
    ...
    async def _create_embeddings(
        ...
        Args:
            ...
            caption_fallback: Optional async callable that converts pending
                ...
                doesn't actually accept images). If every image path fails
                and no VLM recovery adds text, :class:`EmbeddingError` is
                raised so the indexing job does not mark success. When
                this callable is absent and images fully fail, the same
                :class:`EmbeddingError` applies; text from the same record
                may still be written before the failure is raised.
        ...
        try:
            ...
            image_qdrant_points = 0
            caption_docs_added = 0
            if image_chunks:
                ...
                try:
                    ...
                    image_qdrant_points = len(points)
                    if not points:
                        self.logger.error(
                            "❌ Image embedding produced 0 Qdrant points for %d image "
                            "block(s) — no dense vectors were stored. Common causes: "
                            "https blob URIs the embedding API could not fetch (now "
                            "we download in-app), invalid presigned URL, Cohere/Azure "
                            "model name mismatch, or API errors (see prior warnings).",
                            len(image_chunks),
                        )
                    await self._store_image_points(points)
                    if (
                        not points
                        and caption_fallback is not None
                    ):
                        try:
                            z_caption_docs = await caption_fallback(image_chunks)
                        except Exception as caption_err:
                            self.logger.error(
                                "VLM caption fallback after empty image points failed: %s",
                                caption_err,
                            )
                            z_caption_docs = []
                        if z_caption_docs:
                            langchain_document_chunks.extend(z_caption_docs)
                            caption_docs_added = len(z_caption_docs)
                            self.logger.info(
                                "✅ Recovered with %d VLM caption document(s) after "
                                "image API returned no vectors.",
                                caption_docs_added,
                            )
                    ...
                    if caption_fallback is not None:
                        ...
                        if caption_docs:
                            ...
                            caption_docs_added = len(caption_docs)
                            self.logger.info(
            ...
            if image_chunks and image_qdrant_points == 0 and caption_docs_added == 0:
                raise EmbeddingError(
                    "All image block(s) failed to produce Qdrant vectors, and VLM "
                    "caption recovery did not run or produced no text. The indexing run "
                    "fails so the job does not show success. "
                    "If the API returns HTTP 404 (Resource not found) from Azure, the "
                    "`model` in AI_MODELS embedding config must be your **Azure deployment "
                    "name** (Deployments in Azure AI Foundry), not only a Cohere catalog "
                    "id like `embed-v4.0`. "
                    f"Value used in Cohere `embed` calls: {self.model_name!r}."
                )
