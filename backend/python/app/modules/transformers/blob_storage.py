...
class BlobStorage(Transformer):
    def __init__(
        self, logger, config_service, arango_service: BaseArangoService = None
    ) -> None:
        self.logger = logger
    ...
    async def apply(self, ctx: TransformContext) -> TransformContext:
        ...
        document_id = await self.save_record_to_storage(
            ...
            async with session.post(url, json=data, headers=headers) as response:
                if response.status != HttpStatusCode.SUCCESS.value:
                    try:
                        ...
                        self.logger.error(
                            "❌ Failed to get signed URL. Status: %d, Error: %s",
                            response.status,
                            error_response,
                        )
                        ...
                        self.logger.error(
                            "❌ Failed to get signed URL. Status: %d, Response: %s",
                            response.status,
                            error_text[:200],
                        )
            ...
            async with session.put(
                ...
                if response.status != HttpStatusCode.SUCCESS.value:
                    try:
                        error_response = await response.json()
                        self.logger.error(
                            "❌ Failed to upload to signed URL. Status: %d, Error: %s",
                            response.status,
                            error_response,
                        )
                    except aiohttp.ContentTypeError:
            ...
            async with session.post(url, json=data, headers=headers) as response:
                if response.status != HttpStatusCode.SUCCESS.value:
                    try:
                        error_response = await response.json()
                        self.logger.error(
                            "❌ Failed to create placeholder. Status: %d, Error: %s",
                            response.status,
                            error_response,
                        )
                    except aiohttp.ContentTypeError:
                        error_text = await response.text()
                        self.logger.error(
                            "❌ Failed to create placeholder. Status: %d, Response: %s",
                            response.status,
                            error_text[:200],
                        )
                    raise aiohttp.ClientError(f"Failed with status {response.status}")
    ...
    async def save_record_to_storage(
        self, org_id: str, record_id: str, virtual_record_id: str, record: dict
    ) -> str | None:
        ...
        try:
            ...
            if storage_type == "local":
                try:
                    async with aiohttp.ClientSession() as session:
                        ...
                        self.logger.info(
                            "📤 Uploading record to storage: %s", record_id
                        )
                        ...
                        async with session.post(
                            ...
                            if response.status != HttpStatusCode.SUCCESS.value:
                                try:
                                    error_response = await response.json()
                                    self.logger.error(
                                        "❌ Failed to upload record. Status: %d, Error: %s",
                                        response.status,
                                        error_response,
                                    )
                                except aiohttp.ContentTypeError:
                                    error_text = await response.text()
                                    self.logger.error(
                                        "❌ Failed to upload record. Status: %d, Response: %s",
                                        response.status,
                                        error_text[:200],
                                    )
                            ...
                            if not document_id:
                                self.logger.error(
                                    "❌ No document ID in upload response"
                                )
                                raise Exception("No document ID in upload response")

                            self.logger.info(
                                "✅ Successfully uploaded record for document: %s",
                                document_id,
                            )
                            return document_id
                except aiohttp.ClientError as e:
                    self.logger.error(
                        "❌ Network error during upload process: %s", str(e)
                    )
                    raise e
                except Exception as e:
                    self.logger.error(
                        "❌ Unexpected error during upload process: %s", str(e)
                    )
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        ...
                        self.logger.info(
                            "📝 Creating placeholder for record: %s", record_id
                        )
                        placeholder_url = (
                            f"{nodejs_endpoint}{Routes.STORAGE_PLACEHOLDER.value}"
                        )
                        ...
                        if not document_id:
                            self.logger.error(
                                "❌ No document ID in placeholder response"
                            )
                            raise Exception("No document ID in placeholder response")

                        self.logger.info(
                            "📄 Created placeholder with ID: %s", document_id
                        )

                        # Step 2: Get signed URL
                        self.logger.info(
                            "🔑 Getting signed URL for document: %s", document_id
                        )
                        upload_data = {
                            "record": record,
                            "virtualRecordId": virtual_record_id,
                        ...
                        upload_result = await self._get_signed_url(
                            session, upload_url, upload_data, headers
                        )

                        signed_url = upload_result.get("signedUrl")
                        if not signed_url:
                            self.logger.error(
                                "❌ No signed URL in response for document: %s",
                                document_id,
                            )
                        ...
                        self.logger.info(
                            "📤 Uploading record to storage for document: %s",
                            document_id,
                        )
                        await self._upload_to_signed_url(
                            session, signed_url, upload_data
                        )

                        self.logger.info(
                            "✅ Successfully completed record storage process for document: %s",
                            document_id,
                        )
                        return document_id
                    ...
                    self.logger.error(
                        "❌ Network error during storage process: %s", str(e)
                    )
                    raise e
                except Exception as e:
                    self.logger.error(
                        "❌ Unexpected error during storage process: %s", str(e)
                    )
                    self.logger.exception("Detailed error trace:")
                    raise e
            ...
            self.logger.error(
                "❌ Critical error in saving record to storage: %s", str(e)
            )
            self.logger.exception("Detailed error trace:")
            raise e
    ...
    async def get_document_id_by_virtual_record_id(self, virtual_record_id: str) -> str:
        ...
        if not self.arango_service:
            self.logger.error(
                "❌ ArangoService not initialized, cannot get document ID by virtual record ID."
            )
            raise Exception(
                "ArangoService not initialized, cannot get document ID by virtual record ID."
            )
        ...
        try:
            ...
            self.logger.info(
                "No document ID found for virtual record ID: %s", virtual_record_id
            )
            return None
        except Exception as e:
            self.logger.error(
                "❌ Error getting document ID by virtual record ID: %s", str(e)
            )
            raise e
    ...
    async def get_record_from_storage(self, virtual_record_id: str, org_id: str) -> str:
        ...
        """
        self.logger.info(
            "🔍 Retrieving record from storage for virtual_record_id: %s",
            virtual_record_id,
        )
        try:
            ...
            document_id = await self.get_document_id_by_virtual_record_id(
                virtual_record_id
            )
            if not document_id:
                self.logger.info(
                        ...
                        if data.get("signedUrl"):
                            ...
                            async with session.get(signed_url, headers=headers) as resp:
                                if resp.status == HttpStatusCode.OK.value:
                                    data = await resp.json()
                        self.logger.info(
                            "✅ Successfully retrieved record for virtual_record_id from blob storage: %s",
                            virtual_record_id,
                        )
    ...
    async def store_virtual_record_mapping(
        self, virtual_record_id: str, document_id: str
    ) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(
                    "✅ Successfully stored virtual record mapping: virtual_record_id=%s, document_id=%s",
                    virtual_record_id,
                    document_id,
                )
                return True
