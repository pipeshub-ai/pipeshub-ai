    ...
    async def save_metadata_to_db(
        ...
        async with self.arango_data_store.transaction() as tx_store:
            try:
                ...
                cursor = tx_store.txn.aql.execute(
                    category_query,
                    bind_vars={"name": metadata.categories[0]},
                )
                try:
                ...
                def handle_subcategory(
                    name, level, parent_key, parent_collection
                ) -> str:
                    ...
                    query = (
                        f"FOR s IN {collection_name} FILTER s.name == @name RETURN s"
                    )
                    cursor = tx_store.txn.aql.execute(
                        query,
                        bind_vars={"name": name},
