...
class SinkOrchestrator(Transformer):
    ...
    async def _sync_record_group_entity(self, ctx: TransformContext) -> None:
        ...
        resolved it. Best-effort and deduplicated via
        ``EntityVectorStore.sync_entity_if_stale`` — many records share the
        same group, so this is cheap after the first reference.
        """
        ...
        try:
            ...
            await self.entity_vector_store.sync_entity_if_stale(
                EntityRecord(
