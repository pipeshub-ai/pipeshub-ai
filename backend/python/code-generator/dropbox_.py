...
class DropboxSDKMethodGenerator:
    ...
    def _construct_api_endpoint(
        self, method_name: str, client_type: str = "user"
    ) -> str:
    ...
    def _is_api_method(self, method_name: str, method_obj: object) -> bool:
        ...
        skip_methods = {
            ...
            "with_path_root",
            "refresh",
            "set_oauth2_access_token",
            "set_oauth2_refresh_token",
    ...
    def _extract_method_info(
        self, method_name: str, method_obj: object, client_type: str
    ) -> Optional[Dict[str, Union[str, Dict, bool]]]:
        ...
        try:
            ...
            return {
                "namespace": namespace,
                "method": method_name.replace(f"{namespace}_", "")
                if namespace
                else method_name,
    ...
    def _format_type_annotation(self, annotation: object) -> str:
        ...
        str_annotation = (
            str(annotation).replace("typing.", "").replace("NoneType", "None")
        )
        ...
        for method_name in dir(team_client_class):
            ...
            if self._is_api_method(method_name, method_obj):
                ...
                if method_info:
                    ...
                    team_method_name = (
                        f"team_{method_name}"
                        if not method_name.startswith("team_")
                        else method_name
                    )
    ...
    def _generate_method_signature(
        self, endpoint_id: str, endpoint_info: Dict[str, Union[str, Dict, bool]]
    ) -> Tuple[str, str]:
            ...
            if param_info.get("required", False):
                ...
                if default_val is not None:
                    ...
                    optional_params.append(
                        f"{clean_name}: {param_type} = {default_val}"
                    )
                else:
                    optional_params.append(
                        f"{clean_name}: Optional[{param_type}] = None"
                    )
    ...
    def _generate_docstring(
        self, endpoint_info: Dict[str, Union[str, Dict, bool]]
    ) -> str:
    ...
    def _generate_method_body(
        ...
        if parameters:
            ...
            params_str = ", ".join(
                [p for p in filtered_params if not p.endswith("=None")]
            )
            if params_str:
        ...
        if client_type == "team" and actual_sdk_method.startswith("team_team_"):
            ...
            client_type == "team"
            and endpoint_id.startswith("team_")
            and not actual_sdk_method.startswith("team_")
        ):
...
'''
        ...
        for endpoint_id, endpoint_info in discovered_methods.items():
            try:
                signature, method_name = self._generate_method_signature(
...
'''
    ...
    def save_to_file(self, filename: Optional[str] = None):
        ...
        print(
            f"✅ Generated Dropbox SDK wrapper with {len(self.generated_methods)} methods"
        )
        print(f"📁 Saved to: {full_path}")

        # Print summary
        user_count = len(
            [m for m in self.generated_methods if m["client_type"] == "user"]
        )
        team_count = len(
            [m for m in self.generated_methods if m["client_type"] == "team"]
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Dropbox API client by introspecting the official SDK"
    )
