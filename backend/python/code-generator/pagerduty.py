...
try:
    ...
    subprocess.run(
        [sys.executable, "-m", "pip", "install", "pagerduty>=5.0.0"], check=True
    )
...
class PagerDutySDKMethodDiscoverer:
    ...
    def _extract_method_info(
        self, method_name: str, method_obj: Any
        ...
        try:
            ...
            docstring = (
                inspect.getdoc(method_obj) or f"PagerDuty SDK method: {method_name}"
            )
...
def generate_method_code(method_info: Dict[str, Any]) -> str:
    ...
    for param_name, param_info in parameters.items():
        ...
        if required and default is None:
            ...
            default is not None and isinstance(default, str) and default.startswith("'")
        ):
        ...
        param_docs.append(
            f"            {param_name}: {param_info.get('description', 'Parameter')}"
        )
    ...
    if custom_impl:
        ...
        return self.client.rput(endpoint, json=data)"""
                ...
                impl = (
                    f"""        return self.client.rput("{method_name}", json=data)"""
                )
        else:
...
'''
    ...
    for category, category_methods in sorted(categories.items()):
        ...
        method_codes.append(
            f"    # {category.upper()} API ({len(category_methods)} methods)"
        )
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate PagerDuty DataSource from SDK"
    )
    parser.add_argument("--out", default=DEFAULT_OUT, help="Output file path")
    parser.add_argument(
        "--class-name", default=DEFAULT_CLASS, help="Generated class name"
    )
