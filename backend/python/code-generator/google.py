...
class GoogleAPIMethodGenerator:
    ...
    def _generate_method_signature(
        ...
        method_full_name = (
            f"{resource_snake}_{method_snake}" if resource_snake else method_snake
        )
    ...
    def _generate_docstring(
        self, method_info: Dict[str, Any], param_docs: List[Dict]
    ) -> str:
        """Generate method docstring."""
        service_config = self.get_service_config()
        description = method_info.get(
            "description", f"{service_config['description']} method"
        )
    ...
    def _generate_method_body(
        ...
        if method_name in {
            ...
            if resource_name:
                client_call = (
                    f"getattr(self.client.{resource_name}(), '{original_method_name}')"
                )
            else:
...
{body_handling}
    ...
    def _process_resource_methods(
        self, resource_name: str, resource_info: Dict[str, Any]
    ) -> List[str]:
        ...
        for method_name, method_info in resource_info["methods"].items():
            required_params, optional_params, param_docs = self._extract_parameters(
                method_info
            )
            ...
            method_body = self._generate_method_body(
                resource_name, method_name, param_docs, method_info
            )
    ...
    def _process_nested_resources(
        ...
        for resource_name, resource_info in resources.items():
            current_path = (
                f"{parent_path}_{resource_name}" if parent_path else resource_name
            )
            ...
            if "resources" in resource_info:
                nested_methods = self._process_nested_resources(
                    resource_info["resources"], current_path
                )
    ...
    def _generate_model_from_schema(
        self, name: str, schema: Dict[str, Any], indent: str = ""
    ) -> str:
            ...
            if not properties:
                ...
                for prop_name, prop_schema in properties.items():
                    ...
                    if prop_name in required:
                        ...
                        class_def += (
                            f"{indent}    {prop_name}: Optional[{prop_type}] = None"
                        )
...
'''
    ...
    def save_to_file(self, filename: str = None):
        ...
        service_config = self.get_service_config()
        print(
            f"✅ Generated {service_config['description']} client with {len(self.generated_methods)} methods"
        )
    ...
    def save_models_to_file(self, filename: str = None):
        ...
        print(
            f"✅ Generated {service_config['description']} models with {schemas_count} schema definitions"
        )
...
"""
    ...
    print("Created space:", created)""",
        ...
        return examples.get(
            self.service_name,
            f"""    # Example methods for {self.service_name} (check generated methods)
...
def process_google_service(
    ...
    print(
        f"🚀 Starting Google {service_name.capitalize()} API {version} method generation..."
    )
    ...
    try:
        ...
        print(
            f"📡 Fetching Google {service_name.capitalize()} API discovery document..."
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Google API clients from Discovery documents"
    )
    parser.add_argument(
        "service",
        nargs="?",
        help="Google API service name (drive, gmail, calendar, sheets, docs, youtube, admin, forms, slides, meet)",
    )
    # Optional alias so you can run: python google.py --api admin
    parser.add_argument(
        "--api",
        dest="api",
        help="Alias for service (e.g., admin, drive, gmail, calendar, sheets, docs, youtube, forms, slides, meet)",
    )
    parser.add_argument(
        "--version", "-v", default="v3", help="API version (default: v3)"
    )
    parser.add_argument("--filename", "-f", help="Output filename (optional)")
    parser.add_argument(
        "--no-models", action="store_true", help="Skip model generation"
    )
    ...
    try:
        process_google_service(
            selected_service, version, args.filename, generate_models=not args.no_models
        )
        return 0
    except Exception as e:
...
def generate_admin_client(
    version: str = "directory_v1", include_models: bool = True
) -> None:
