"""DocuSign Client Code Generator.

This script generates comprehensive DocuSign client code
by analyzing the official docusign-esign SDK.

Usage:
    python code-generator/docusign.py

Generates:
    backend/python/app/sources/client/docusign/docusign.py
"""

from pathlib import Path

# Import DocuSign SDK to introspect available APIs
try:
    ...
    print("Error: docusign-esign SDK not installed")
    print("Run: pip install docusign-esign")
    exit(1)


class DocuSignCodeGenerator:
    """Generator for DocuSign client code."""
    ...
    def __init__(self) -> None:
        ...
        self.api_classes = {
            ...
            "groups": GroupsApi,
            "folders": FoldersApi,
            "bulk_envelopes": BulkEnvelopesApi,
            "workspaces": WorkspacesApi,
        }

    def generate_client_code(self) -> str:
        """Generate complete client code."""
        return '''"""DocuSign API Client using official Python SDK.
...
AUTO-GENERATED by code-generator/docusign.py
"""
...
class DocuSignClient:
    ...
    def get_group(self, group_id: str) -> dict[str, Any]:
        ...
        """
        try:
            response = self.groups.get_groups(self.account_id, group_id)
            return response.to_dict()
        except ApiException as e:
...
'''
    ...
    def run(self) -> None:
        ...
        print("DocuSign Client Code Generator")
        print("=" * 60)

        # Determine project root
        script_dir = Path(__file__).resolve().parent

        # Create /docusign folder next to the script (same as Slack generator pattern)
        docusign_dir = script_dir / "docusign"
        docusign_dir.mkdir(exist_ok=True)

        # Generate client code
        client_code = self.generate_client_code()
        client_path = docusign_dir / "docusign_client.py"
        self.write_file(client_path, client_code)
        ...
        print(f"  {client_path}")
        print("\nNext steps:")
        print("1. Review the generated client file")
        print("2. Run: ruff check --fix app/sources/client/docusign/")
        print("3. Manually create datasource if needed")
        print("4. Test with: python -m app.sources.external.docusign.example")


def main() -> None:
    """Main entry point."""
    generator = DocuSignCodeGenerator()
    generator.run()
