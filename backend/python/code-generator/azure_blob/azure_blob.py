...
class AzureBlobCodeGenerator:
    ...
    def parse_spec(self) -> None:
        ...
        if "parameters" in self.spec:
            for param_name, param_def in self.spec["parameters"].items():
                self.parameter_definitions[param_name] = self._parse_parameter(
            ...
            for method, operation_def in path_item.items():
                if method.lower() in ["get", "post", "put", "delete", "head"]:
                    operation = self._parse_operation(
                        path, method.upper(), operation_def
                    )
                    if operation:
                        self.operations.append(operation)

    def _parse_parameter(
        self, param_def: Dict[str, Any], param_name: str = ""
    ) -> Parameter:
    ...
    def _parse_operation(
        self, path: str, method: str, operation_def: Dict[str, Any]
    ) -> Optional[Operation]:
    ...
    def _get_client_level(self, operation: Operation) -> str:
        ...
        if "service" in operation.tags:
            ...
            tag in operation.tags
            for tag in ["blob", "blockblob", "pageblob", "appendblob"]
        ):
            return "blob"
        else:
    ...
    def _generate_method_signature(self, operation: Operation) -> str:
        ...
        method_name = self.sdk_method_mappings.get(
            operation.operation_id, operation.operation_id.lower()
        )
        ...
        for param in non_path_params:
            if param.required:
                param_name = (
                    param.name.replace("-", "_").replace("x_ms_", "").strip("_")
                )
    ...
    def _generate_method_body(self, operation: Operation) -> str:
        ...
        if client_level == "container" and "container_name" in path_params:
            ...
            for param in operation.parameters:
                if param.name == "snapshot" and not param.required:
                    ...
                    lines.append(
                        '                version_id=version_id if "version_id" in locals() else None,'
                    )
        ...
        for param in operation.parameters:
            if (
                param.location not in ["path", "header"]
                and param.name not in excluded_params
            ):
                param_name = (
                    param.name.replace("-", "_").replace("x_ms_", "").strip("_")
                )
    ...
    def generate_code(self) -> Tuple[str, int]:
        ...
        for operation in self.operations:
            ...
            try:
                ...
                print(
                    f"Warning: Failed to generate method {operation.operation_id}: {e}"
                )
                skipped_operations.append(operation.operation_id)
        ...
        if skipped_operations:
            print(
                f"⚠️  Skipped {len(skipped_operations)} operations without SDK mappings:"
            )
        ...
        # Combine all parts
        code_parts = imports + class_header + methods + utility_methods
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Azure Blob Storage data source from OpenAPI spec"
    )
    parser.add_argument(
        "--input",
        "-i",
        default="azure_spec.json",
        help="Input OpenAPI specification file (JSON)",
    )
    parser.add_argument(
        "--output", "-o", default="azure_blob_data_source.py", help="Output Python file"
    )
    ...
    print(
        f"✅ Generated Azure Blob Storage data source with {successfully_generated} methods"
    )
