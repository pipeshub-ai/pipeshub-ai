...
def generate_method_name(method: str, path: str) -> str:
    ...
    static_segments = [s for s in segments if not (s.startswith('{') and s.endswith('}'))]
    base_name = "_".join([to_snake_case(s) for s in static_segments])
...
class WorkdayCodeGenerator:
    ...
    def _generate_method(self, ep: Dict[str, Any]) -> str:
        ...
        exec_code = [
            ...
            '',
            f'            success = response.status < {self.HTTP_ERROR_THRESHOLD}',
            '            return WorkdayResponse(',
            '                success=success,',
            '                data=response.json() if response.text else None,',
            '                message=f"{method_name} {{\'succeeded\' if success else \'failed\'}}",',
