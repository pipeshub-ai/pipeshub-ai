...
class TeamsOperation:
    ...
    def _determine_operation_type(self) -> str:
        ...
        if "/teams/{team-id}/channels" in path_lower:
            ...
            "/teams/{team-id}/members" in path_lower
            or "/teams/{team-id}/owners" in path_lower
        ):
            return "members"
...
def extract_path_parameters(path: str) -> List[Dict[str, str]]:
    ...
    for match in matches:
        ...
        if "id" in match.lower():
            description = (
                f"Teams {match.replace('-', ' ').replace('_', ' ')} identifier"
            )
...
def _is_teams_operation(path: str, op_id: str) -> bool:
    ...
    return any(
        keyword in path_lower or keyword in op_lower for keyword in teams_core_keywords
    )
...
def extract_teams_operations(spec: Mapping[str, Any]) -> List[TeamsOperation]:
    ...
    for path, item in paths.items():
        ...
        if not any(
            _path_matches_pattern(path, pattern) for pattern in TEAMS_PATH_PATTERNS
        ):
    ...
    print(
        f"Teams filtering summary (MODIFY EXCLUDED_KEYWORDS/EXCLUDED_PATHS to change):"
    )
    print(f"  - Skipped {skipped_count_ops} count operations")
    print(
        f"  - Skipped {skipped_non_teams} excluded operations (OneDrive, Outlook, SharePoint, etc.)"
    )
...
def get_teams_query_parameters(path: str, method: str) -> Tuple[str, str]:
    ...
    if "/teams" in path_lower and method_lower == "get":
        return (
            ...
            "TeamsRequestBuilder.TeamsRequestBuilderGetRequestConfiguration",
        )
    elif "/chats" in path_lower and method_lower == "get":
        return (
            "ChatsRequestBuilder.ChatsRequestBuilderGetQueryParameters",
            "ChatsRequestBuilder.ChatsRequestBuilderGetRequestConfiguration",
        )
        ...
        return (
            "MembersRequestBuilder.MembersRequestBuilderGetQueryParameters",
            "MembersRequestBuilder.MembersRequestBuilderGetRequestConfiguration",
        )
    else:
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Generate comprehensive Teams API client")
    ...
    ap.add_argument(
        "--show-patterns", action="store_true", help="Show all Teams path patterns"
    )
...
def _show_patterns() -> None:
    ...
    pattern_groups = {
        ...
        "Team Management": [
            p
            for p in TEAMS_PATH_PATTERNS
            if p.startswith("/teams/") and "/channels/" not in p and "/members" not in p
        ],
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    try:
        ...
        print(
            f"  - Current exclusions: OneDrive, Outlook, SharePoint, OneNote operations"
        )
