...
class OneDriveOperation:
    ...
    def _determine_operation_type(self) -> str:
        ...
        if "workbook" in path_lower:
            ...
            keyword in path_lower for keyword in ["upload", "content", "createsession"]
        ):
            return "upload"
        elif any(
            keyword in path_lower
            for keyword in ["permission", "invite", "createlink", "share"]
        ):
            return "sharing"
        elif any(
            keyword in path_lower for keyword in ["thumbnail", "preview", "analytics"]
        ):
            return "metadata"
            ...
            keyword in path_lower
            for keyword in ["search", "delta", "recent", "following"]
        ):
            return "discovery"
        else:
            return "file"
...
def extract_path_parameters(path: str) -> List[Dict[str, str]]:
    ...
    for match in matches:
        ...
        if "id" in match.lower():
            description = (
                f"OneDrive {match.replace('-', ' ').replace('_', ' ')} identifier"
            )
...
def _is_onedrive_operation(path: str, op_id: str) -> bool:
    ...
    return any(
        ...
        for keyword in onedrive_core_keywords
    )
...
def extract_onedrive_operations(spec: Mapping[str, Any]) -> List[OneDriveOperation]:
    ...
    for path, item in paths.items():
        ...
        if not any(
            _path_matches_pattern(path, pattern) for pattern in ONEDRIVE_PATH_PATTERNS
        ):
    ...
    print(
        f"OneDrive filtering summary (MODIFY EXCLUDED_KEYWORDS/EXCLUDED_PATHS to change):"
    )
    print(f"  - Skipped {skipped_count_ops} count operations")
    print(
        f"  - Skipped {skipped_non_onedrive} excluded operations (chats, uploads, sharing, workbooks)"
    )
...
def _get_query_param_classes(
    ...
    if "/me/drive/items" in path_lower and method_lower == "get":
        ...
        return (
            "DriveRequestBuilder.DriveRequestBuilderGetQueryParameters",
            "DriveRequestBuilder.DriveRequestBuilderGetRequestConfiguration",
        )
...
def build_onedrive_method_code(op: OneDriveOperation) -> str:
    ...
    params_doc = (
        "\n".join(param_docs)
        if param_docs
        else "            (standard OneDrive parameters)"
    )
...
def _build_onedrive_request_builder_chain(
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    ap.add_argument(
        "--show-patterns", action="store_true", help="Show all OneDrive path patterns"
    )
...
def _show_patterns() -> None:
    ...
    pattern_groups = {
        "Personal OneDrive": [
            p for p in ONEDRIVE_PATH_PATTERNS if p.startswith("/me/drive")
        ],
        "User OneDrive": [p for p in ONEDRIVE_PATH_PATTERNS if p.startswith("/users/")],
        "Group OneDrive": [
            p for p in ONEDRIVE_PATH_PATTERNS if p.startswith("/groups/")
        ],
        "SharePoint Sites": [
            p for p in ONEDRIVE_PATH_PATTERNS if p.startswith("/sites/")
        ],
        "Generic Drives": [
            p
            for p in ONEDRIVE_PATH_PATTERNS
            if p.startswith("/drives") and not p.startswith("/drives/")
        ],
        "Drive Operations": [
            p for p in ONEDRIVE_PATH_PATTERNS if p.startswith("/drives/{drive-id}")
        ],
    }
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    try:
        ...
        print(f"Features:")
        print(
            f"  - Basic OneDrive API endpoint coverage (no uploads/sharing/workbooks)"
        )
