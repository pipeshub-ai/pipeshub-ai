...
class SharePointOperation:
    ...
    def _determine_operation_type(self) -> str:
        ...
        if any(keyword in path_lower for keyword in ["list", "lists", "items"]):
            ...
            for keyword in ["drives", "drive", "root:", "children", "driveitem"]
        ):
            return "drives"
        elif any(
            keyword in path_lower
            for keyword in ["pages", "canvaslayout", "webparts", "horizontalsection"]
        ):
            return "pages"
            ...
            keyword in path_lower
            for keyword in ["columns", "columndefinition", "columnlinks"]
        ):
            return "columns"
        elif any(
            keyword in path_lower for keyword in ["onenote", "notebooks", "sections"]
        ):
            return "onenote"
        elif any(keyword in path_lower for keyword in ["permissions", "permission"]):
            return "permissions"
        elif any(
            keyword in path_lower for keyword in ["analytics", "itemactivitystats"]
        ):
            return "analytics"
        elif any(
            keyword in path_lower
            for keyword in ["termstores", "termstore", "sets", "terms"]
        ):
            return "termStore"
            ...
            keyword in path_lower for keyword in ["informationprotection", "labels"]
        ):
...
def extract_path_parameters(path: str) -> List[Dict[str, str]]:
    ...
    for match in matches:
        ...
        if "id" in match.lower():
            description = (
                f"SharePoint {match.replace('-', ' ').replace('_', ' ')} identifier"
            )
...
def _is_sharepoint_operation(path: str, op_id: str) -> bool:
    ...
    sharepoint_core_keywords = [
        ...
        "onenote",
        "notebooks",
        "sections",  # Site-specific OneNote
        "externalcolumns",
        "columnlinks",
        "basetypes",
    ]
    ...
    if path_lower.startswith("/sites/"):
        return any(
            ...
            for keyword in sharepoint_core_keywords
        )
...
def extract_sharepoint_operations(spec: Mapping[str, Any]) -> List[SharePointOperation]:
    ...
    for path, item in paths.items():
        ...
        if not any(
            _path_matches_pattern(path, pattern) for pattern in SHAREPOINT_PATH_PATTERNS
        ):
    ...
    print(
        f"SharePoint filtering summary (MODIFY EXCLUDED_KEYWORDS/EXCLUDED_PATHS to change):"
    )
    print(f"  - Skipped {skipped_count_ops} count operations")
    print(
        f"  - Skipped {skipped_non_sharepoint} excluded operations (OneDrive personal, Outlook, Teams, etc.)"
    )
...
def _get_query_param_classes(
    ...
    if "/sites/{site-id}/lists" in path_lower and method_lower == "get":
        ...
        return (
            "ContentTypesRequestBuilder.ContentTypesRequestBuilderGetQueryParameters",
            "ContentTypesRequestBuilder.ContentTypesRequestBuilderGetRequestConfiguration",
        )
        ...
        return (
            "SitesRequestBuilder.SitesRequestBuilderGetQueryParameters",
...
def build_sharepoint_method_code(op: SharePointOperation) -> str:
    ...
    params_doc = (
        "\n".join(param_docs)
        if param_docs
        else "            (standard SharePoint parameters)"
    )
...
def _build_sharepoint_request_builder_chain(
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out", help="Output .py file path (default: sharepoint_client.py)"
    )
    ...
    ap.add_argument(
        "--show-patterns", action="store_true", help="Show all SharePoint path patterns"
    )
...
def _show_patterns() -> None:
    ...
    pattern_groups = {
        "Sites & Site Collections": [
            p
            for p in SHAREPOINT_PATH_PATTERNS
        ...
        "Document Libraries": [
            p for p in SHAREPOINT_PATH_PATTERNS if "/drives" in p or "/drive" in p
        ],
        "Modern Pages": [
            p
            for p in SHAREPOINT_PATH_PATTERNS
            if "/pages" in p or "canvas" in p.lower()
        ],
        ...
        "Analytics & Operations": [
            p
            for p in SHAREPOINT_PATH_PATTERNS
            if "/analytics" in p or "/operations" in p
        ],
        "Term Store": [
            p
            for p in SHAREPOINT_PATH_PATTERNS
            if "termStores" in p or "sets" in p or "terms" in p
        ],
        "Permissions & Protection": [
            p
            for p in SHAREPOINT_PATH_PATTERNS
            if "/permissions" in p or "informationProtection" in p
        ],
        "Other Features": [
            p
            for p in SHAREPOINT_PATH_PATTERNS
            if "recycleBin" in p or "external" in p.lower()
        ],
    }
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    try:
        ...
        print(f"Features:")
        print(
            f"  - Complete SharePoint API endpoint coverage (Sites, Lists, Libraries)"
        )
        ...
        print(
            f"  - Current exclusions: Personal OneDrive, Outlook, Teams, Personal OneNote"
        )
