...
from app.sources.client.servicenow.servicenow import ServiceNowClient, ServiceNowResponse"""
    ...
    def _generate_endpoint_method(self, endpoint: EndpointInfo) -> str:
        ...
        if len(params) > 0:
            ...
            method_signature = (
                f"    async def {method_name}(self) -> ServiceNowResponse:"
            )
...
class ConsolidatedAPIGenerator:
    ...
    def __init__(
        self, class_name: str = "ServiceNowDataSource", auth_type: str = "basic"
    ):
    ...
    def save_to_file(
        ...
        print(
            f"✅ Generated {self.class_name} with {self.generation_stats['total_endpoints']} endpoints"
        )
...
def main():
    ...
    parser.add_argument("specs", nargs="+", help="Paths to OpenAPI spec files")
    parser.add_argument(
        "--class-name",
        "-c",
