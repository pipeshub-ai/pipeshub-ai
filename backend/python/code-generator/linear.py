...
class LinearDataSourceGenerator:
    ...
    def _extract_operation_info(
        self, operation_name: str, operation_data: Dict[str, Any], operation_type: str
    ) -> Dict[str, Any]:
        ...
        method_info = {
            ...
            "description": operation_data.get(
                "description", f"Linear GraphQL {operation_type}: {operation_name}"
            ),
    ...
    def _generate_docstring(self, method_info: Dict[str, Any]) -> str:
        ...
        description = method_info.get(
            "description", f"Linear {operation_type}: {operation_name}"
        )
        parameters = method_info.get("parameters", {})
        ...
        docstring += (
            f"        GraphQL Operation: {operation_type.title()} {operation_name}\n"
        )
        ...
        if parameters:
            ...
            for param_name in parameters.keys():
                ...
                variables_lines.append(
                    f'            variables["{param_name}"] = {param_name}'
                )
...
{variables_setup}
    ...
    def _discover_linear_operations(self) -> Dict[str, Dict[str, Any]]:
        ...
        for mutation_name, mutation_data in self.comprehensive_operations[
            "mutations"
        ].items():
            method_info = self._extract_operation_info(
                mutation_name, mutation_data, "mutation"
            )
...
'''
        ...
        query_operations = {
            k: v
            for k, v in discovered_operations.items()
            if v["operation_type"] == "query"
        }
        mutation_operations = {
            k: v
            for k, v in discovered_operations.items()
            if v["operation_type"] == "mutation"
        }
...
'''
    ...
    def save_to_file(self, filename: Optional[str] = None):
        ...
        print(
            f"Generated Linear data source with {len(self.generated_methods)} methods"
        )
        ...
        mutation_count = len(
            [m for m in self.generated_methods if m["type"] == "mutation"]
        )
