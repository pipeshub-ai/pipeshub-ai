...
class TestProcessInBatches:
    ...
    than every document being retained for a final concatenate."""

    @staticmethod
    def _one_block_container(page: int):
        from app.models.blocks import Block, BlocksContainer, BlockType

        return BlocksContainer(
            blocks=[Block(index=0, type=BlockType.TEXT, name=f"page-{page}")]
        )

    @pytest.mark.asyncio
    async def test_each_batch_is_converted_before_the_next_is_parsed(self):
        processor = _make_processor()
        call_order = []

        async def fake_parse(doc_name, content, page_range=None):
            call_order.append(f"parse{page_range}")
            return MagicMock()

        async def fake_create_blocks(doc, page_number=None):
            call_order.append("create_blocks")
            return self._one_block_container(len(call_order))

        processor.parse_document = fake_parse
        processor.create_blocks = fake_create_blocks

        with patch(
            "app.modules.parsers.pdf.docling_processor.get_pdf_page_count", return_value=5
        ):
            result = await processor.process_in_batches("doc.pdf", b"pdf", batch_size=2)

        assert call_order == [
            "parse(1, 2)", "create_blocks",
            "parse(3, 4)", "create_blocks",
            "parse(5, 5)", "create_blocks",
        ]
        assert [b.name for b in result.blocks] == ["page-2", "page-4", "page-6"]
        assert [b.index for b in result.blocks] == [0, 1, 2]

    @pytest.mark.asyncio
    async def test_single_batch_document_skips_the_merge_path(self):
        processor = _make_processor()
        container = self._one_block_container(1)
        processor.parse_document = AsyncMock(return_value=MagicMock())
        processor.create_blocks = AsyncMock(return_value=container)

        with patch(
            "app.modules.parsers.pdf.docling_processor.get_pdf_page_count", return_value=3
        ):
            result = await processor.process_in_batches("doc.pdf", b"pdf", batch_size=10)

        assert result is container
        processor.parse_document.assert_awaited_once_with("doc.pdf", b"pdf")
