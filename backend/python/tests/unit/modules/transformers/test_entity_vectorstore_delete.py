...
class TestCountOrgEntities:
    """count_org_entities must not pass an unsupported ``with_payload`` kwarg
    to ``scroll`` — every provider's scroll signature is
    ``(collection_name, scroll_filter, limit, offset)`` and raises TypeError
    on an unexpected keyword, which the admin status endpoint reported as a
    generic error instead of a count.
    """

    @pytest.mark.asyncio
    async def test_scroll_called_without_with_payload_kwarg(self) -> None:
        vector_db_service = MagicMock()
        vector_db_service.filter_collection = AsyncMock(return_value={"must": []})
        vector_db_service.scroll = AsyncMock(
            return_value=ScrollResult(points=[MagicMock(), MagicMock()], next_offset=None)
        )
        store = _make_store(vector_db_service)

        result = await store.count_org_entities("org-1")

        assert result["count"] == 2
        assert result["is_estimate"] is False
        _, kwargs = vector_db_service.scroll.call_args
        assert "with_payload" not in kwargs

    @pytest.mark.asyncio
    async def test_hits_page_cap_reports_estimate(self) -> None:
        vector_db_service = MagicMock()
        vector_db_service.filter_collection = AsyncMock(return_value={"must": []})
        vector_db_service.scroll = AsyncMock(
            return_value=ScrollResult(points=[MagicMock()], next_offset="cursor")
        )
        store = _make_store(vector_db_service)

        result = await store.count_org_entities("org-1", page_cap=3, page_size=1)

        assert result["count"] == 3
        assert result["is_estimate"] is True

    @pytest.mark.asyncio
    async def test_counts_empty_and_legacy_nested_membership(self) -> None:
        vector_db_service = MagicMock()
        vector_db_service.filter_collection = AsyncMock(return_value={"must": []})
        points = [
            MagicMock(payload={"metadata": {"entityId": "e1"}, "connectorIds": ["c1"]}),
            MagicMock(payload={"metadata": {"entityId": "e2"}, "connectorIds": []}),
            MagicMock(payload={"metadata": {"entityId": "e3", "connectorIds": ["c1"]}}),
        ]
        vector_db_service.scroll = AsyncMock(
