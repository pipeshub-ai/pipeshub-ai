...
# ===================================================================
# _normalize_azure_ai_endpoint_for_image_client
# ===================================================================

class TestNormalizeAzureAiEndpointForImageClient:
    """Tests for the endpoint normaliser used by _process_image_embeddings_azure_ai."""

    def _normalize(self, endpoint):
        from app.modules.transformers.vectorstore import (
            _normalize_azure_ai_endpoint_for_image_client,
        )
        return _normalize_azure_ai_endpoint_for_image_client(endpoint)

    def test_none_returns_none(self):
        assert self._normalize(None) is None

    def test_strips_openai_v1_suffix(self):
        result = self._normalize(
            "https://my-resource.services.ai.azure.com/openai/v1/"
        )
        assert result == "https://my-resource.services.ai.azure.com/models"
    ...
    def test_appends_models_for_services_endpoint(self):
        result = self._normalize(
    ...
    def test_serverless_endpoint_unchanged(self):
        result = self._normalize(
            "https://embed-v-4-0-xyz.eastus.models.ai.azure.com/"
        )
        assert result == "https://embed-v-4-0-xyz.eastus.models.ai.azure.com"
...
# ===================================================================
# _process_image_embeddings_azure_ai
# ===================================================================

class TestProcessImageEmbeddingsAzureAi:
    """Tests for the Azure AI Inference image-embedding handler."""
    ...
    async def test_raises_when_endpoint_missing(self):
        ...
        vs.endpoint = None
        vs.api_key = "k"
        vs.model_name = "embed-v-4-0"

        with pytest.raises(EmbeddingError, match="endpoint"):
            await vs._process_image_embeddings_azure_ai(
                [{"metadata": {}}], ["data:image/png;base64,abc"]
            )

    @pytest.mark.asyncio
    async def test_skips_image_when_normalize_returns_none(self):
        vs = _make_vectorstore()
        vs.endpoint = "https://res.services.ai.azure.com/openai/v1/"
        vs.api_key = "k"
        vs.model_name = "embed-v-4-0"

        with patch.object(vs, "_normalize_image_with_mime", return_value=None), \
             patch("azure.ai.inference.ImageEmbeddingsClient"):
            points = await vs._process_image_embeddings_azure_ai(
                [{"metadata": {"orgId": "o"}}], ["bad-uri"]
            )

        assert points == []

    @pytest.mark.asyncio
    async def test_successful_embed_produces_point(self):
        ...
        chunk = {"metadata": {"orgId": "o", "virtualRecordId": "v1", "blockIndex": 0}, "image_uri": "data:image/png;base64,abc"}

        async def _fake_normalize(uri):
            return ("abc", "image/png")

        with patch.object(vs, "_normalize_image_with_mime", side_effect=_fake_normalize), \
             patch("azure.ai.inference.ImageEmbeddingsClient", return_value=mock_client):
            points = await vs._process_image_embeddings_azure_ai([chunk], ["data:image/png;base64,abc"])
