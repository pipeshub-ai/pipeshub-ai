...
class TestProcessImageEmbeddings:
    ...
    @pytest.mark.asyncio
    async def test_azure_ai_dispatch(self):
        """azureAI provider should dispatch to the Azure AI Inference handler."""
        from app.utils.aimodels import EmbeddingProvider
        vs = _make_vectorstore()
        vs.embedding_provider = EmbeddingProvider.AZURE_AI.value
        vs._process_image_embeddings_azure_ai = AsyncMock(return_value=[])
        await vs._process_image_embeddings([], [])
