...
async def test_empty_optional_fields_are_written_as_null_not_omitted() -> None:
    ...
    """
    docs = await _project(_PYTHON, "client.py", "python")
...
@pytest.mark.asyncio
async def test_referenced_names_sit_on_one_block_per_file() -> None:
    docs = await _project(_PYTHON, "client.py", "python")
    carriers = [doc for doc in docs if doc["referencedNames"] is not None]
    assert len(carriers) == 1
    names = carriers[0]["referencedNames"]
