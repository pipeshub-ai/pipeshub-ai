...
class TestRunConnectorMigration:
    ...
    async def test_success(self, mock_migration_cls):
        ...
        result = await run_connector_migration(container, mock_arango)
        assert result is True
        mock_migration.migrate_all.assert_awaited_once()
        mock_migration_cls.assert_called_once_with(
            arango_service=mock_arango,
            config_service=container.config_service.return_value,
            logger=container.logger.return_value,
        )

    @pytest.mark.asyncio
    @patch("app.containers.connector.ConnectorMigrationService", side_effect=Exception("fail"))
...
class TestRunFilesToRecordsMigration:
    ...
    async def test_success_with_updates(self, mock_migration):
        ...
        result = await run_files_to_records_migration_wrapper(container, mock_arango)
        assert result is True
    ...
    @pytest.mark.asyncio
    @patch("app.containers.connector.run_files_to_records_migration", new_callable=AsyncMock)
    async def test_exception_returns_false(self, mock_migration):
        mock_migration.side_effect = Exception("fail")
        container = MagicMock()
        container.logger.return_value = MagicMock()
        container.config_service.return_value = MagicMock()
...
class TestRunDriveToDriveWorkspaceMigration:
    ...
    async def test_success(self, mock_migration):
        ...
        result = await run_drive_to_drive_workspace_migration_wrapper(container, mock_arango)
        assert result is True
