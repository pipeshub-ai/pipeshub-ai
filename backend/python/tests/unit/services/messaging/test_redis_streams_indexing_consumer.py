...
class TestInitialize:
    ...
    async def test_successful_initialize(self, logger, config):
        ...
        # 2 topics -> 2 xgroup_create + 2 xgroup_delconsumer
...
class TestDrainPending:
    ...
    async def test_drains_pending_messages(self, consumer):
        """Re-processes messages from the PEL, then stops when PEL is empty."""
        consumer.running = True
        consumer.redis = AsyncMock()
        ...
        async def mock_xreadgroup(**kwargs):
            ...
            if call_count == 1:
                return [
                    ("stream-a", [("1-0", _valid_fields()), ("2-0", _valid_fields())])
                ]
            return []  # PEL drained
    ...
    async def test_drain_none_result(self, consumer):
        """None result from xreadgroup exits immediately."""
        consumer.running = True
        consumer.redis = AsyncMock()
    ...
    async def test_drain_results_all_empty_messages(self, consumer):
        """Results exist but all streams have zero messages -> PEL is drained."""
        consumer.running = True
        consumer.redis = AsyncMock()
        consumer.redis.xreadgroup = AsyncMock(
            return_value=[("stream-a", []), ("stream-b", [])]
        )
