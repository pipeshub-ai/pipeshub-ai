...
class TestRedisUpsert:
    @pytest.mark.asyncio
    async def test_upsert_uses_json_set(self, service, mock_redis_client):
        ...
        # First call should be JSON.SET
        first_call = pipeline.execute_command.call_args_list[0]
        assert first_call.args[0] == "JSON.SET"
        assert "coll:p1" in first_call.args[1]
        # Verify the JSON doc includes page_content and dense_embedding
        doc = json.loads(first_call.args[3])
        assert doc["page_content"] == "Hello world"
        assert doc["dense_embedding"] == [0.1, 0.2]
...
class TestRedisDeletePointsPaged:
    ...
    async def test_paged_delete_never_accumulates_all_keys(self):
        ...
        # Patch page_size by patching the constant in the method call scope
        # (the method uses 500; we won't patch, instead verify call count behavior)
        svc.client.delete = AsyncMock()

        with MagicMock() as _:
            # Can't easily change page_size without monkey-patching; just test the
            # behavior when execute_command returns empty on the third call.
            # We verify delete was called for both non-empty pages.
            pass
...
class TestRedisScrollOffset:
    ...
    async def test_scroll_next_offset_none_when_last_page(self):
        ...
        svc = _make_redis_service()
        # total=1, one key, no fields (NOCONTENT-like; but scroll uses RETURN)
        # Format: [total, key, fields_list]
        doc = json.dumps({"page_content": "hello", "metadata": {}})
        svc.client.execute_command = AsyncMock(
            return_value=[1, b"records:k1", ["$", doc.encode()]]
        )
    ...
    async def test_scroll_next_offset_set_when_more_remain(self):
        ...
        svc = _make_redis_service()
        doc = json.dumps({"page_content": "hello", "metadata": {}})
        # total=20, returned 10
        reply = [20] + [
            item
            for _ in range(10)
            for item in [f"records:k{_}".encode(), ["$", doc.encode()]]
        ]
