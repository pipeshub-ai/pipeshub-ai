...
class TestToolsetDeactivationNotification:
    ...
    async def test_deactivation_notifies_owner_with_actions_link(
        ...
        # Payload reason comes from the record, so it can never contradict the persisted field.
        assert kwargs["payload"]["deauthReason"] == "refresh_token_invalid"
    ...
    @pytest.mark.asyncio
    async def test_failed_persistence_does_not_notify(
        self, mock_config_service: MagicMock, mock_notification_service: MagicMock
    ) -> None:
        """set_config returns False instead of raising; an unpersisted deauth must not be
        reported as done — the record is still authenticated and refresh continues."""
        service = ToolsetTokenRefreshService(mock_config_service, notification_service=mock_notification_service)
        mock_config_service.get_config = AsyncMock(
            return_value={"isAuthenticated": True, "toolsetType": "confluence", "orgId": "org-1", "userId": "user-1"}
        )
...
class TestToolsetInvalidClient:
    ...
    async def test_refresh_token_deauth_skips_already_unauthenticated(
        ...
        """Sequential idempotency: an already-deauthed record is not rewritten and its
        owner is not re-notified. The concurrent half of duplicate protection is covered
        by test_concurrent_handlers_for_same_instance_notify_once."""
        service = ToolsetTokenRefreshService(mock_config_service, notification_service=mock_notification_service)
    ...
    async def test_concurrent_handlers_for_same_instance_notify_once(
        ...
        """When the shared client breaks, every user's refresh fails at once — two
        ...
        async def _list(prefix) -> list:
            ...
            return [self.INSTANCE_PATH_1, self.INSTANCE_PATH_2]
        ...
        await asyncio.gather(
            service._handle_invalid_client(self.INSTANCE_PATH_1, InvalidClientError("invalid_client")),
            service._handle_invalid_client(self.INSTANCE_PATH_2, InvalidClientError("invalid_client")),
        )

        assert sorted(writes) == sorted([self.INSTANCE_PATH_1, self.INSTANCE_PATH_2])
