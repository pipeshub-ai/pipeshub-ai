...
class TestOnRecordMetadataUpdateKb:
    @pytest.mark.asyncio
    async def test_calls_process_record_and_handle_updated_record(self):
        proc = _make_processor()
        tx_store = _make_tx_store()
        ...
        with patch.object(proc, "_process_record", new_callable=AsyncMock, return_value=(record, [])) as mock_process, patch.object(
            proc, "_handle_updated_record", new_callable=AsyncMock, return_value=[]
        ) as mock_updated:
            await proc.on_record_metadata_update(record)
