...
class TestSearchInternalKnowledgeInput:
    ...
    def test_record_names_default_none(self):
        inp = SearchInternalKnowledgeInput(query="test")
        assert inp.record_names is None
        assert inp.record_group_names is None

    def test_record_names_custom_values(self):
        inp = SearchInternalKnowledgeInput(
            query="find the doc",
            record_names=["Q3 Security Report"],
            record_group_names=["Legal Contracts"],
        )
        assert inp.record_names == ["Q3 Security Report"]
        assert inp.record_group_names == ["Legal Contracts"]
