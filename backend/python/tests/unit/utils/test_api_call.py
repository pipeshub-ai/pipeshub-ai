...
class _SpyBytesBudget:
    """Records reserve/release calls so tests can assert the reservation
    lifecycle without depending on the resource governor package."""

    def __init__(self) -> None:
        self.reserved_with: list[int | None] = []
        self.released = False

    async def reserve(self, content_length):
        self.reserved_with.append(content_length)

    async def ensure(self, total_bytes_so_far):
        return
...
class TestMakeApiCallByteBudget:
    """make_api_call must reserve after headers arrive/before the body is
    buffered, and release on every failure path — but never on success,
    since the caller holds the reservation for as long as the bytes stay
    resident (plan section 1.3 / phase 2)."""

    @pytest.mark.asyncio
    async def test_reserves_using_content_length_header(self):
        response = _make_mock_response(
            status=200,
            content_type="application/octet-stream",
            binary_data=b"\x00" * 1024,
        )
        response.headers = {"Content-Type": "application/octet-stream", "Content-Length": "1024"}
        session = _make_mock_session(response)
        budget = _SpyBytesBudget()

        with patch("app.utils.api_call.aiohttp.ClientSession", return_value=session):
            result = await make_api_call.retry_with(stop=lambda rs: True)(
                "http://api/file", "token", byte_budget=budget
            )

        assert result["data"] == b"\x00" * 1024
        assert budget.reserved_with == [1024]
        # Held on success — the caller releases it once done with the bytes.
        assert budget.released is False

    @pytest.mark.asyncio
    async def test_reserves_none_when_content_length_missing(self):
        response = _make_mock_response(status=200, content_type="application/octet-stream", binary_data=b"x")
        session = _make_mock_session(response)
        budget = _SpyBytesBudget()

        with patch("app.utils.api_call.aiohttp.ClientSession", return_value=session):
            await make_api_call.retry_with(stop=lambda rs: True)(
                "http://api/file", "token", byte_budget=budget
            )

        assert budget.reserved_with == [None]

    @pytest.mark.asyncio
    async def test_releases_on_http_error(self):
        response = _make_mock_response(status=500, text="Internal Server Error")
        session = _make_mock_session(response)
        budget = _SpyBytesBudget()
    ...
    @pytest.mark.asyncio
    async def test_releases_on_client_payload_error(self):
        payload_error = aiohttp.ClientPayloadError("transfer encoding error")
        response = _make_mock_response(status=200, content_type="application/json")
        response.json = AsyncMock(side_effect=payload_error)
        response.__aenter__ = AsyncMock(return_value=response)
    ...
    @pytest.mark.asyncio
    async def test_no_budget_is_backward_compatible(self):
        """Existing callers that never pass byte_budget see no behaviour
        change at all."""
        response = _make_mock_response(
            status=200, content_type="application/json", json_data={"ok": True}
        )
        session = _make_mock_session(response)

        with patch("app.utils.api_call.aiohttp.ClientSession", return_value=session):
            result = await make_api_call.retry_with(stop=lambda rs: True)("http://api/file", "token")
