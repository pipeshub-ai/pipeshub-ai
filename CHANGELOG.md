...
- **Classification runs as its own pipeline stage, after a record is searchable.** Summary, category, topic and department extraction ran inside the record's indexing attempt while it held its index permit, so a slow or unavailable LLM stalled unrelated records, and a classification retry re-downloaded, re-parsed and re-embedded the record. A record is now searchable as soon as its vectors are written, and classification runs as a `classify` stage with its own topic (`pipeline.classify`), permits (`MAX_CONCURRENT_INDEXING_LLM_CALLS`), retries and dead-lettering. `extractionStatus` reads `QUEUED` until classification finishes, then `COMPLETED`, `SKIPPED` (with the reason) or `FAILED`; an LLM outage pauses classification without spending retries and leaves indexing throughput alone. Reindexing unchanged content with the same model, prompt and departments makes no LLM call when the reindex keeps the record's virtual record id (connector types with reconciliation enabled); other reindexes assign a new one and classify again, and `forceReindex` always does. Stage progress is kept in a new `stageStates` graph collection, and records gain a `contentRev` field; the indexing service applies this schema itself before it starts consuming, so it no longer depends on the connector service starting first. **Kafka:** the indexing service creates `pipeline.classify` at startup with `KAFKA_TOPIC_PARTITIONS` partitions and replication factor 1, as Node does for its topics; where its Kafka principal may not create topics, or the cluster needs more replicas, create the topic before upgrading. If it cannot be created, records are still indexed and searchable, classification waits, and the indexing service retries in the background (`/health` shows `stages.publisher_bound: false` until then). Lag dashboards gain the consumer group `pipeline_classify_group`. `DEFER_EXTRACTION` is removed (it never deferred anything); drop it from custom compose files. Existing records keep their classification. The record page shows each record's parse, search and classification status, connector and collection stats count classification results, and a new "Retry failed classification" action re-runs only classification for the records where it failed (`POST /api/v1/connectors/{id}/reindex` with `stages: ["classify"]`). **Split deployments:** upgrade the Extraction service with or before the indexing service; an older Extraction service's answer to a model outage is recorded as `SKIPPED`. Records the old inline classification left with `extractionStatus` `IN_PROGRESS` (a pod stopped mid-classification) or `NOT_STARTED` (`DEFER_EXTRACTION=true`) are not picked up on their own: re-run classification for them with the same endpoint and `statusFilters: ["IN_PROGRESS", "NOT_STARTED"]` (records indexed before the upgrade get a full reindex). **Rolling back to 0.7.0 on ArangoDB** needs a data cleanup first: 0.7.0's strict records schema rejects every update to a record that carries `contentRev`, `awaitingEventSince` or a status value it does not know. See `docs/indexing-service.md` §2.5.
- **Indexing-time model calls share one cap, one timeout and a circuit breaker per provider, and a provider outage no longer fails records.** Image description, VLM OCR and Excel/CSV table summaries called the model directly, outside the `MAX_CONCURRENT_INDEXING_LLM_CALLS` cap, and the cap itself was counted per event loop, so real concurrency was a multiple of the setting. Every indexing-time call now goes through one gateway per process (indexing, parsing and Extraction services): one cap across event loops, a 600 s ceiling on hangs, and a breaker per provider endpoint that fails fast after five consecutive provider failures and lets one real call probe after 30 s. While a provider is down, an image is left undescribed, a VLM OCR page comes back empty (an outage still stops the document so it is retried), and an Excel or CSV table gets a plain summary naming its rows and columns instead of failing the record. A classification paused by an outage waits it out however long it lasts: each hand-back waits longer (15 s, 1 min, 4 min, then every 5 min, never less than the provider's cooldown) and none counts toward the delivery backstop. The Extraction service's 424 `LLM_UNAVAILABLE` carries `Retry-After` while the provider's circuit is open. New metrics `pipeshub_indexing_llm_calls_total`, `pipeshub_indexing_llm_tokens_total` and `pipeshub_indexing_llm_breaker_open`, and an `llm_gateway` block on the `/health` of the indexing, parsing and Extraction services. VLM OCR also no longer fails on an organization that has never saved AI settings. No action required.
- **Heavy documents are no longer throttled three times over in the all-in-one image.** The indexing, parsing and Docling processes each ran a resource governor on the same container's memory reading, so a PDF waited on three memory brakes in series, each at its own floor and flipping at a different moment: with memory near the soft threshold, heavy parsing slowed to a crawl with CPU idle while Docling returned 429s. The process that admits a parse under its memory brake now tells the services it calls (`x-pipeshub-admitted-in`), and a service that shares its memory admits the parse up to its own fixed ceiling instead of braking it again. Split deployments, where each service has its own memory, are unchanged. No action required.
- **Concurrent classifications can no longer use another organization's model.** The in-process classifier kept the model it resolved for a document on a shared object and read it back after awaiting, so two documents from different organizations classified at the same moment could each be sent with the other's model and credentials. The model is now held per call. The indexing service also runs every message consumer's handlers on one worker event loop, and the ArangoDB, OpenSearch, Redis-vector and accessible-records clients keep one connection per event loop: with a second loop, ArangoDB sessions were being closed under in-flight requests ("Server disconnected"), and the accessible-records cache was bypassed after "attached to a different loop" errors. The HTTP clients' circuit breakers are now safe to share across threads. No action required.
- **Topics, categories and languages are now per organization.** Extraction found or created these taxonomy nodes by name alone, so every org in a deployment shared one "Security" category node, and two records classified at the same moment could each create one. Nodes are now keyed by `(org, collection, normalized name)` and created outside the record's transaction with an insert-if-absent write (new `IGraphDBProvider.ensure_nodes`), which also removes the write conflict on popular terms. Existing name-only nodes stay in place; a record's edges move to its org's nodes the next time it is classified. Filtering by category, topic, department or language is unchanged. No action required.
- **A failed classification no longer looks like a successful one.** When the model returned nothing, the extraction fallback stored an empty-named category and deleted the record's department and topic links, and the Extraction service answered 200 with no classification, so its circuit breaker never saw an LLM outage. The fallback now keeps only the summary and leaves existing links alone; a model failure marks only `extractionStatus` FAILED (the record stays searchable); and the Extraction service answers 424 with an `error_code`: `LLM_UNAVAILABLE` when the provider cannot serve the call (classification pauses and resumes once it is back, spending no retries) or `LLM_FAILED` when the model produced nothing usable (retried, then FAILED). An organization with no LLM configured now gets `extractionStatus` SKIPPED with the reason, not FAILED; the Extraction service answers 422 `LLM_NOT_CONFIGURED`, which is neither retried nor counted against the breaker. The extraction prompt's JSON example is also fixed (doubled braces and a missing comma were reaching the model).
- **Docling reports a failed or timed-out parse as HTTP 422 with an `errorCode`** (`PARSE_TIMEOUT`, `PARSE_FAILED`) instead of 200 with `success: false`. 422 rather than 5xx keeps the client from retrying a document that already used its 40-minute budget and from counting it against the circuit breaker.
- **Spreadsheets processed at the same time no longer read each other's workbook.** The parsing service and the in-process indexer shared one Excel parser whose loaded workbook was instance state, and block creation awaits LLM calls between loading and reading it, so two concurrent `.xlsx` files could produce blocks from the wrong workbook, across organizations. Each document now gets its own parser. Workbook and CSV loading also moved off the indexing event loop.
- **`MAX_TABLE_ROWS_FOR_LLM` now applies to every table, per record.** Only the Excel and CSV parsers honoured it; tables in PDFs, Word documents and connector blocks had every row described by the LLM (a 10,000-row PDF table was about 200 calls). The same running budget (default 1,000 rows per record) now covers all of them, and rows past it get plain `column: value` text. Expect fewer LLM calls on table-heavy documents.
- **Image URLs inside documents can no longer reach loopback, link-local or cloud-metadata addresses.** Images referenced from Markdown, HTML and connector blocks were fetched with no host restriction and redirects were followed blindly, so a document could make the indexer request `169.254.169.254` or a service on `localhost`. Those addresses are now refused on the first request, on every redirect hop, and at connect time (which also covers DNS rebinding). Private networks stay reachable, because self-hosted Confluence and Jira serve attachments from them. Images fetched for agent tool results follow the same policy.
- **Vector points get stable IDs.** IDs were random per write, so a retried or partially completed embedding batch left duplicate points beside the originals. Points are now keyed by virtual record and block (and by text for sentence-level chunks), so a retry overwrites its own points. Existing points are replaced the next time their record is re-indexed.
- **Smaller indexing fixes.** The SQL table/view row limit for stored records now holds when enrichment rewrites the blob (it re-stored every row); the parsing-service path validates the block container before any write, as the in-process path always did; an organization that has never saved AI settings indexes with the default embedding model (every upload failed before); the OCR decision for a PDF samples fixed pages, so a retry of the same file decides the same way; and every record status field accepts the same values, including a new `SKIPPED`. The records list shows an unrecognised status as itself rather than as "Queued".
- **Indexing no longer collapses to a few records once a Jira/Confluence sync hits its attachments.** The consumers' read-ahead cap was one number shared by both document tiers, so attachments (heavy: PDF, Office, images) queued for minutes on the heavy index gate filled it and light pages and issues were never dispatched although the light pool sat idle — throughput dropped to the heavy-parse rate after the first few hundred records, with CPU and memory unused. Dispatch admission is now per tier: heavy may queue only `max(8, 2 × its current index limit)` tasks under the unchanged total (`MAX_PENDING_INDEXING_TASKS` still pins the total), the fair-scheduling key gains a tier level below `(orgId, connectorId)` so a connector's heavy and light records are sibling queues the scheduler can alternate between, and reads pause only when no tier can spawn a task. Same behaviour on Kafka and Redis Streams; the broker-order path (fair scheduling off) keeps the shared total. New gauge `pipeshub_indexing_gate_waiters{broker,tier}` and a `dispatch` block on the indexing service's `/health`. Full write-up in `docs/indexing-service.md` §5.
- **A cancelled graph transaction now rolls back instead of leaking its Neo4j connection.** `asyncio.CancelledError` is a `BaseException`, so the `except Exception` around the transaction context manager let the record-processing timeout (and the lost-lease guard) cancel a record mid-transaction with neither rollback nor commit; the session stayed registered and its pooled connection was never returned. Under load this exhausted the 100-connection pool roughly 25 minutes into a run (`failed to obtain a connection from the pool within 60s`), which slowed every query, tripped more timeouts, and cascaded into Redis lease and retry-counter failures. Both the transaction context manager and the explicit transaction in the entity handler now roll back on any interruption.
- **Indexing in-flight width is now bounded by what downstream can absorb, and adapts to downstream health, on both Kafka and Redis Streams.** The resource governor already derived the parse and index ceilings from the container's CPU quota, but the index ceilings alone could reach 512 records per tier (352 in flight on 16 CPUs) with nothing about Neo4j's 100-connection driver pool, the lease Redis pool or the parsing/embedding services bounding them. The total index budget is now `6 × CPU` between 24 and 96 (heavy sized first, light keeping a reserve), `MAX_CONCURRENT_INDEXING` still overrides it, and each parse ceiling is held to its index tier. Downstream symptoms — a 429, an exhausted Neo4j or Redis connection pool, an unreachable service, repeated request timeouts, or a rising per-record hold time — now feed the governor, which halves (or trims) the index pools within one sample and holds growth until two clean samples have passed. Both consumers read the same gates, so Kafka pauses partitions and Redis stops reading at the same width. The read-ahead cap (`pending_task_ceiling`) follows the live limits instead of the fixed ceilings. No new environment variables: the thresholds are constants, and `MAX_CONCURRENT_PARSING` / `MAX_CONCURRENT_INDEXING` remain optional caps that should normally be left unset.
- **Failure amplifiers under a downstream stall are removed.** The parsing client's read timeout is derived from `RECORD_PROCESSING_TIMEOUT` (80% of it, at most 40 min; it was a flat 40 min, longer than the record budget, so it could never fire) and a retry is only scheduled when it can finish inside that budget; blob-storage uploads share the pooled `aiohttp` session with a per-read stall timeout instead of opening an unbounded session per call; transient blob-storage failures (connection resets, gateway 502/503/504, socket timeouts) are retried up to three times with jittered backoff on the idempotent requests (record fetch, signed-URL fetch, signed upload) so one blip no longer fails a record into a full re-parse, while placeholder creation is retried only when the connection was never established; a cancelled record no longer cancels the XACK/commit/requeue already in flight on the main loop (each cancellation tore down a Redis connection and forced a reconnect against a starved pool); and leases lost because renewals kept failing are no longer released one by one against the Redis that is failing. The Neo4j client retries a lost connection on its existing pool before rebuilding the driver, rebuilds at most once per 30s, ensures the database exists once rather than on every rebuild, fails pool acquisition after 30s instead of 60s, and reaps transaction sessions nobody committed or aborted. `NEO4J_MAX_CONNECTION_POOL_SIZE` (default 100) is the one new setting, for deployments whose Neo4j bolt thread pool is sized differently.
- **Real Neo4j transactions, behind `NEO4J_EXPLICIT_TRANSACTIONS=true` (off by default).** What the graph layer called a transaction was a session on which every query auto-committed, so a rollback could not undo anything and a record cancelled mid-write left partial graph state. With the flag on, `begin_transaction` opens a real transaction (120s server-side timeout), abort rolls it back, and a transaction block that hits a deadlock or lock timeout is re-run up to three times. It ships off for one release because explicit transactions hold node locks until commit, which changes contention on nodes many records write to (departments, categories); enable it on a staging deployment first and watch for `DeadlockDetected` in the indexing logs.
- **Waiting for a parse slot no longer eats a record's processing budget.** Each index tier admits twice its parse slots by design, so a heavy record routinely queues for a slot behind Docling conversions that take many minutes; that wait was charged to `RECORD_PROCESSING_TIMEOUT`, and three such timeouts dead-lettered a record that had never reached the parsing service. The record clock now pauses while queued, the wait is bounded separately (by the same value), a record that is not admitted in time is re-queued without counting an attempt, waits over 5s are logged with their duration, and the admission gate hands out permits in arrival order instead of waking every waiter to race for them.
- **A document the parsing service cannot parse now returns 422, not 500.** Only `UNSUPPORTED_FORMAT` and `INVALID_INPUT` were 422; every other `ParseError` was a 500, which the indexing service's HTTP client treated as a service outage: it retried the document, counted it against the parsing circuit breaker, and five unparseable files in a row opened the breaker so every other record in flight failed fast for 30s and was re-queued to the back of its lane with a retry attempt spent. The client reads the error code from the body on any non-2xx, so the record now fails as a document error on the first attempt and the breaker only opens for real outages.
- **Dead-lettering is more robust on both brokers.** The retry counter's `INCR` and `EXPIRE` are one transaction (a failure between them left a counter with no TTL); a Redis error while acknowledging an abandoned message leaves it pending for the next drain instead of dispatching it as healthy; and the Kafka consumer gains the delivery-count backstop the Redis consumer already had, so a message that crashes its consumer before the failure counter is written is abandoned after `REDIS_MAX_DELIVERIES` deliveries rather than redelivered forever.
...
- **Records whose indexing event was lost are now recovered automatically.** A brand-new connector record was persisted `QUEUED` *before* its event was published, so a failed publish left it `QUEUED` with no event and nothing to ever pick it up (observed: 35 records stuck for hours after a Redis outage). New records are now stored `NOT_STARTED` and promoted to `QUEUED` only once the event is acked, and the stranded-record sweep (`STRANDED_RECORD_REPUBLISH_AFTER_SECONDS`) is **on by default at 3600s** (was off). The sweep records its republish claim *before* sending, so a failed marker write can no longer re-send the same record every tick. It times a record from when it first saw the record waiting (a clock of its own, `awaitingEventSince`, cleared when a consumer picks the record up), not from `updatedAtTimestamp`, which connectors fill with the source system's time and which made every freshly synced record look hours or years old, so events still waiting in the broker were sent again. It also holds re-sends while the record consumer has a backlog (for up to six intervals), since an event behind a backlog is late, not lost. On ArangoDB the sweep's marker was not declared in the strict records schema, so it could not re-send anything there; `awaitingEventSince` is declared.
