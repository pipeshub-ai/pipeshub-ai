...
- **שירותי Python** — מחברים מסנכרנים את המקורות שלך; האינדוקס מנתח אותם ויוצר הטמעות; השאילתה עונה עם ציטוטים.

הנתונים נמצאים בגרף ידע (Neo4j), במאגר וקטורים (Qdrant) וב-MongoDB. Redis הוא המטמון. קבצים נשמרים בדיסק או באחסון אובייקטים. השירותים מעבירים עבודה זה לזה דרך Redis במחשב מקומי, או דרך Kafka בהתקנה גדולה יותר. ראו את [סקירת המערכת](https://docs.pipeshub.com/system-overview).
